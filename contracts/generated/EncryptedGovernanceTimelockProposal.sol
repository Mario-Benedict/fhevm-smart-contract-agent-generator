...
contract EncryptedGovernanceTimelockProposal is ZamaEthereumConfig, Ownable {
    ...
    function propose(string calldata title, string calldata desc, uint256 votingDays) external returns (uint256 id) {
        ...
        proposals[id] = Proposal({
            title: title, description: desc, proposer: msg.sender,
            forVotes: FHE.asEuint64(0), againstVotes: FHE.asEuint64(0), abstainVotes: FHE.asEuint64(0),
            startTime: block.timestamp + 1 days,
            endTime: block.timestamp + 1 days + votingDays * 1 days,
            executionTime: block.timestamp + 1 days + votingDays * 1 days + TIMELOCK_DELAY,
            status: ProposalStatus.Pending, quorumReached: false
        });
