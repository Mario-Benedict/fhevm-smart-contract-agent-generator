...
contract PrivateTelecommunicationsNetworkSharingAgreement is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerDeal(
        ...
        id = dealCount++;
        deals[id] = NetworkSharingDeal({
            mno1: msg.sender, mno2: mno2, sharingModel: sharingModel, generation: generation,
            siteRegionRef: siteRegionRef, sitesShared: sites, infrastructureCostUSD: infraCost,
            mno1CostShareBps: mno1Share, mno2CostShareBps: mno2Share,
            mno1TrafficLoadBps: FHE.asEuint64(0), spectrumContribValueUSD: spectrum,
            energySavingsUSD: energySavings, annualSettlementUSD: FHE.asEuint64(0),
            agreementStart: block.timestamp, agreementEnd: block.timestamp + termDays * 1 days
        });
