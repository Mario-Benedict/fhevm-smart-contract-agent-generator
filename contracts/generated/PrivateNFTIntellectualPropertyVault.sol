...
contract PrivateNFTIntellectualPropertyVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerIPAsset(
        ...
        id = assetCount++;
        ipAssets[id] = IPAsset({
            primaryOwner: msg.sender, ipCategory: ipCategory, ipTitle: ipTitle,
            registrationRef: registrationRef, baseRoyaltyRateBps: baseRoyalty,
            totalLicenseRevenueUSD: FHE.asEuint64(0), totalLicensees: FHE.asEuint32(0),
            ownershipShareBps: ownerShare, active: true
        });
    ...
    function grantLicense(
        ...
        licId = licenseCount++;
        licenses[licId] = LicenseGrant({
            assetId: assetId, licensee: licensee, scope: scope, licenseFeeUSD: fee,
            royaltyRateBps: royaltyRate, revenueReportedUSD: FHE.asEuint64(0),
            royaltyOwedUSD: FHE.asEuint64(0), grantedAt: block.timestamp,
            expiresAt: block.timestamp + durationDays * 1 days, active: true
        });
