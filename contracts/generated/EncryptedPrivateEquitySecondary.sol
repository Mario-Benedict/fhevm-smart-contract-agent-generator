...
contract EncryptedPrivateEquitySecondary is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createListing(
        ...
        id = listingCount++;
        listings[id] = SecondaryListing({
            seller: msg.sender, fundName: fund, vintageYear: vintage,
            navUSD: nav, askPriceUSD: ask, discountToNAVBps: discount,
            unfundedCommitmentUSD: unfunded, ownershipInterestBps: ownership,
            status: ListingStatus.Active, listedAt: block.timestamp, buyer: address(0)
        });
        FHE.allowThis(listings[id].navUSD);
        FHE.allow(listings[id].navUSD, msg.sender);
