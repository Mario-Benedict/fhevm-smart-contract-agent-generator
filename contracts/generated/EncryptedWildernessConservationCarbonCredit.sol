...
contract EncryptedWildernessConservationCarbonCredit is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerConservationArea(
        ...
        areaId = areaCount++;
        areas[areaId] = ConservationArea({
            areaName: name, country: country, ecosystemType: eType, landowner: landowner,
            areaHectares: hectares, carbonSequesteredTCO2: carbon, biodiversityScore: biodiv,
            annualPaymentUSD: payment, totalPaymentsUSD: FHE.asEuint64(0),
            speciesProtected: species, complianceScore: FHE.asEuint8(100),
            covenantStartDate: block.timestamp,
            covenantEndDate: block.timestamp + covenantYears * 365 days,
            status: ConservationStatus.STABLE, verified: false
        });
