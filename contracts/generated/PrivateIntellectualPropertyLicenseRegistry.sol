...
contract PrivateIntellectualPropertyLicenseRegistry is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerIPAsset(
        ...
        id = assetCount++;
        assets[id] = IPAsset({
            owner: msg.sender, ipType: ipType, assetRef: assetRef, jurisdiction: jurisdiction,
            valuationUSD: valuation, royaltyRateBps: royaltyRate, minimumGuaranteedUSD: mg,
            totalRoyaltiesEarned: FHE.asEuint64(0), registered: true
        });
    ...
    function grantLicense(
        ...
        licId = licenseCount++;
        licenses[licId] = License({
            assetId: assetId, licensee: licensee, scope: scope, upfrontFeeUSD: upfront,
            royaltyRateOverrideBps: royOverride, revenueShareBps: revShare,
            royaltiesPaidUSD: FHE.asEuint64(0), grantedAt: block.timestamp,
            expiryDate: block.timestamp + durationYears * 365 days, active: true
        });
