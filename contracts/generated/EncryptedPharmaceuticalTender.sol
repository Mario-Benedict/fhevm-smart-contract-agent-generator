...
contract EncryptedPharmaceuticalTender is ZamaEthereumConfig, Ownable {
    ...
    function publishTender(
        ...
        id = tenderCount++;
        tenders[id] = Tender({
            drugName: drug, formulation: formulation, estimatedDemandUnits: demand,
            maxBudgetUSD: budget, winningBidPrice: FHE.asEuint64(type(uint64).max),
            winner: address(0), deadline: block.timestamp + durationDays * 1 days,
            closed: false, awarded: false
        });
