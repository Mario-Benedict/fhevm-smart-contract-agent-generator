...
contract EncryptedPokerCardGameRNG is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function startHand(address[] calldata players) external onlyOwner returns (uint256 handId) {
        ...
        handId = handCount++;
        hands[handId] = PokerHand({
            players: players,
            phase: GamePhase.PREFLOP,
            pot: FHE.asEuint64(0),
            currentBet: FHE.asEuint64(0),
            rakeUSD: FHE.asEuint64(0),
            deckSeed: seed,
            communityCard1: FHE.asEuint8(0),
            communityCard2: FHE.asEuint8(0),
            communityCard3: FHE.asEuint8(0),
            communityCard4: FHE.asEuint8(0),
            communityCard5: FHE.asEuint8(0),
    ...
    function dealHoleCards(uint256 handId) external onlyOwner {
        ...
        for (uint256 i = 0; i < h.players.length; i++) {
            ...
            playerStates[handId][p] = PlayerState({
                stackUSD: playerBankroll[p],
                totalBetThisHand: FHE.asEuint64(0),
                holeCard1: c1,
                holeCard2: c2,
