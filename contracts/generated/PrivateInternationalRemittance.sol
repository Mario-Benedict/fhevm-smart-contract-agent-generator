...
contract PrivateInternationalRemittance is ZamaEthereumConfig, Ownable {
    ...
    function createOrder(
        ...
        id = orderCount++;
        orders[id] = RemittanceOrder({
            sender: msg.sender, recipientCountry: recipientCountry, recipientBankCode: recipientBankCode,
            senderAmountUSD: sendAmt, recipientAmount: recipientAmt, fxRateMicroUSD: fxRate,
            feesChargedUSD: fees, complianceScore: compScore, status: RemittanceStatus.Pending,
            createdAt: block.timestamp, processedAt: 0
        });
