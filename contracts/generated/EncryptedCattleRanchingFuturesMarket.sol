...
contract EncryptedCattleRanchingFuturesMarket is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerRanch(
        ...
        id = ranchCount++;
        ranches[id] = RanchRegistry({
            rancher: msg.sender, ranchId: ranchId, stateRegion: region,
            headCount: heads, feedCostPerHeadUSD: feedCost, estimatedValueUSD: value,
            mortalityRateBps: mortality, avgGrade: grade, registered: true
        });
    ...
    function openFutures(
        ...
        id = futuresCount++;
        futures[id] = FuturesContract({
            ranchId: ranchId, position: pos, contractedHeads: heads,
            strikePriceCentsPerCWT: strike, currentPriceCentsPerCWT: strike,
            marginPostedUSD: margin, unrealizedPnLUSD: FHE.asEuint64(0),
    ...
    function updateMarketPrice(
        ...
        euint64 diff = FHE.select(priceUp,
            FHE.sub(current, f.strikePriceCentsPerCWT),
            FHE.sub(f.strikePriceCentsPerCWT, current));
    ...
    function closeFutures(uint256 futuresId) external {
        ...
        _totalOpenInterestUSD = FHE.sub(_totalOpenInterestUSD, f.marginPostedUSD);
        FHE.allowThis(_totalOpenInterestUSD);
