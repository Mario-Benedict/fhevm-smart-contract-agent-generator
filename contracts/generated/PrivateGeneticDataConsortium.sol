...
contract PrivateGeneticDataConsortium is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerDataset(
        ...
        id = datasetCount++;
        datasets[id] = GeneticDataset({
            datasetId: datasetId, condition: condition, sampleSize: sampleSize,
            alleleFrequency: freq, oddsRatio: or_, pValueLog: pval,
            accessFeeUSD: fee, qualityScore: quality,
            deidentified: true, accessible: false, contributor: msg.sender
        });
        ...
        FHE.allow(datasets[id].qualityScore, msg.sender);
