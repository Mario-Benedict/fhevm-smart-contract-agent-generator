...
contract EncryptedCrowdfundedInfrastructureBond is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueBond(
        ...
        id = bondCount++;
        bonds[id] = InfrastructureBond({
            projectName: name, infraType: infraType,
            fundingTargetUSD: target, raisedSoFarUSD: FHE.asEuint64(0),
            yieldRateBps: yield_, minInvestmentUSD: min, maxInvestmentUSD: max,
            fundingDeadline: fundingDeadline, bondMaturity: maturity,
            fundingComplete: false, matured: false
        });
    ...
    function invest(
        ...
        euint64 remaining = FHE.sub(bond.fundingTargetUSD, bond.raisedSoFarUSD);
