...
contract PrivateSocialImpactMeasurementBond is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function launchBond(
        ...
        bondId = bondCount++;
        bonds[bondId] = SocialImpactBond({
            bondName: name, targetPopulation: targetPop, primaryOutcome: outcome,
            commissioner: msg.sender, serviceProvider: serviceProvider,
            totalInvestmentUSD: FHE.asEuint64(0), targetOutcomeValue: target,
            achievedOutcomeValue: FHE.asEuint64(0), outcomePaymentUSD: maxPayout,
            investorReturnBps: returnBps, repaymentAmountUSD: FHE.asEuint64(0),
            beneficiaryCount: benefic, evaluationScore: FHE.asEuint8(0),
            contractStart: block.timestamp,
            contractEnd: block.timestamp + durationDays * 1 days,
            status: SIBStatus.FUNDRAISING, outcomePaid: false
        });
        FHE.allowThis(bonds[bondId].totalInvestmentUSD);
        FHE.allowThis(bonds[bondId].targetOutcomeValue);
        FHE.allow(bonds[bondId].targetOutcomeValue, serviceProvider);
