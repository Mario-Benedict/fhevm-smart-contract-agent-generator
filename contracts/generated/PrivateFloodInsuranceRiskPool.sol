...
contract PrivateFloodInsuranceRiskPool is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issuePolicy(
        ...
        id = policyCount++;
        policies[id] = PolicyHolder({
            policyHolder: msg.sender, floodZone: zone, propertyRef: propertyRef,
            insuredValueUSD: insuredValue, annualPremiumUSD: premium, deductibleUSD: deductible,
            floodRiskScoreBps: riskScore, totalPremiumsPaidUSD: FHE.asEuint64(0),
            active: true, policyStart: block.timestamp, policyEnd: block.timestamp + termDays * 1 days
        });
