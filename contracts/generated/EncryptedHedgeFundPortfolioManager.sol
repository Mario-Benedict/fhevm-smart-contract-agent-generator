...
contract EncryptedHedgeFundPortfolioManager is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function openPosition(
        ...
        positions[assetId] = Position({
            assetId: assetId, strategy: strategy, isLong: isLong,
            notionalValueUSD: notional, entryPriceUSD: entryPrice,
            currentPriceUSD: entryPrice, unrealizedPnLUSD: FHE.asEuint64(0),
            marginRequirementUSD: marginReq, active: true
        });
        if (isLong) {
    ...
    function redeem(externalEuint64 encShares, bytes calldata sProof) external nonReentrant {
        ...
        FHE.allow(inv.pendingPerformanceFee, msg.sender);
        FHE.allowThis(fundStats.totalAUM);
        FHE.allowTransient(netAfterFees, msg.sender);
