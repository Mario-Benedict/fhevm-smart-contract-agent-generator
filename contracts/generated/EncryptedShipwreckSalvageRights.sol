...
contract EncryptedShipwreckSalvageRights is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerWreck(
        ...
        id = wreckCount++;
        wrecks[id] = Shipwreck({
            vesselName: vesselName, discoveryCoordinates: coords, sinkingYear: sinkYear,
            estimatedArtifactValue: estVal, highestBidUSD: FHE.asEuint64(0),
            stateRevShareBps: stateShare, finderPercBps: finderPerc,
            highestBidder: address(0),
            auctionEnd: block.timestamp + auctionDays * 1 days,
            status: WreckStatus.AuctionOpen
        });
