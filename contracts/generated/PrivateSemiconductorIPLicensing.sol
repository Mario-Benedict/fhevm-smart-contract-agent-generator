...
contract PrivateSemiconductorIPLicensing is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function grantLicense(
        ...
        id = licenseCount++;
        licenses[id] = IPCoreLicense({
            ipHouse: msg.sender, chipMaker: chipMaker,
            coreType: coreType, licenseType: licenseType,
            coreName: coreName, processNode: processNode,
            upfrontFeeUSD: upfront, royaltyRateBps: royaltyRate,
            minimumRoyaltyUSD: minRoyalty, totalRoyaltiesPaidUSD: FHE.asEuint64(0),
            volumeMilestone: milestone,
            expiryDate: block.timestamp + licenseYears * 365 days,
            status: LicenseStatus.Active
        });
        _totalIPMarketValue = FHE.add(_totalIPMarketValue, upfront);
        FHE.allowThis(licenses[id].upfrontFeeUSD);
        FHE.allow(licenses[id].upfrontFeeUSD, msg.sender);
