...
contract PrivateCreditRatingAgency is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueRating(
        ...
        id = ratingCount++;
        ratings[id] = CreditRating({
            issuer: issuer, category: category, issuerName: issuerName, ratingScore: score,
            debtOutstanding: debt, defaultProbabilityBps: defProb, recoveryRateBps: recovery,
            outlook: outlook, issuedAt: block.timestamp, validUntil: block.timestamp + validityDays * 1 days,
            published: false, leadAnalyst: msg.sender
        });
