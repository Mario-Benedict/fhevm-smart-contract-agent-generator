...
contract EncryptedNuclearFuelCycleManagement is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerReactor(
        ...
        reactors[reactorId] = ReactorUnit({
            reactorName: name, country: country, reactorType: rType,
            thermalPowerMWt: thermal, electricPowerMWe: electric,
            capacityFactorBps: capFactor, fuelCostPerMWhUSD: FHE.asEuint64(0),
            assemblyCount: FHE.asEuint32(0), operational: true
        });
    ...
    function loadFuelAssembly(
        ...
        asmId = assemblyCount++;
        assemblies[asmId] = FuelAssembly({
            assemblyId: assemblyId, reactorType: reactors[reactorId].reactorType,
            enrichmentPct: enrichment, burnupMWdPerTU: FHE.asEuint64(0),
            acquisitionCostUSD: cost, disposalCostReserveUSD: disposal,
            safeguardsScore: safeguards, massKgHeavyMetal: massKg,
            loadDate: block.timestamp, status: FuelStatus.IN_REACTOR
        });
