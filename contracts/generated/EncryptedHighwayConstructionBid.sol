...
contract EncryptedHighwayConstructionBid is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function publishProject(
        ...
        id = projectCount++;
        projects[id] = HighwayProject({
            governmentAuthority: msg.sender, projectCode: code, roadType: rType,
            routeDescription: route, lengthKm: length, engineerEstimateUSD: estimate,
            contingencyPercBps: contingency, performanceBondUSD: bond,
            liquidatedDamagesPerDay: ld,
            tenderClose: block.timestamp + tenderDays * 1 days,
    ...
    function submitBid(
        ...
        bids[bidId] = ContractorBid({
            projectId: projectId, contractor: msg.sender, pkg: pkg,
            totalBidUSD: total, mobilizationCostUSD: mob,
            plantAndEquipmentUSD: plant, laborCostUSD: labor,
            proposedDuration: duration, technicalScore: techScore,
            disqualified: false
        });
