...
contract PrivateWineEnCheursFutures is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createVintageLot(
        ...
        id = lotCount++;
        lots[id] = VintageLot({
            chateau: msg.sender, vintageName: vintageName, region: region, harvestYear: harvestYear,
            totalCasesProduced: totalCases, casesAllocatedToMarket: allocated,
            futurePricePerCaseUSD: futurePrice, vineyardQualityScore: quality,
            projectedReleasePriceUSD: projRelease, status: AllocationStatus.Open,
            offeringStart: block.timestamp, deliveryDate: block.timestamp + deliveryDays * 1 days
        });
