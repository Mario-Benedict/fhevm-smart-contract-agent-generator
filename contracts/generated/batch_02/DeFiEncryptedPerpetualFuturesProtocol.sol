...
contract DeFiEncryptedPerpetualFuturesProtocol is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function closePosition(uint256 positionId) external nonReentrant {
        ...
        if (p.side == PositionSide.Long) {
            m.openInterestLong = FHE.sub(m.openInterestLong, p.notionalCents);
        } else {
            m.openInterestShort = FHE.sub(m.openInterestShort, p.notionalCents);
        }
        _totalCollateralLocked = FHE.sub(_totalCollateralLocked, p.collateralCents);
    ...
    function liquidatePosition(uint256 positionId) external onlyLiquidator nonReentrant {
        ...
        _protocolRevenue = FHE.add(_protocolRevenue, FHE.sub(p.collateralCents, liquidatorReward));
