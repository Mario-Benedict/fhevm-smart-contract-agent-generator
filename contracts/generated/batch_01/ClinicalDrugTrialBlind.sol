...
contract ClinicalDrugTrialBlind is ZamaEthereumConfig, Ownable {
        ...
        external onlyClinician returns (uint256 id)
    {
        ...
        trials[id].controlGroupSize = FHE.asEuint32(0);
        trials[id].treatmentGroupSize = FHE.asEuint32(0);
        trials[id].avgBaselineControl = FHE.asEuint64(0);
        trials[id].avgBaselineTreatment = FHE.asEuint64(0);
        trials[id].status = TrialPhase.Enrollment;
        trials[id].startDate = block.timestamp;
        trials[id].endDate = block.timestamp + durationDays * 1 days;
