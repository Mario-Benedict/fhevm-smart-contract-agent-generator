...
contract GameEncryptedDungeonLoot is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function mintItem(
        ...
        id = itemCount++;
        items[id].itemName = name;
        items[id].itemType = iType;
        items[id].rarityRoll = rarityRoll;
        items[id].attackBonus = atk;
        items[id].defenseBonus = def;
        items[id].magicBonus = mag;
        items[id].marketValue = val;
        items[id].rarity = rarity;
        items[id].tradeable = tradeable;
