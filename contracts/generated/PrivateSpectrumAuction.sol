...
contract PrivateSpectrumAuction is ZamaEthereumConfig, Ownable {
    ...
    function createBand(
        ...
        id = bandCount++;
        bands[id] = FrequencyBand({
            bandName: name, region: region, licenseYears: licenseYears,
            reservePrice: reserve, currentHighest: FHE.asEuint64(0),
            winningBidder: address(0), roundEnd: block.timestamp + roundDurationDays * 1 days,
            round: 1, awarded: false
        });
    ...
    function awardBand(uint256 bandId) external onlyOwner {
        ...
        if (FHE.isInitialized(meetsReserve) && b.winningBidder != address(0)) {
            ...
            FHE.allow(b.currentHighest, b.winningBidder);
