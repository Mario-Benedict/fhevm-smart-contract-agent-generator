...
contract PrivateFractionalArtOwnership is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerArtwork(
        ...
        id = artworkCount++;
        artworks[id] = Artwork({
            artist: artist, title: title, ipfsImage: ipfs, totalShares: totalShares,
            appraisedValueUSD: appraised, insuranceValueUSD: insured, insurancePremiumBps: premium,
            accruedInsuranceCost: FHE.asEuint64(0), acquisitionDate: block.timestamp,
            lastReappraisalDate: block.timestamp, active: true
        });
