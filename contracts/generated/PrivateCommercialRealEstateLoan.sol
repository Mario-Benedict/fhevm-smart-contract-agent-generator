...
contract PrivateCommercialRealEstateLoan is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function originateLoan(
        ...
        id = loanCount++;
        loans[id] = CMBSLoan({
            borrower: borrower, propertyAddress: propertyAddress, propType: propType,
            principalBalanceUSD: principal, appraisedValueUSD: appraisal,
            ltvRatioBps: ltv, annualNOI_USD: noi, dscrBps: dscr,
            interestRateBps: rate, debtServiceReserveUSD: reserve,
            maturityDate: block.timestamp + maturityYears * 365 days,
            originationDate: block.timestamp, status: LoanStatus.Current
        });
