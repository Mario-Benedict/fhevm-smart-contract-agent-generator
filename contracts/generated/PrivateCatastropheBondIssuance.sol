...
contract PrivateCatastropheBondIssuance is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueBond(
        ...
        id = bondCount++;
        bonds[id] = CatBond({
            bondId: bondId, peril: peril,
            principalUSD: principal, couponRateBps: coupon,
            triggerLossUSD: trigger, expectedLossBps: expLoss,
            recoveryRateBps: recovery, totalInvested: FHE.asEuint64(0),
            maturityDate: maturity, triggered: false, matured: false, active: true
        });
