...
contract PrivateOlympicSponsorshipAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createLot(
        ...
        id = lotCount++;
        lots[id] = SponsorshipLot({
            category: category, lotDescription: description, exclusivityTerm: term,
            reservePriceUSD: reserve, winningBidUSD: FHE.asEuint64(0),
            totalBidVolume: FHE.asEuint64(0),
            auctionDeadline: block.timestamp + deadlineDays * 1 days,
            status: AuctionStatus.Open, winner: address(0)
        });
