...
contract EncryptedBlackjackCardGame is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function dealHand(externalEuint64 encBet, bytes calldata proof) external nonReentrant returns (uint256 handId) {
        ...
        handId = handCount++;
        hands[handId] = BlackjackHand({
            player: msg.sender, card1: c1, card2: c2, card3: FHE.asEuint8(0),
            handTotal: total, dealerTotal: dealer, betAmount: bet,
            payout: FHE.mul(bet, FHE.asEuint64(1)), settled: false, playedAt: block.timestamp
        });
