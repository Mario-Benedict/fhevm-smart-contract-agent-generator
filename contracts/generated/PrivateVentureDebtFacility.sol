...
contract PrivateVentureDebtFacility is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function disburseLoan(
        ...
        id = loanCount++;
        loans[id] = VentureDebtLoan({
            startup: startup, debtLender: msg.sender, stage: stage, repaymentType: repayType,
            principalUSD: principal, outstandingBalanceUSD: principal, interestRateBps: rate,
            warrantCoverageBps: warrantCoverage, ipCollateralValueUSD: ipCollateral,
            monthlyRevenueUSD: FHE.asEuint64(0), revenueCapBps: revCap,
            totalRepaidUSD: FHE.asEuint64(0), disbursedAt: block.timestamp,
