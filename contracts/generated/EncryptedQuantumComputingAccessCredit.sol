...
contract EncryptedQuantumComputingAccessCredit is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function purchaseSubscription(
        ...
        _totalQPUHoursSold = FHE.add(_totalQPUHoursSold, qpuHours);
    ...
    function submitJob(
        ...
        euint32 qubits = FHE.fromExternal(encQubits, qProof);
        euint64 jobHours = FHE.fromExternal(encHours, hProof);
        ...
        ebool hoursOk = FHE.le(FHE.add(s.usedQPUHours, jobHours), s.purchasedQPUHours);
        s.usedQPUHours = FHE.add(s.usedQPUHours, jobHours);
        jobs[subscriptionId].push(JobRecord({
            subscriptionId: subscriptionId, circuitDepth: depth, qubitsUsed: qubits,
            hoursConsumed: jobHours, fidelityScore: FHE.asEuint32(0),
            submittedAt: block.timestamp, completed: false
        }));
        jobIndex = jobs[subscriptionId].length - 1;
        FHE.allowThis(depth); FHE.allowThis(qubits); FHE.allowThis(jobHours);
