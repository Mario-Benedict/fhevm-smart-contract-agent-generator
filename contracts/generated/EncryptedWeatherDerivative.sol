...
contract EncryptedWeatherDerivative is ZamaEthereumConfig, Ownable {
    ...
    function createContract(
        ...
        id = contractCount++;
        contracts[id] = WeatherContract({
            buyer: msg.sender, seller: seller, eventType: eventType, location: location,
            strikeDegrees: strike, tickSizeUSD: tick, maxPayoutUSD: maxPayout,
            premiumPaid: premium, actualAccumulatedDeg: FHE.asEuint16(0),
            measureStart: block.timestamp, measureEnd: block.timestamp + measureDays * 1 days,
            settled: false, margined: false
        });
    ...
    function settleContract(uint256 contractId) external {
        ...
        euint16 excessDeg = FHE.select(exceeded,
            FHE.sub(c.actualAccumulatedDeg, c.strikeDegrees),
            FHE.asEuint16(0));
        euint64 rawPayout = FHE.mul(c.tickSizeUSD, FHE.asEuint64(uint64(0))); // cast excessDeg
