...
contract EncryptedSpaceSatelliteDataMarket is ZamaEthereumConfig, Ownable {
    ...
    function listDataset(
        ...
        id = datasetCount++;
        datasets[id] = SatelliteDataset({
            datasetName: name, resolution: resolution, coverageArea: coverage,
            provider: msg.sender, baseAccessFeeUSD: baseFee, perKm2FeeUSD: perKm2,
            totalRevenue: FHE.asEuint64(0), accessGranted: FHE.asEuint32(0), active: true
        });
        FHE.allowThis(datasets[id].baseAccessFeeUSD);
        FHE.allow(datasets[id].baseAccessFeeUSD, msg.sender);
