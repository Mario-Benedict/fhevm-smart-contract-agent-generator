...
contract AgriculturePrivateCommodityHedge is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createHedge(
        ...
        hedges[id] = HedgeContract({
            farmer: msg.sender, commodity: commodity,
            quantityBushels: qty, lockedPrice: lockPrice,
            totalLocked: FHE.sub(totalValue, fee),
            hedgeRatioBps: ratio,
    ...
    function settleHedge(uint256 hedgeId) external onlyOwner nonReentrant {
        ...
        _totalHedgedValue = FHE.sub(_totalHedgedValue, h.totalLocked);
        FHE.allowThis(_totalHedgedValue);
