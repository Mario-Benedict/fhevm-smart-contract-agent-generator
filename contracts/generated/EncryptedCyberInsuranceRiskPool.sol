...
contract EncryptedCyberInsuranceRiskPool is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issuePolicy(
        ...
        euint64 risk = FHE.div(FHE.mul(FHE.sub(FHE.asEuint64(1000), secScore), 6), 10);
        id = policyCount++;
        policies[id] = CyberPolicy({
            insured: insured, vertical: vertical, premiumUSD: premium,
            coverageLimitUSD: limit, deductibleUSD: deductible, cyberSecurityScore: secScore,
            revenueUSD: revenue, dataRecordsCount: dataRecords, riskScoreBps: risk,
            active: true, policyStart: block.timestamp, policyEnd: block.timestamp + duration
        });
    ...
    function adjudicateClaim(uint256 policyId, uint256 claimIdx, externalEuint64 encApproved, bytes calldata proof) external {
        ...
        CyberPolicy storage pol = policies[policyId];
        euint64 maxPayout = FHE.sub(pol.coverageLimitUSD, pol.deductibleUSD);
        ebool withinLimit = FHE.le(approved, maxPayout);
        cl.approvedPayoutUSD = FHE.select(withinLimit, approved, maxPayout);
        cl.paid = true;
        _totalClaimReserve = FHE.sub(_totalClaimReserve, cl.approvedPayoutUSD);
    ...
    function updateSecurityScore(uint256 policyId, externalEuint64 encScore, bytes calldata proof) external {
        ...
        policies[policyId].riskScoreBps = FHE.div(FHE.mul(FHE.sub(FHE.asEuint64(1000), secScore), 6), 10);
