...
contract PrivateArchaeologicalSiteAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function listSite(
        ...
        id = siteCount++;
        sites[id] = Site({
            siteName: name, country: country,
            reservePriceUSD: reserve, environmentalImpactScore: envImpact,
            archaeologicalValueScore: archValue, estimatedArtifacts: artifacts,
            biddingDeadline: deadline, permitDuration: permitDays,
            awarded: false, cancelled: false
        });
