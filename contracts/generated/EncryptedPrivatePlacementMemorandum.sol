...
contract EncryptedPrivatePlacementMemorandum is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createOffering(
        ...
        id = offeringCount++;
        offerings[id] = PrivatePlacement({
            issuer: msg.sender, companyName: companyName, secType: secType,
            offeringSizeUSD: size, minimumInvestmentUSD: minInv, maximumInvestmentUSD: maxInv,
            totalSubscribedUSD: FHE.asEuint64(0), valuationCapUSD: valCap,
            discountRateBps: discount,
            closingDate: block.timestamp + closingDays * 1 days,
            status: OfferingStatus.Open
        });
        FHE.allowThis(offerings[id].offeringSizeUSD); FHE.allow(offerings[id].offeringSizeUSD, msg.sender);
