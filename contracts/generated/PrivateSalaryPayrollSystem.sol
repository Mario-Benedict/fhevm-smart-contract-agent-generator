...
contract PrivateSalaryPayrollSystem is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function onboardEmployee(
        ...
        employees[id] = Employee({
            wallet: wallet, employmentType: empType, employeeRef: employeeRef,
            grossSalaryUSD: gross, taxWithholdingBps: taxRate, deductionsUSD: deductions,
            netSalaryUSD: net, totalPaidUSD: FHE.asEuint64(0), performanceScore: performance, active: true
        });
    ...
    function allowPayrollStats(address viewer) external onlyOwner {
        FHE.allow(_totalPayrollExpenseUSD, viewer);
