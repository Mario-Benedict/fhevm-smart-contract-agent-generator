...
contract PrivateConstructionContractBid is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function postProject(
        ...
        id = projectCount++;
        projects[id] = ConstructionProject({
            projectName: name, specifications: specs, developer: msg.sender,
            estimatedBudgetUSD: budget, winningBidUSD: FHE.asEuint64(type(uint64).max),
            qualityWeightBps: qWeight,
            bidDeadline: block.timestamp + bidDeadlineDays * 1 days,
            projectStartDate: block.timestamp + startDays * 1 days,
            completionDate: block.timestamp + completionDays * 1 days,
            status: ProjectStatus.Open, awardedContractor: address(0)
        });
