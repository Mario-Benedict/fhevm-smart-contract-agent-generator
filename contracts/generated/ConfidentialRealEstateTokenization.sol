...
contract ConfidentialRealEstateTokenization is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function addProperty(
        ...
        properties[id] = Property({
            name: name, location: location, ipfsDocHash: ipfs, totalTokens: totalTokens,
            purchasePrice: price, monthlyRent: rent, vacancyRateBps: vacancy,
            managementFeeBps: mgmtFee, acquisitionDate: block.timestamp, active: true
        });
    ...
    function transferTokens(uint256 propId, address to, externalEuint64 encAmount, bytes calldata proof) external nonReentrant {
        ...
        from.tokenBalance = FHE.sub(from.tokenBalance, actual);
