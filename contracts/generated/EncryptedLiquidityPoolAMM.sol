...
contract EncryptedLiquidityPoolAMM is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createPool(
        ...
        pools[id] = Pool({
            tokenA: tokenA, tokenB: tokenB, reserveA: reserveA, reserveB: reserveB,
            totalLPSupply: initLP, cumulativeFeesA: FHE.asEuint64(0), cumulativeFeesB: FHE.asEuint64(0),
            swapFeeBps: fee, createdAt: block.timestamp
        });
    ...
    function addLiquidity(
        ...
        FHE.allowThis(positions[posId].lpTokensHeld); FHE.allow(positions[posId].lpTokensHeld, msg.sender);
