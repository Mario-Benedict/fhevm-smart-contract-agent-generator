...
contract EncryptedSportsBettingExchange is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createMarket(
        ...
        id = marketCount++;
        markets[id] = BettingMarket({
            eventRef: eventRef, description: description, settlementOracle: settlementOracle,
            totalMatchedUSD: FHE.asEuint64(0), totalCommissionUSD: FHE.asEuint64(0),
            backOdds: backOdds, layOdds: layOdds, status: MarketStatus.Open,
            outcome: BettingOutcome.Unresolved, closingTime: block.timestamp + closingHours * 1 hours
        });
    ...
    function placeBet(uint256 marketId, BetType betType, externalEuint64 encStake, bytes calldata proof) external nonReentrant returns (uint256 betId) {
        ...
        betId = betCount++;
        bets[betId] = Bet({ marketId: marketId, bettor: msg.sender, betType: betType, stakeUSD: netStake, potentialWinUSD: potentialWin, liabilityUSD: liability, odds: oddsUsed, matched: true, settled: false });
