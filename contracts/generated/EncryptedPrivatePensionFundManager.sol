...
contract EncryptedPrivatePensionFundManager is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function enrollMember(
        ...
        members[id] = PensionMember({
            memberWallet: memberWallet, tier: tier, accumulatedBenefitUSD: FHE.asEuint64(0),
            employerContribYTD: FHE.asEuint64(0), employeeContribYTD: FHE.asEuint64(0),
            projectedRetirementBenefit: projBenefit, yearsOfService: yearsService,
            retired: false, enrolledAt: block.timestamp
        });
        _totalLiabilityUSD = FHE.add(_totalLiabilityUSD, projBenefit);
        FHE.allowThis(members[id].accumulatedBenefitUSD); FHE.allow(members[id].accumulatedBenefitUSD, memberWallet);
