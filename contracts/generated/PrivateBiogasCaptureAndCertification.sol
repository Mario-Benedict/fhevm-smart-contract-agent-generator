...
contract PrivateBiogasCaptureAndCertification is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerFacility(
        ...
        id = facilityCount++;
        facilities[id] = BiogasFacility({
            operator: msg.sender, feedstockSource: source, facilityRef: facilityRef, state: state,
            dailyCaptureMMBtu: dailyCapture, annualRNGCertValue: FHE.asEuint64(0),
            tippingFeeUSD: tippingFee, carbonCreditsIssued: FHE.asEuint64(0),
            methaneDestructionBps: methDestruction, totalRevenueUSD: FHE.asEuint64(0),
            certBody: certBody, active: true
        });
