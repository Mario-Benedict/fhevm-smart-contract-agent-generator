...
contract DeFiPrivateSyntheticAsset is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function openPosition(
        ...
        euint64 netMint = FHE.sub(actualMint, fee);
    ...
    function burnAndClose(externalEuint64 encBurnAmount, bytes calldata proof) external nonReentrant {
        ...
        p.minted = FHE.sub(p.minted, actualBurn);
        p.collateral = FHE.sub(p.collateral, collateralReturn);
        if (FHE.isInitialized(burnAll)) p.active = false;
        _totalMinted = FHE.sub(_totalMinted, actualBurn);
    ...
    function liquidate(address user) external onlyOwner nonReentrant {
        ...
        if (FHE.isInitialized(undercollateralized)) {
            p.active = false;
            _totalCollateral = FHE.sub(_totalCollateral, p.collateral);
            _totalMinted = FHE.sub(_totalMinted, p.minted);
