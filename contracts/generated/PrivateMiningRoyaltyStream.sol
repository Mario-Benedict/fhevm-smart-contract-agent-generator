...
contract PrivateMiningRoyaltyStream is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createStream(
        ...
        id = streamCount++;
        streams[id] = RoyaltyStream({
            mineId: mineId, mineOperator: msg.sender, mineralType: mineral,
            royaltyRateBps: rate, productionCapacity: capacity,
            totalProductionKg: FHE.asEuint64(0), totalRoyaltyPaid: FHE.asEuint64(0),
            pricePerKgUSD: spot, streamStart: block.timestamp, streamEnd: streamEnd, active: true
        });
    ...
    function subscribeAsInvestor(
        ...
        FHE.allow(investors[streamId][msg.sender].totalReceivedUSD, msg.sender);
    ...
    function reportProduction(
        ...
        euint64 revenue = FHE.mul(production, st.pricePerKgUSD);
    ...
    function distributeRoyalty(
        ...
        for (uint256 i = 0; i < streamInvestors.length; i++) {
            ...
            inv.yieldBps = FHE.div(FHE.mul(inv.totalReceivedUSD, FHE.asEuint64(10000)), inv.upfrontPaymentUSD);
