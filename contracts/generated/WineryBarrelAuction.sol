...
contract WineryBarrelAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function listBarrel(string calldata vintage, string calldata grape, uint256 liters, uint256 readyYear,
        ...
        id = barrelCount++;
        barrels[id] = Barrel({ vintage: vintage, grape: grape, liters: liters, expectedReadyYear: readyYear,
            winery: msg.sender, reservePrice: reserve, highestBid: FHE.asEuint64(0),
            highestBidder: address(0), auctionEnd: block.timestamp + days_ * 1 days, sold: false });
