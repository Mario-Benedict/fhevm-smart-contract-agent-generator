...
contract PrivateArtificialIntelligenceModelAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function listAIAsset(
        ...
        id = listingCount++;
        listings[id] = AIAssetListing({
            seller: msg.sender, assetType: assetType, modelName: modelName,
            reservePriceUSD: reserve, winningBidUSD: FHE.asEuint64(0),
            royaltyDataBps: royBps, winner: address(0),
            state: AuctionState.Open, closeAt: block.timestamp + durationDays * 1 days
        });
