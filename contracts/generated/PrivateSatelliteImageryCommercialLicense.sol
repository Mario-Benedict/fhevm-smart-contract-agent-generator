...
contract PrivateSatelliteImageryCommercialLicense is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueLicense(
        ...
        id = licenseCount++;
        licenses[id] = ImageryLicense({
            provider: msg.sender, licensee: licensee, resolution: resolution,
            licenseScope: scope, coverageAreaRef: coverageAreaRef,
            annualLicenseFeeUSD: fee, usageVolumeCapSqKm: volumeCap,
            usedVolumeSqKm: FHE.asEuint64(0), exportControlScore: exportScore,
            securityClearanceLevel: secClearance,
            licenseStart: block.timestamp, licenseEnd: block.timestamp + durationDays * 1 days, active: true
        });
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalLicenseRevenueUSD, viewer);
