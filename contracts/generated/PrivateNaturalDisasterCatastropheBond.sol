...
contract PrivateNaturalDisasterCatastropheBond is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueCatBond(
        ...
        id = catBondCount++;
        catBonds[id] = CatBond({
            sponsor: msg.sender, specialPurposeVehicle: spv, perilType: perilType,
            triggerType: triggerType, bondRef: bondRef, principalAtRiskUSD: principal,
            couponRateBps: coupon, expectedLossBps: el, triggerThresholdUSD: trigger,
            totalCouponPaidUSD: FHE.asEuint64(0), recoveryAmountUSD: FHE.asEuint64(0),
            triggered: false, maturityDate: block.timestamp + maturityDays * 1 days
        });
    ...
    function allowBondStats(address viewer) external onlyOwner {
        FHE.allow(_totalPrincipalAtRiskUSD, viewer);
