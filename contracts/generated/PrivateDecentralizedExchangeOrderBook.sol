...
contract PrivateDecentralizedExchangeOrderBook is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function placeOrder(
        ...
        id = orderCount++;
        orders[id] = Order({
            trader: msg.sender, side: side, orderType: orderType,
            tokenIn: tokenIn, tokenOut: tokenOut, amountIn: amtIn,
            limitPrice: limit, stopPrice: stop, filledAmount: FHE.asEuint64(0),
            feeTierBps: feeTier, placedAt: block.timestamp,
            expiryTime: block.timestamp + expiryHours * 1 hours, filled: false, cancelled: false
        });
