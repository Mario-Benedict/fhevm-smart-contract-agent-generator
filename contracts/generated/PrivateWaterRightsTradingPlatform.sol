...
contract PrivateWaterRightsTradingPlatform is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerRight(
        ...
        id = rightCount++;
        rights[id] = WaterRight({
            holder: msg.sender, source: source, useType: useType,
            allocatedAcreFeet: alloc, priorityDateEncoded: priority,
            currentAvailable: alloc, consumedThisSeason: FHE.asEuint64(0),
            estimatedValue: value, seniorRight: seniorRight, transferable: transferable
        });
        ...
        FHE.allow(rights[id].allocatedAcreFeet, msg.sender);
    ...
    function requestTransfer(
        ...
        txId = transferCount++;
        transfers[txId] = WaterTransfer({
            rightId: rightId, seller: msg.sender, buyer: buyer,
            acreFeetTransferred: actualVol, pricePerAcreFoot: price,
            totalPrice: total, transferDate: block.timestamp,
    ...
    function completeTransfer(uint256 txId) external nonReentrant {
        ...
        rights[newId] = WaterRight({
            holder: tx_.buyer, source: wr.source, useType: wr.useType,
            allocatedAcreFeet: tx_.acreFeetTransferred,
            priorityDateEncoded: wr.priorityDateEncoded,
            currentAvailable: tx_.acreFeetTransferred,
            consumedThisSeason: FHE.asEuint64(0),
            estimatedValue: tx_.pricePerAcreFoot,
            seniorRight: false, transferable: true
        });
