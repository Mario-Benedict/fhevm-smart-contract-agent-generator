...
contract PrivateShippingContainerMarket is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function listVoyage(
        ...
        id = voyageCount++;
        voyages[id] = VoyageListing({
            vesselName: vessel, originPort: origin, destinationPort: dest, containerType: cType,
            slotsAvailable: slots, baseRatePerSlotUSD: rate, surchargeUSD: surcharge,
            departureDate: block.timestamp + departureDays * 1 days,
            deadline: block.timestamp + bookingDeadlineDays * 1 days, active: true, carrier: msg.sender
        });
        FHE.allowThis(voyages[id].slotsAvailable);
        FHE.allow(voyages[id].slotsAvailable, msg.sender);
