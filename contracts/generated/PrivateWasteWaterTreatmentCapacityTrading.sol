...
contract PrivateWasteWaterTreatmentCapacityTrading is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerFacility(
        ...
        id = facilityCount++;
        facilities[id] = TreatmentFacility({
            operator: msg.sender, facilityRef: facilityRef, waterAuthority: waterAuthority,
            treatmentLevel: treatmentLevel, capacityMLD: capacity, availableCapacityMLD: capacity,
            pricePerMLDPerMonthUSD: price, effluentQualityScore: quality,
    ...
    function bookCapacity(
        ...
        bookings[bookingId] = CapacityBooking({
            facilityId: facilityId, industrialUser: msg.sender, effluentClass: effluentClass,
            bookedCapacityMLD: effectiveCap, monthlyFeeUSD: monthlyFee,
            pollutionPenaltyUSD: FHE.asEuint64(0),
            bookingStart: block.timestamp, bookingEnd: block.timestamp + durationMonths * 30 days, active: true
        });
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalTreatmentRevenueUSD, viewer);
