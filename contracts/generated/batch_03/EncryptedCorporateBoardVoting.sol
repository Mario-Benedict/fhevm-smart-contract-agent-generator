...
contract EncryptedCorporateBoardVoting is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function proposeResolution(
        ...
        resolutions[id].resType = resType;
        resolutions[id].votesFor = FHE.asEuint64(0);
        resolutions[id].votesAgainst = FHE.asEuint64(0);
        resolutions[id].quorumThreshold = quorum;
        resolutions[id].superMajorityThreshold = superMaj;
        resolutions[id].budgetApproved = budget;
        resolutions[id].voterCount = 0;
        resolutions[id].passed = false;
        resolutions[id].deadline = block.timestamp + deadlineDays * 1 days;
