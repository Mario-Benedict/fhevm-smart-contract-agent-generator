...
contract EncryptedLuxuryWatchAuthentication is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    event WatchRegistered(uint256 indexed id, WatchBrand brand, string _reference);
    ...
    function registerWatch(
        ...
        id = watchCount++;
        watches[id].currentOwner = msg.sender;
        watches[id].brand = brand;
        watches[id].modelName = modelName;
        watches[id].referenceNumber = _reference;
        watches[id].serialHash = serial;
        watches[id].originalRetailUSD = retail;
        watches[id].currentInsuredValueUSD = insured;
        watches[id].lastServiceCostUSD = FHE.asEuint64(0);
        watches[id].conditionScore = condition;
        watches[id].manufactureYear = manufactureYear;
        ...
        FHE.allowThis(_totalRegisteredValue);
        emit WatchRegistered(id, brand, _reference);
    }
