...
contract EncryptedNaturalGasPipelineCapacity is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function listSlot(
        ...
        id = slotCount++;
        slots[id].pipelineId = pipelineId;
        slots[id].entryPoint = entry;
        slots[id].exitPoint = exit;
        slots[id].capacityMMBTUperDay = cap;
        slots[id].reservationPriceUSD = reserve;
        slots[id].awardedTariffUSD = FHE.asEuint64(0);
        slots[id].auctionEnd = block.timestamp + auctionDays * 1 days;
        slots[id].status = SlotStatus.Bidding;
        slots[id].winner = address(0);
