...
contract EncryptedQuadraticVotingProtocol is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerVoter(address voter, externalEuint64 encCredits, bytes calldata proof) external onlyOwner {
        ...
        FHE.allowThis(balances[voter].totalCredits); FHE.allow(balances[voter].totalCredits, voter);
    ...
    function createProposal(
        ...
        id = proposalCount++;
        proposals[id] = QVProposal({
            title: title, description: description, proposer: msg.sender,
            totalVoiceCreditsSpent: FHE.asEuint64(0), uniqueVoterCount: FHE.asEuint64(0),
            fundingMatchEstimate: FHE.asEuint64(0), budgetRequested: budget,
            deadline: block.timestamp + deadlineDays * 1 days, funded: false
        });
