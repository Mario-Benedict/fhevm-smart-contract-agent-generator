...
contract PrivateAntiMoneyLaunderingCompliance is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function recordAssessment(
        ...
        id = assessmentCount++;
        assessments[id] = TransactionRiskAssessment({
            sender: sender, receiver: receiver, amountUSD: amount, riskScore: riskScore,
            sanctionsMatchScore: sanctions, pepExposureScore: pep, jurisdictionRiskScore: jur,
            riskLevel: level, assessedAt: block.timestamp, flagged: flagged
        });
        if (flagged) {
