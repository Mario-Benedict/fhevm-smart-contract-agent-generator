...
contract PrivateDefenseContractorBidding is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createProgram(
        ...
        id = programCount++;
        programs[id] = ProcurementProgram({
            programName: name, classificationLevel: classification, systemType: stype,
            estimatedBudgetUSD: budget, technicalRequirements: techReq,
            minCapabilityScore: minCap, biddingDeadline: deadline, deliveryDate: delivery,
            awardDecided: false, classified: true
        });
    ...
    function submitBid(
        ...
        bidId = bidCount++;
        bids[bidId] = ContractorBid({
            programId: programId, contractor: msg.sender,
            bidPriceUSD: price, lifecycleCostUSD: lifecycle,
            technicalScore: techScore, capabilityScore: capability,
            itarComplianceScore: itar, clearanceLevel: clearance,
            disqualified: false, selected: false
        });
