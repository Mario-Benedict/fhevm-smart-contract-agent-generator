...
contract PrivateCottonTextileMillProcurement is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function placeProcurementOrder(
        ...
        id = orderCount++;
        orders[id] = CottonProcurementOrder({
            gin: gin, mill: msg.sender, origin: origin, balesOrdered: bales,
            pricePerBaleUSD: price, totalOrderValueUSD: totalValue, stapleLength32nds: staple,
            micronaireBps: micronaire, strengthGramsTex: strength,
            deliveryDate: block.timestamp + deliveryDays * 1 days, delivered: false
        });
