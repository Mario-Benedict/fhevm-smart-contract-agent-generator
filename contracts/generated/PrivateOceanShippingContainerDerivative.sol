...
contract PrivateOceanShippingContainerDerivative is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function openDerivative(
        ...
        id = derivativeCount++;
        derivatives[id] = FreightDerivative({
            trader: msg.sender, route: route, derivType: derivType, side: side,
            contractSizeTEU: teu, strikeIndexBps: strikeIndex, currentIndexBps: strikeIndex,
            marginPostedUSD: margin, unrealizedPnLUSD: FHE.asEuint64(0),
            expiryDate: block.timestamp + expiryDays * 1 days, settled: false
        });
    ...
    function updateMarketIndex(
        ...
        euint64 longPnL = FHE.sub(currentIdx, d.strikeIndexBps);
