...
contract EncryptedAnonymousWhistleblowerRewardScheme is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    struct Report {
        ...
        address reporter;             // could be address(0) for anonymous
        bool anonymous;
        bool reviewed;
        bool rewarded;
        uint256 submittedAt;
    ...
    constructor(
        externalEuint64 encInitPool, bytes memory poolProof,
        externalEuint32 encRate, bytes memory rateProof,
        externalEuint32 encMinScore, bytes memory scoreProof
    ) Ownable(msg.sender) {
    ...
    function submitReport(
        ...
        r.reporter = anonymous ? address(0) : msg.sender;
    ...
    function reviewReport(
        bytes32 reportId,
        externalEuint32 encCredibility, bytes calldata credProof,
        externalEuint32 encCompleteness, bytes calldata compProof
    ) external onlyOwner {
        Report storage r = reports[reportId];
        ...
        r.rewardEntitlement = FHE.select(qualifies, baseReward, FHE.asEuint64(0));
        r.reviewed = true;
        FHE.allowThis(r.credibilityScore);
        FHE.allowThis(r.completenessScore);
        FHE.allowThis(r.rewardEntitlement);
    ...
    function distributeReward(bytes32 reportId, address recipientAddress) external onlyOwner nonReentrant {
        Report storage r = reports[reportId];
        require(r.reviewed && !r.rewarded, "Not eligible");
        ebool poolSufficient = FHE.ge(_rewardPool, r.rewardEntitlement);
        euint64 payment = FHE.select(poolSufficient, r.rewardEntitlement, _rewardPool);
        _rewardPool = FHE.sub(_rewardPool, payment);
        _totalRewardsDistributed = FHE.add(_totalRewardsDistributed, payment);
        _totalRecoveryEnabled = FHE.add(_totalRecoveryEnabled, r.potentialRecovery);
        r.rewarded = true;
        FHE.allowThis(_rewardPool);
        FHE.allowThis(_totalRewardsDistributed);
        FHE.allowThis(_totalRecoveryEnabled);
