...
contract PrivateMiningRoyaltyStreamToken is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerMiningAsset(
        ...
        id = assetCount++;
        assets[id] = MiningAsset({
            miningCompany: msg.sender, mineral: mineral, structure: structure, mineRef: mineRef,
            country: country, reservesEstTonnes: reserves, annualProductionTonnes: production,
            royaltyRateBps: royaltyRate, mineralPriceUSDPerTonne: mineralPrice, oreGrade: oreGrade, active: true
        });
    ...
    function distributeRoyalty(uint256 dealId, externalEuint64 encTonnesProduced, bytes calldata proof) external whenNotPaused nonReentrant {
        ...
        FHE.allow(royaltyAmt, d.streamingBuyer);
