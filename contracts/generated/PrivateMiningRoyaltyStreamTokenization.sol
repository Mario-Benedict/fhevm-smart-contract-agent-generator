...
contract PrivateMiningRoyaltyStreamTokenization is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createRoyaltyToken(
        ...
        tokenId = tokenCount++;
        royaltyTokens[tokenId] = MiningRoyaltyToken({
            mineName: mineName, jurisdiction: jurisdiction,
            commodity: commodity, royaltyType: royaltyType,
            royaltyOwner: msg.sender, miningCompany: miningCompany,
            royaltyRateBps: rate, productionAnnualOz: production,
            spotPriceUSD: spotPrice, annualRoyaltyUSD: annualRoyalty,
            npvRoyaltyStreamUSD: npv, reservesRemainingOz: reserves,
            mineLifeYears: mineLife, technicalRiskScore: techRisk,
            tokenizationDate: block.timestamp, active: true
        });
        _totalNPVPortfolio = FHE.add(_totalNPVPortfolio, npv);
        FHE.allowThis(royaltyTokens[tokenId].royaltyRateBps);
        FHE.allow(royaltyTokens[tokenId].royaltyRateBps, msg.sender);
    ...
    function distributeRoyalty(uint256 tokenId, address holder) external nonReentrant {
        require(isRoyaltyManager[msg.sender], "Not manager");
        euint64 holderShare = FHE.mul(holders[tokenId][holder].tokensOwned, royaltyTokens[tokenId].annualRoyaltyUSD);
