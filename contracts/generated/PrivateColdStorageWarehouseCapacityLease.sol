...
contract PrivateColdStorageWarehouseCapacityLease is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createLease(
        ...
        id = leaseCount++;
        leases[id] = ColdStorageLease({
            warehouseOperator: msg.sender, lessee: lessee, temperatureZone: tempZone,
            goodsCategory: goodsCategory, warehouseRef: warehouseRef, palletPositionsLeased: pallets,
            monthlyRatePerPalletUSD: rate, handlingFeePerPalletUSD: handlingFee,
            temperatureComplianceBps: compliance, hazardousSurchargeUSD: hazardous,
            totalMonthlyBillUSD: monthlyBill, totalPaidUSD: FHE.asEuint64(0),
            leaseStart: block.timestamp, leaseEnd: block.timestamp + termMonths * 30 days, active: true
        });
