...
contract PrivateCommodityForwardContract is ZamaEthereumConfig, Ownable {
    ...
    function createForward(
        ...
        id = forwardCount++;
        forwards[id].buyer = msg.sender;
        forwards[id].seller = seller;
        forwards[id].commodity = commodity;
        forwards[id].forwardPrice = fwdPrice;
        forwards[id].quantity = qty;
        forwards[id].margin = margin;
        forwards[id].spotPrice = FHE.asEuint64(0);
        forwards[id].settlementPnL = FHE.asEuint64(0);
        forwards[id].deliveryDate = block.timestamp + deliveryDays * 1 days;
        forwards[id].status = SettlementStatus.Pending;
