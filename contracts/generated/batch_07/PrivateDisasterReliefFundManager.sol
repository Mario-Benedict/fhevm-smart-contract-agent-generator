...
contract PrivateDisasterReliefFundManager is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function declareDisaster(
        ...
        id = eventCount++;
        events[id].eventRef = eventRef;
        events[id].location = location;
        events[id].disasterType = disasterType;
        events[id].estimatedDamageUSD = damage;
        events[id].fundsAllocatedUSD = allocation;
        events[id].fundsDistributedUSD = FHE.asEuint64(0);
        events[id].beneficiaryCount = FHE.asEuint32(0);
        events[id].declaredAt = block.timestamp;
