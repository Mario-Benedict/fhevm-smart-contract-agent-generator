...
contract PrivateElectricVehicleFleetLease is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerAsset(
        ...
        id = assetCount++;
        EVAsset storage _s0 = assets[id];
        _s0.vin = vin;
        _s0.vehicleType = vType;
        _s0.make = make;
        _s0.model = model;
        _s0.modelYear = modelYear;
        _s0.batteryCapacityKWh = cap;
        _s0.batteryHealthPercBps = soh;
        _s0.residualValueUSD = residual;
        _s0.odometer = FHE.asEuint32(0);
        _s0.currentLeaseRateUSD = FHE.asEuint64(0);
        _s0.status = LeaseStatus.Available;
    ...
    function createLease(
        ...
        id = leaseCount++;
        leases[id].lessee = lessee;
        leases[id].vehicleId = vehicleId;
        leases[id].monthlyRateUSD = rate;
        leases[id].contractedMileage = mileage;
        leases[id].depositUSD = deposit;
        leases[id].totalPaidUSD = FHE.asEuint64(0);
        leases[id].excessMileageRateCents = excessRate;
        leases[id].startDate = block.timestamp;
        leases[id].endDate = block.timestamp + leaseDays * 1 days;
        leases[id].active = true;
