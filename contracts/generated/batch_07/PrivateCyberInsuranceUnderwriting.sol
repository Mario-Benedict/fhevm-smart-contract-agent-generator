...
contract PrivateCyberInsuranceUnderwriting is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createOrgProfile(
        ...
        OrganizationProfile storage org = orgs[msg.sender];
        org.orgName = name;
        org.industry = industry;
        org.annualRevenueUSD = revenue;
        org.employeeCount = employees;
        org.securityPostureScore = posture;
        org.mfaAdoptionPct = mfa;
        org.patchCompliancePct = FHE.asEuint8(0);
        org.cyberTrainingScore = FHE.asEuint8(0);
        org.breachHistoryYears = FHE.asEuint16(0);
        org.dataRecordsStored = records;
        org.registered = true;
        org.premiumWaived = false;
    ...
    function issuePolicy(
        ...
        CyberPolicy storage _s0 = policies[policyId];
        _s0.insured = insured;
        _s0.insurer = msg.sender;
        _s0.coverageTypes = defaultCoverage;
        _s0.aggregateLimitUSD = limit;
        _s0.retentionUSD = retain;
        _s0.annualPremiumUSD = premium;
        _s0.premiumPaidUSD = FHE.asEuint64(0);
        _s0.claimsReserveUSD = FHE.div(limit, 10);
        _s0.sublimitRansomUSD = ransom;
        _s0.inceptionDate = block.timestamp;
        _s0.expiryDate = block.timestamp + durationDays * 1 days;
        _s0.active = true;
        _s0.underReview = false;
    ...
    function fileClaim(
        ...
        claimId = claimCount++;
        claims[claimId].policyId = policyId;
        claims[claimId].coverageType = cType;
        claims[claimId].claimedAmountUSD = claimed;
        claims[claimId].approvedAmountUSD = FHE.asEuint64(0);
        claims[claimId].forensicsCostUSD = forensics;
        claims[claimId].reputationDamageUSD = FHE.asEuint64(0);
        claims[claimId].incidentDescription = description;
        claims[claimId].incidentDate = incidentDate;
        claims[claimId].reportDate = block.timestamp;
        claims[claimId].forensicsComplete = false;
        claims[claimId].settled = false;
