...
contract PrivateCrossChainBridgeLiquidity is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function initiateBridge(
        ...
        txId = txCount++;
        transactions[txId].user = msg.sender;
        transactions[txId].sourceChain = srcChain;
        transactions[txId].destChain = dstChain;
        transactions[txId].amountUSD = amount;
        transactions[txId].feeUSD = fee;
        transactions[txId].outputAmountUSD = output;
        transactions[txId].relayer_ = relayer_;
        transactions[txId].initiatedAt = block.timestamp;
        transactions[txId].completed = false;
        transactions[txId].refunded = false;
