...
contract PrivateElectricGridBatteryStorageAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function submitStorageBid(
        ...
        id = bidCount++;
        bids[id].storageOperator = msg.sender;
        bids[id].technology = technology;
        bids[id].serviceType = serviceType;
        bids[id].capacityMWh = cap;
        bids[id].powerRatingMW = power;
        bids[id].roundTripEffBps = eff;
        bids[id].capacityPricePerMWhUSD = capPrice;
        bids[id].totalCapacityPaymentUSD = totalPay;
        bids[id].degradationRateBps = deg;
        bids[id].accepted = false;
        bids[id].submittedAt = block.timestamp;
