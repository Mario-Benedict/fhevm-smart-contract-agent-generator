...
contract PrivateDeFiYieldFarming is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createPool(
        ...
        id = poolCount++;
        pools[id].poolName = name;
        pools[id].lpTokenSymbol = lpSymbol;
        pools[id].tvlCap = tvlCap;
        pools[id].currentTVL = FHE.asEuint64(0);
        pools[id].aprBps = apr;
        pools[id].rewardPerBlock = rewardPB;
        pools[id].totalRewardsPaid = FHE.asEuint64(0);
        pools[id].whaleDepositLimitBps = whaleLimit;
        pools[id].startBlock = block.number;
        pools[id].endBlock = block.number + durationBlocks;
