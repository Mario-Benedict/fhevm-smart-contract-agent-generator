...
contract PrivateDebtCollectionAgency is ZamaEthereumConfig, Ownable {
    ...
    function purchaseDebtAccount(
        ...
        id = accountCount++;
        accounts[id].debtor = debtor;
        accounts[id].originalCreditor = originalCreditor;
        accounts[id].originalBalance = balance;
        accounts[id].currentBalance = balance;
        accounts[id].accruedInterest = FHE.asEuint64(0);
        accounts[id].commissionRateBps = commission;
        accounts[id].purchasedAt = block.timestamp;
        accounts[id].lastActivity = block.timestamp;
        accounts[id].status = DebtStatus.Active;
        accounts[id].collector = msg.sender;
