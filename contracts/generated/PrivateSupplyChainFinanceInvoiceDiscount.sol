...
contract PrivateSupplyChainFinanceInvoiceDiscount is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function submitInvoice(
        ...
        id = invoiceCount++;
        invoices[id] = SCFInvoice({
            supplier: msg.sender, anchorBuyer: anchorBuyer, financier: address(0),
            invoiceRef: invoiceRef, faceValueUSD: faceValue, discountRateBps: FHE.asEuint64(0),
            discountedValueUSD: FHE.asEuint64(0), financierReturnUSD: FHE.asEuint64(0),
            supplierCreditScore: creditScore, status: InvoiceStatus.Submitted,
            invoiceDate: block.timestamp, paymentDueDate: block.timestamp + dueDays * 1 days
        });
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalFinancedUSD, viewer);
