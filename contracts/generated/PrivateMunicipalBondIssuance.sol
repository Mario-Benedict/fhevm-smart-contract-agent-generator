...
contract PrivateMunicipalBondIssuance is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function issueBond(
        ...
        id = bondCount++;
        bonds[id] = MunicipalBond({
            issuer: msg.sender, bondType: bondType, rating: rating, bondRef: bondRef,
            projectDescription: projectDescription, principalAmountUSD: principal,
            couponRateBps: coupon, debtServiceCoverageX10: dscr, totalInterestPayable: totalInterest,
            reserveFundUSD: reserve, issuanceDate: block.timestamp,
            maturityDate: block.timestamp + maturityYears * 365 days, callable: callable
        });
    ...
    function allowDebtStats(address viewer) external onlyOwner {
        FHE.allow(_totalDebtOutstanding, viewer); FHE.allow(_totalInterestPaid, viewer);
    }
}
