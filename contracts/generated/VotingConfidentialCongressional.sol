...
contract VotingConfidentialCongressional is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerMember(
        ...
        FHE.allow(members[memberAddr].lobbyingScore, memberAddr);
    ...
    function introduceBill(
        ...
        id = billCount++;
        bills[id] = Bill({
            billNumber: billNumber, title: title,
            votesYea: FHE.asEuint64(0), votesNay: FHE.asEuint64(0),
            votesPresent: FHE.asEuint64(0), quorumRequired: quorum,
            passThreshold: threshold, votingDeadline: deadline,
            passed: false, vetoed: false, enacted: false
        });
    ...
    function castVote(uint256 billId, uint8 choice) external nonReentrant {
        ...
        if (choice == 0) {
            bill.votesYea = FHE.add(bill.votesYea, weight);
