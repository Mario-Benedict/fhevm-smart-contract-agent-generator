...
contract DeFiPrivateOptionsVault is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function writeOption(
        ...
        options[id] = Option({
            optionType: optType, writer: msg.sender, buyer: address(0),
            strikePrice: strike, premium: premium, collateral: collateral,
            expiry: block.timestamp + expiryDays * 1 days,
            exercised: false, expired: false
        });
    ...
    function exercise(uint256 id, externalEuint64 encCurrentPrice, bytes calldata proof) external nonReentrant {
        ...
        opt.exercised = true;
        _totalCollateralLocked = FHE.sub(_totalCollateralLocked, opt.collateral);
