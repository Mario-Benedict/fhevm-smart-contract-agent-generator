...
contract PrivateSportsTeamValuationBond is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueBond(
        ...
        id = bondCount++;
        bonds[id] = TeamBond({
            teamOwner: teamOwner, league: league, teamName: teamName, bondFaceValueUSD: faceValue,
            mediaRightsIncomeUSD: mediaRights, ticketingRevenueUSD: ticketing, valuationMultiple: multiple,
            baseCouponBps: baseCoupon, performanceBonusBps: FHE.asEuint16(0),
            outstandingNotionalUSD: faceValue, status: BondStatus.Issuance,
            issuanceDate: block.timestamp, maturityDate: block.timestamp + maturityDays * 1 days
        });
