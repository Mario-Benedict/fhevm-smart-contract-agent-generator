...
contract PrivateWineHarvestYieldAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerVintage(
        ...
        id = vintageCount++;
        vintages[id] = WineVintage({
            chateau: msg.sender, appellation: appellation, varietal: varietal,
            harvestYear: harvestYear, yieldKgPerHectare: yield, brixSugarContent: brix,
            pHLevel: pH, totalBottles: bottles, pricePerBottleCents: price,
            totalSaleRevenueCents: FHE.asEuint64(0), criticScoreBps: FHE.asEuint32(0),
            status: VintageStatus.Growing
        });
    ...
    function assignCriticScore(uint256 vintageId, externalEuint32 encScore, bytes calldata proof) external onlySommelier {
        ...
        FHE.allow(vintages[vintageId].criticScoreBps, vintages[vintageId].chateau);
    }
