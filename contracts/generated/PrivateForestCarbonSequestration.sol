...
contract PrivateForestCarbonSequestration is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerPlot(
        ...
        id = plotCount++;
        plots[id] = ForestPlot({
            landowner: msg.sender, forestType: fType, country: country, plotId: plotId,
            areaHectares: area, treeDensityPerHa: density,
            baselineCarbonTonnes: baseline, currentCarbonTonnes: baseline,
            additionalityTonnes: FHE.asEuint64(0), leakageRiskBps: leakage,
            registrationDate: block.timestamp, status: VerificationStatus.Pending
        });
