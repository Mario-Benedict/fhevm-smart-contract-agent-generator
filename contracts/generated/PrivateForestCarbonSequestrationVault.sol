...
contract PrivateForestCarbonSequestrationVault is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerPlot(
        ...
        pid = plotCount++;
        plots[pid] = ForestPlot({
            plotId: plotId, landowner: landowner,
            areaCubicMeters: area, carbonTonnesAnnual: carbon,
            creditPriceUSD: price, annualPaymentUSD: payment,
            totalPaymentsUSD: FHE.asEuint64(0),
            forestHealthScore: health, deforestationRiskScore: risk,
            verificationDate: block.timestamp, verified: false
        });
