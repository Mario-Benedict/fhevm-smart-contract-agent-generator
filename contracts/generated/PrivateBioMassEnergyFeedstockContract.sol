...
contract PrivateBioMassEnergyFeedstockContract is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createFeedstockContract(
        ...
        id = contractCount++;
        contracts_[id] = FeedstockContract({
            supplier: msg.sender, energyProducer: energyProducer, bioMassType: bioMassType,
            annualTonnesCommitted: tonnes, pricePerTonneUSD: price, minimumCalorificKJkg: calorific,
            qualityPenaltyBps: penaltyBps, totalContractValueUSD: totalValue,
            deliveredTonnes: FHE.asEuint64(0), penaltiesAssessedUSD: FHE.asEuint64(0),
            status: ContractStatus.Active, startDate: block.timestamp,
