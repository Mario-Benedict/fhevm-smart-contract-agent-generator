...
contract EncryptedTimelockGovernanceVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function addGuardian(address g, externalEuint64 encVetoWeight, bytes calldata proof) external onlyOwner {
        ...
        FHE.allowThis(guardians[g].vetoWeight); FHE.allow(guardians[g].vetoWeight, g);
    }
    ...
    function queueOperation(
        ...
        operations[opId] = TimelockOperation({
            target: target, data: data, description: description, valueAttached: value,
            approvalWeight: FHE.asEuint64(0), requiredWeight: required, guardianVetoes: FHE.asEuint8(0),
            eta: block.timestamp + delayDays * 1 days, proposedAt: block.timestamp,
