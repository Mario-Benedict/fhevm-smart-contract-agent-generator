...
contract ArtworkProvenance is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function listArtwork(
        ...
        id = artworkCount++;
        artworks[id] = Artwork({
            artist: msg.sender, title: title, medium: medium,
            ipfsImage: ipfsImage, reservePrice: reserve,
            highestBid: FHE.asEuint64(0), highestBidder: address(0),
            auctionEnd: block.timestamp + auctionDays * 1 days,
            sold: false, provenanceYear: provenanceYear
        });
    ...
    function finalizeAuction(uint256 artworkId) external nonReentrant {
        ...
        euint64 artistCut = FHE.sub(art.highestBid, commission);
