...
contract EncryptedAircraftLeaseFinancing is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerAircraft(
        ...
        aircraft[id] = Aircraft({
            tailNumber: tailNumber, aircraftType: aircraftType, manufacturer: manufacturer,
            manufactureYear: manufactureYear, currentMarketValueUSD: value,
            maintenanceReserveUSD: FHE.asEuint64(0), monthlyDepreciation: depreciation,
            status: AircraftStatus.Available, currentLessee: address(0), lastAppraisalDate: block.timestamp
        });
    ...
    function proposeLease(
        ...
        leases[leaseId] = LeaseAgreement({
            aircraftId: aircraftId, lessor: msg.sender, lessee: lessee,
            monthlyRentUSD: rent, securityDepositUSD: deposit,
            maintenanceReserveBps: reserveBps, totalRentPaidUSD: FHE.asEuint64(0),
            outstandingArrearsUSD: FHE.asEuint64(0),
            commencementDate: block.timestamp,
            termMonths: termMonths_,
            terminationDate: block.timestamp + termMonths_ * 30 days,
            status: LeaseStatus.Proposed, missedPayments: 0
        });
    ...
    function terminateLease(uint256 leaseId) external onlyLessor {
        ...
        _totalMonthlyRentRoll = FHE.sub(_totalMonthlyRentRoll, l.monthlyRentUSD);
        FHE.allowThis(_totalMonthlyRentRoll);
    ...
    function reappraise(
        ...
        _totalFleetValue = FHE.sub(_totalFleetValue, aircraft[aircraftId].currentMarketValueUSD);
