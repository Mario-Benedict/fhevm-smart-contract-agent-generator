...
contract EncryptedRockPaperScissorsGame is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createGame(
        ...
        id = gameCount++;
        games[id] = Game({
            player1: msg.sender, player2: address(0), move1: move, move2: FHE.asEuint8(0),
            wager: wager, state: GameState.WaitingPlayer2, winner: address(0),
            createdAt: block.timestamp, commitDeadline: block.timestamp + commitWindowHours * 1 hours
        });
        FHE.allowThis(games[id].move1);
        FHE.allowThis(games[id].move2);
        FHE.allowThis(games[id].wager); FHE.allow(games[id].wager, msg.sender);
