...
contract PrivateSovereignDebtRestructuring is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerBond(
        ...
        bondId = bondCount++;
        bonds[bondId] = SovereignBond({
            issuingCountry: country,
            isinCode: isin,
            bondClass: bClass,
            outstandingPrincipal: principal,
            couponRateBps: coupon,
            proposedHaircutBps: FHE.asEuint64(0),
            recoveryValueUSD: FHE.asEuint64(0),
    ...
    function createRestructuringProposal(
        ...
        proposals[propId] = RestructuringProposal({
            totalDebtOutstanding: totalDebt,
            proposedNewDebt: newDebt,
            gdpWarrantValue: gdpWar,
            cashPaymentUSD: cash,
            extensionYears: uint32(extensionYears),
            votingDeadline: block.timestamp + votingDuration,
            yesVoteWeight: 0,
            totalVoteWeight: 0,
            status: RestructuringStatus.VOTING
        });
