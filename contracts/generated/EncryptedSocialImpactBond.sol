...
contract EncryptedSocialImpactBond is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createProgram(
        ...
        id = programCount++;
        programs[id] = SIBProgram({
            programName: name, outcomeArea: area,
            commissioner: msg.sender, serviceProvider: serviceProvider,
            targetOutcomeCount: targetOutcomes, achievedOutcomeCount: FHE.asEuint64(0),
            maxPaymentUSD: maxPayment, paymentPerOutcome: payPerOutcome,
            totalInvestorCapital: FHE.asEuint64(0), returnMultiplierBps: multiplier,
            programStart: block.timestamp, programEnd: programEnd, active: true, settled: false
        });
    ...
    function invest(uint256 programId, externalEuint64 encCapital, bytes calldata proof) external nonReentrant {
        ...
        FHE.allowThis(inv.capitalInvested);
        FHE.allow(inv.capitalInvested, msg.sender);
        FHE.allowThis(inv.expectedReturn);
