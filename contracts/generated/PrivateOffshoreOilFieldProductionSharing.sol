...
contract PrivateOffshoreOilFieldProductionSharing is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerPSA(
        ...
        id = psaCount++;
        psas[id] = PSAContract({
            ioc: ioc, noc: noc, fieldType: fieldType, blockRef: blockRef, country: country,
            dailyProductionBOPD: FHE.asEuint64(0), cumulativeProductionBO: FHE.asEuint64(0),
            costOilRecoveredUSD: FHE.asEuint64(0), profitOilNOCShareBps: profNOC,
            profitOilIOCShareBps: profIOC, royaltyRateBps: royalty, totalRoyaltyPaidUSD: FHE.asEuint64(0),
            oilPricePerBBLUSD: FHE.asEuint64(0), status: PSAStatus.Exploration,
            signedAt: block.timestamp, expiryDate: block.timestamp + termDays * 1 days
        });
    ...
    function updateProduction(
        ...
        euint64 dailyRevenue = FHE.mul(dailyProd, oilPrice);
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalProductionRevUSD, viewer);
