...
contract PrivateTreasuryCashManagement is ZamaEthereumConfig, Ownable {
    ...
    function createAccount(
        ...
        id = accountCount++;
        accounts[id] = TreasuryAccount({
            accountName: name, currency: currency, balance: initBal,
            minimumBalance: minBal, maximumBalance: maxBal, dailyTransferLimit: dailyLimit,
            transferredToday: FHE.asEuint64(0), lastSweepDate: block.timestamp,
            active: true, accountManager: manager
        });
        _totalCashGroup = FHE.add(_totalCashGroup, initBal);
        FHE.allowThis(accounts[id].balance);
        FHE.allow(accounts[id].balance, manager);
