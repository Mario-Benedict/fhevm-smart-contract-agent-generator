...
contract PrivatePharmaSupplyChainAuthenticity is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createBatch(
        ...
        id = batchCount++;
        batches[id] = PharmaBatch({
            manufacturer: msg.sender, productCode: productCode, batchNumber: batchNumber,
            unitCount: units, batchValueUSD: batchVal, temperatureRecordBps: FHE.asEuint16(10000),
            authenticityScore: authenticity, serialRangeStart: serialStart,
            status: BatchStatus.InProduction, manufacturedAt: block.timestamp,
