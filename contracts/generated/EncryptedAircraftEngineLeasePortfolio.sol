...
contract EncryptedAircraftEngineLeasePortfolio is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function addEngine(
        ...
        engines[id] = EngineAsset({
            ...
            ageMonths: age,
            llpCycleCost: llp,
            nextShopVisit: block.timestamp + 365 days,
            status: LeaseStatus.AVAILABLE,
            airworthinessCurrent: true
        });
    ...
    function executeLease(
        ...
        leases[leaseId] = LeaseAgreement({
            engineId: engineId,
            lessee: lessee,
            monthlyLeaseRate: mRate,
            maintenanceReserveRate: mrRate,
            securityDeposit: dep,
            accruedMR: FHE.asEuint64(0),
            totalPaid: FHE.asEuint64(0),
            leaseTerm: term,
            monthsPaid: FHE.asEuint32(0),
            startDate: block.timestamp,
            endDate: block.timestamp + 365 days * 3,
    ...
    function reimburseMaintenance(uint256 leaseId, externalEuint64 encAmount, bytes calldata proof) external onlyOwner {
        ...
        leases[leaseId].accruedMR = FHE.sub(leases[leaseId].accruedMR, reimbAmt);
        _totalMRFundBalance = FHE.sub(_totalMRFundBalance, reimbAmt);
        FHE.allowThis(leases[leaseId].accruedMR);
        FHE.allowThis(_totalMRFundBalance);
    ...
    function updateEngineAppraisal(uint256 engineId, externalEuint64 encNewValue, bytes calldata proof) external {
        ...
        _portfolioNAV = FHE.sub(_portfolioNAV, engines[engineId].currentMarketValue);
