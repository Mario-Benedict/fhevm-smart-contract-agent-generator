...
contract EncryptedDecentralizedScienceFundingDAO is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function submitProposal(
        ...
        proposals[id] = ResearchProposal({
            principalInvestigator: msg.sender, domain: domain, proposalRef: proposalRef, abstractIPFSHash: abstractHash,
            requestedGrantUSD: grantRequest, approvedGrantUSD: FHE.asEuint64(0), disbursedGrantUSD: FHE.asEuint64(0),
            ipLicensingRevenue: FHE.asEuint64(0), peerReviewScore: FHE.asEuint16(0), impactScore: FHE.asEuint16(0),
            confidentialityLevel: confLevel, phase: ProposalPhase.Submitted, submittedAt: block.timestamp
        });
    ...
    function disburseGrant(uint256 proposalId) external onlyResearchCommittee nonReentrant {
        ...
        _daoTreasuryUSD = FHE.sub(_daoTreasuryUSD, p.approvedGrantUSD);
