...
contract PrivateCatastropheRiskModelingPool is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueCATBond(
        ...
        catBonds[bondId] = CATBond({
            peril: peril, trigger: trigger, status: BondStatus.ISSUANCE,
            principalUSD: principal, attachmentPoint: attachment,
            exhaustionPoint: exhaustion, couponRateBps: coupon,
            expectedLossBps: el, probOfAttachmentBps: poa,
            payoutToSponsors: FHE.asEuint64(0), returnToInvestors: principal,
            reserveEscrow: principal, issuanceDate: block.timestamp,
            riskPeriodEnd: riskPeriodEnd, maturityDate: maturityDate, triggered: false
        });
