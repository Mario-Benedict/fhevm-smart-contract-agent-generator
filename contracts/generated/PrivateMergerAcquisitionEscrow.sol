...
contract PrivateMergerAcquisitionEscrow is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createDeal(
        ...
        id = dealCount++;
        deals[id] = Deal({
            buyer: msg.sender, seller: seller, targetCompany: targetCompany,
            totalConsiderationUSD: consideration, escrowedAmountUSD: escrow,
            breakupFeeUSD: breakup, earnoutTotalUSD: earnout,
            earnoutReleasedUSD: FHE.asEuint64(0),
            closingDeadline: block.timestamp + closingDays * 1 days,
            status: DealStatus.InDueDiligence
        });
