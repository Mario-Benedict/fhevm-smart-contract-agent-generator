...
contract PrivateMeatPackingSupplyChain is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function recordLivestockPurchase(
        ...
        euint64 totalPurchase = FHE.mul(FHE.asEuint64(1), pricePerHead); // proxy single head
        id = purchaseCount++;
    ...
    function createProcessingBatch(
        ...
        batchId = batchCount++;
        batches[batchId] = ProcessingBatch({
            purchaseId: purchaseId, meatType: p.meatType, inputWeightKg: inputKg,
            outputWeightKg: outputKg, yieldEfficiencyBps: yieldEff,
            coldChainScoreBps: FHE.asEuint16(10000), contaminationRiskScore: contamRisk,
            saleValueUSD: saleValue, status: BatchStatus.Processing, processedAt: block.timestamp
        });
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalLivestockCostUSD, viewer);
