...
contract PrivateRealEstateConstructionEscrow is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createProject(
        ...
        projects[id] = ConstructionProject({
            developer: developer, generalContractor: generalContractor, lender: lender,
            projectRef: projectRef, totalBudgetUSD: budget, disbursedUSD: FHE.asEuint64(0),
            retainageHeldUSD: FHE.asEuint64(0), approvedContingencyUSD: contingency,
            completionBps: FHE.asEuint16(0), startDate: block.timestamp,
            expectedCompletionDate: block.timestamp + durationDays * 1 days
        });
    ...
    function allowEscrowView(address viewer) external onlyOwner {
        FHE.allow(_totalEscrowedUSD, viewer);
