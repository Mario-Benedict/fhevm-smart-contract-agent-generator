...
contract PrivateRenewableEnergyCertificateMarket is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueCertificate(
        ...
        certId = certCount++;
        certificates[certId] = RECIssuance({
            certId: certId,
            producer: msg.sender,
            source: source,
            grade: grade,
            ...
            country: country,
            mwhProduced: mwh,
            certificatesIssued: mwh, // 1 REC = 1 MWh
            pricePerRECUSD: price,
            carbonAvoidedTonnes: co2,
            generationYear: genYear,
            expiryDate: block.timestamp + 3 * 365 days,
            retired: false,
            listed: false
        });
        _totalCarbonAvoided = FHE.add(_totalCarbonAvoided, co2);
        FHE.allowThis(certificates[certId].mwhProduced);
        FHE.allow(certificates[certId].mwhProduced, msg.sender);
