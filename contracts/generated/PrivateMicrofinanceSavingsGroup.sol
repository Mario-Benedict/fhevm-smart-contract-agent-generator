...
contract PrivateMicrofinanceSavingsGroup is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createGroup(
        ...
        id = groupCount++;
        groups[id] = SavingsGroup({
            groupName: groupName, communityRef: communityRef, facilitator: msg.sender,
            totalSavingsUSD: FHE.asEuint64(0), loanFundUSD: FHE.asEuint64(0),
            interestAccruedUSD: FHE.asEuint64(0), activeMemberCount: FHE.asEuint32(0),
            cycleInterestRateBps: cycleRate, status: GroupStatus.Forming,
            cycleStart: block.timestamp, cycleEnd: block.timestamp + cycleDays * 1 days
        });
    ...
    function allowGroupStats(address viewer) external onlyOwner {
        FHE.allow(_totalCommunityWealthUSD, viewer);
    }
}
