...
contract EncryptedDebtRestructuringNegotiation is ZamaEthereumConfig, AccessControl, ReentrancyGuard {
    ...
    function proposePlan(
        ...
        planId = nextPlanId++;
        plans[planId] = RestructuringPlan({
            debtor: msg.sender,
            totalDebt: FHE.fromExternal(encTotalDebt, debtProof),
            proposedHaircutBps: FHE.fromExternal(encHaircut, haircutProof),
            newPaymentScheduleAmt: FHE.fromExternal(encNewPayment, paymentProof),
            proposalDate: block.timestamp,
            votingDeadline: block.timestamp + votingDays * 1 days,
            phase: RestructuringPhase.Proposal,
            creditorVotesFor: 0,
            creditorVotesAgainst: 0,
