...
contract SportsAthleteEndorsementBid is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function openDealForBids(address athlete, string calldata athleteName) external onlyAgent returns (uint256 id) {
        id = dealCount++;
        deals[id] = EndorsementDeal({
            athlete: athlete, athleteName: athleteName,
            annualBaseSalary: FHE.asEuint64(0), signingBonus: FHE.asEuint64(0),
            performanceBonus: FHE.asEuint64(0), contractYears: FHE.asEuint8(0), milestonesAchieved: 0,
            signedAt: 0, active: false, brand: address(0)
        });
