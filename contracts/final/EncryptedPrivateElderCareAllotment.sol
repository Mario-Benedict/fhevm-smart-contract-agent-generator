...
contract EncryptedPrivateElderCareAllotment is ZamaEthereumConfig, AccessControl, ReentrancyGuard {
    ...
    function requestAllotment(
        ...
        allotments[id].resident = msg.sender;
        allotments[id].careHome = careHome;
        allotments[id].monthlyFee = FHE.asEuint64(0);
        allotments[id].subsidy = FHE.asEuint64(0);
        allotments[id].copayment = FHE.asEuint64(0);
        allotments[id].assetsMeansTest = assets;
        allotments[id].needScore = FHE.asEuint32(0);
        allotments[id].assessmentDate = block.timestamp;
        allotments[id].admissionDate = 0;
        allotments[id].status = AllotmentStatus.Pending;
