...
contract EncryptedNuclearWasteTracking is ZamaEthereumConfig, AccessControl, ReentrancyGuard {
    ...
    function registerCanister(
        ...
        id = canisterCount++;
        WasteCanister storage _s0 = canisters[id];
        _s0.manifestId = manifestId;
        _s0.wasteType = wasteType;
        _s0.radioactivityBq = activity;
        _s0.halfLifeYears = halfLife;
        _s0.temperatureCelsius = FHE.asEuint64(20);
        _s0.siteLatEncoded = lat;
        _s0.siteLonEncoded = lon;
        _s0.massKg = mass;
        _s0.packagingDate = block.timestamp;
        _s0.nextInspectionDue = nextInspection;
        _s0.transportAuthorized = false;
        _s0.disposed = false;
        _s0.currentCustodian = msg.sender;
    ...
    function requestTransport(
        ...
        transports[transportId].canisterId = canisterId;
        transports[transportId].transporter = msg.sender;
        transports[transportId].destinationOperator = destinationOperator;
        transports[transportId].distanceKm = distance;
        transports[transportId].riskScore = risk;
        transports[transportId].dispatchTime = block.timestamp;
        transports[transportId].estimatedArrival = estimatedArrival;
        transports[transportId].completed = false;
        transports[transportId].regulatorApproved = false;
        transports[transportId].siteApproved = false;
