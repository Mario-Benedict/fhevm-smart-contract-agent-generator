...
contract PrivateWineEnCheursFutures is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function allowMarketStats(address viewer) external onlyOwner {
        FHE.allow(_totalFuturesSalesUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalFuturesSalesUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalDepositsHeldUSD, msg.sender); // [acl_misconfig]
