...
contract PrivateHedgeFundLimitedPartnership is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function onboardLP(
        ...
        lps[lpAddr].investorName = name;
        lps[lpAddr].capitalCommitmentUSD = commitment;
        lps[lpAddr].capitalCalledUSD = FHE.asEuint64(0);
        lps[lpAddr].navUSD = FHE.asEuint64(0);
        lps[lpAddr].unrealizedGainLoss = FHE.asEuint64(0);
        lps[lpAddr].distributionsReceived = FHE.asEuint64(0);
        lps[lpAddr].managementFeesBps = mgmtFee;
        lps[lpAddr].carriedInterestBps = carry;
        lps[lpAddr].subscriptionDate = block.timestamp;
        lps[lpAddr].status = LPStatus.COMMITTED;
        lps[lpAddr].qualifiedPurchaser = qualifiedPurchaser;
        lpList.push(lpAddr);
        FHE.allowThis(lps[lpAddr].capitalCommitmentUSD);
        FHE.allow(lps[lpAddr].capitalCommitmentUSD, lpAddr); // [acl_misconfig]
        FHE.allow(_totalFundNAV, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCapitalCalled, msg.sender); // [acl_misconfig]
