...
contract EncryptedCryptocurrencyOTCDealerBook is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issueQuote(
        ...
        if (direction == TradeDirection.BUY) {
            clientRate = FHE.add(FHE.add(midPrice, halfSpread), markup);
        } else {
            // Safe: verify midPrice can cover spread + markup before subtracting
            euint64 sellDeduction = FHE.add(halfSpread, markup);
            ebool _sellSafe = FHE.ge(midPrice, sellDeduction);
            clientRate = FHE.select(_sellSafe, FHE.sub(midPrice, sellDeduction), FHE.asEuint64(0));
        }

        quoteId = keccak256(abi.encodePacked(client, asset, block.timestamp));
        ClientQuote storage _s0 = quotes[quoteId];
        _s0.client = client;
        _s0.direction = direction;
        _s0.asset = asset;
        _s0.notionalAmount = notional;
        _s0.dealerMidPrice = midPrice;
        _s0.spread = spread;
        _s0.clientRate = clientRate;
        _s0.dealerMarkup = markup;
        _s0.clientCreditUsed = notional;
        _s0.filled = false;
        _s0.quoteTime = block.timestamp;
        _s0.expiryTime = block.timestamp + validForSeconds;
