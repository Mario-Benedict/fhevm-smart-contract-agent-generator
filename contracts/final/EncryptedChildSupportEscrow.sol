...
contract EncryptedChildSupportEscrow is ZamaEthereumConfig, AccessControl, Pausable, ReentrancyGuard {
    ...
    function createOrder(
        ...
        orders[orderId].payer = payer;
        orders[orderId].payee = payee;
        orders[orderId].monthlyAmount = monthly;
        orders[orderId].totalPaid = FHE.asEuint64(0);
        orders[orderId].arrears = FHE.asEuint64(0);
        orders[orderId].orderDate = block.timestamp;
        orders[orderId].durationMonths = durationMonths;
        orders[orderId].monthsPaid = 0;
