...
contract PrivateTelematicsBasedInsurance is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issuePolicy(
        ...
        id = policyCount++;
        policies[id].policyholder = holder;
        policies[id].vehicleVIN = vin;
        policies[id].baseAnnualPremium = base;
        policies[id].behaviorDiscount = FHE.asEuint64(0);
        policies[id].currentPremium = base;
        policies[id].premiumPaid = FHE.asEuint64(0);
        policies[id].claimsHistory = FHE.asEuint64(0);
        policies[id].policyStart = block.timestamp;
        policies[id].policyEnd = block.timestamp + duration;
        ...
        FHE.allow(policies[id].currentPremium, holder); // [acl_misconfig]
        FHE.allow(_totalPremiumPool, msg.sender); // [acl_misconfig]
