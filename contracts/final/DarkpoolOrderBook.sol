...
contract DarkpoolOrderBook is ZamaEthereumConfig, Ownable {
    ...
    function matchOrders(uint256 buyId, uint256 sellId) public onlyOwner {
        ...
        // Deduct from orders
        ebool _safeSub93 = FHE.ge(orders[buyId].amount, actualSwapAmount);
        orders[buyId].amount = FHE.select(_safeSub93, FHE.sub(orders[buyId].amount, actualSwapAmount), FHE.asEuint64(0));
        ebool _safeSub94 = FHE.ge(orders[sellId].amount, actualSwapAmount);
        orders[sellId].amount = FHE.select(_safeSub94, FHE.sub(orders[sellId].amount, actualSwapAmount), FHE.asEuint64(0));
