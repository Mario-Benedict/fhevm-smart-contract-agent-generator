...
contract PrivateMergerAcquisitionEscrow is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createDeal(
        ...
        euint64 considerationWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 considerationExposure = FHE.sub(considerationWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        FHE.allow(deals[id].totalConsiderationUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalDealValueUSD, msg.sender); // [acl_misconfig]
