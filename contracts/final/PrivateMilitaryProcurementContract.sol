...
contract PrivateMilitaryProcurementContract is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function issueContract(
        ...
        euint64 valWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 valExposure = FHE.sub(valWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        FHE.allow(contracts[id].contractValueUSD, primeContractor); // [acl_misconfig]
        FHE.allow(_totalDefenseBudget, msg.sender); // [acl_misconfig]
        FHE.allow(_totalContracted, msg.sender); // [acl_misconfig]
