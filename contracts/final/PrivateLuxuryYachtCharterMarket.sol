...
contract PrivateLuxuryYachtCharterMarket is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function listYacht(
        ...
        id = yachtCount++;
        yachts[id].owner = msg.sender;
        yachts[id].yachtName = name;
        yachts[id].flag = flag;
        yachts[id].yachtClass = yClass;
        yachts[id].loa_dm = loa;
        yachts[id].maxGuests = guests;
        yachts[id].weeklyCharterRateUSD = rate;
        yachts[id].apaPercentageBps = apa;
        yachts[id].ownerNetUSD = FHE.asEuint64(0);
        yachts[id].crewCount = crew;
        yachts[id].available = true;
    ...
    function bookCharter(
        ...
        euint32 _weeks = FHE.fromExternal(encWeeks, wProof);
        euint64 total = FHE.add(FHE.add(agreedRate, apaAllowance), commission); // [arithmetic_overflow_underflow]
        euint64 commissionScaled = FHE.mul(commission, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        id = bookingCount++;
        bookings[id].yachtId = yachtId;
        bookings[id].charterer = msg.sender;
        bookings[id].agreedRateUSD = agreedRate;
        bookings[id].apaAllowanceUSD = apaAllowance;
        bookings[id].brokerCommissionUSD = commission;
        bookings[id].charterWeeks = _weeks;
        bookings[id].totalCharterCostUSD = total;
        bookings[id].embarked = embarkDate;
        bookings[id].disembarked = 0;
        bookings[id].status = CharterStatus.Confirmed;
        y.available = false;
    ...
    function allowMarketStats(address viewer) external onlyOwner {
        FHE.allow(_totalCharterRevenue, viewer); // [acl_misconfig]
        FHE.allow(_totalCharterRevenue, msg.sender); // [acl_misconfig]
        FHE.allow(_totalBrokerCommissions, msg.sender); // [acl_misconfig]
