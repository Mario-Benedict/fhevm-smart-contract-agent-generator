...
contract PrivateDeepSeaMiningRoyalty is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function grantConcession(
        ...
        id = concessionCount++;
        concessions[id].operator = operator;
        concessions[id].blockId = blockId;
        concessions[id].oceanRegion = oceanRegion;
        concessions[id].primaryMineral = mineral;
        concessions[id].licenseDepthMeters = depth;
        concessions[id].annualProductionTonnes = quota;
        concessions[id].royaltyRateBps = royaltyRate;
        concessions[id].totalRoyaltiesPaidUSD = FHE.asEuint64(0);
        concessions[id].licenseExpiry = block.timestamp + licenseYears * 365 days;
        concessions[id].status = ConcessionStatus.Active;
