...
contract EncryptedHospitalBillingAdjudicator is ZamaEthereumConfig, AccessControl, ReentrancyGuard {
    ...
    function submitClaim(
        ...
        claims[claimId].patient = patient;
        claims[claimId].hospital = msg.sender;
        claims[claimId].insurer = insurer;
        claims[claimId].billedAmount = billed;
        claims[claimId].negotiatedRate = FHE.asEuint64(0);
        claims[claimId].patientObligation = FHE.asEuint64(0);
        claims[claimId].insurerObligation = FHE.asEuint64(0);
        claims[claimId].serviceDate = serviceDate;
        claims[claimId].status = ClaimStatus.Submitted;
        claims[claimId].diagnosis = diagnosis;
    ...
    function adjudicateClaim(
        ...
        FHE.allow(c.negotiatedRate, c.hospital); // [acl_misconfig]
        ...
        FHE.allow(c.insurerObligation, c.patient); // [acl_misconfig]
        FHE.allow(c.billedAmount, c.patient); // [acl_misconfig]
