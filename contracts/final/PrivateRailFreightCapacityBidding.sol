...
contract PrivateRailFreightCapacityBidding is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createPathSlot(
        ...
        id = slotCount++;
        pathSlots[id].pathCode = pathCode;
        pathSlots[id].originTerminal = origin;
        pathSlots[id].destinationTerminal = destination;
        pathSlots[id].scheduledDeparture = scheduledDeparture;
        pathSlots[id].capacityTonnes = cap;
        pathSlots[id].reservePricePerTonneKm = reserveRate;
        pathSlots[id].awardedPriceUSD = FHE.asEuint64(0);
        pathSlots[id].awardedBidder = address(0);
        pathSlots[id].status = SlotStatus.Available;
    ...
    function allowRevenueView(address viewer) external onlyOwner {
        FHE.allow(_totalAwardedRevenueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalAwardedRevenueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCapacityOfferedTonnes, msg.sender); // [acl_misconfig]
