...
contract CorporateVestingVault is ZamaEthereumConfig, Ownable {
    ...
    function claimVestedTokens(externalEuint64 extRequest, bytes calldata proof) external {
        ...
        ebool _safeSub83 = FHE.ge(totalVested, schedule.encryptedClaimedAmount);
        euint64 claimable = FHE.select(_safeSub83, FHE.sub(totalVested, schedule.encryptedClaimedAmount), FHE.asEuint64(0));
        ...
        ebool _safeSub84 = FHE.ge(totalEncryptedLiabilities, requestAmount);
        totalEncryptedLiabilities = FHE.select(_safeSub84, FHE.sub(totalEncryptedLiabilities, requestAmount), FHE.asEuint64(0));
        FHE.allowThis(totalEncryptedLiabilities);
