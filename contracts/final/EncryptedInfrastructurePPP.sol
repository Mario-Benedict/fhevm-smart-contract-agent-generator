...
contract EncryptedInfrastructurePPP is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function recordRevenue(
        ...
        FHE.allowThis(_totalInfraRevenue);
        // Expose portfolio-level aggregates so concessionaire can benchmark their project
        FHE.allow(_totalCapexPortfolio, msg.sender); // [acl_misconfig]
        FHE.allow(_totalVGFDisbursed, msg.sender); // [acl_misconfig]
        FHE.allow(_totalInfraRevenue, msg.sender); // [acl_misconfig]
