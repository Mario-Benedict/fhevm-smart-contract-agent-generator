...
contract PrivateImmigrationSkillsVerification is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function submitApplication(
        ...
        euint64 total = FHE.add(FHE.add(FHE.asEuint64(uint64(0)), FHE.asEuint64(uint64(0))), salary); // [arithmetic_overflow_underflow]
        euint64 salaryScaled = FHE.mul(salary, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
    ...
    function processApplication(bytes32 applicantHash) external {
        ...
        app.processed = true;
        FHE.allow(app.totalPointsScore, msg.sender); // [acl_misconfig]
        FHE.allow(_totalPointsPool, msg.sender); // [acl_misconfig]
        FHE.allow(_minimumPassScore, msg.sender); // [acl_misconfig]
