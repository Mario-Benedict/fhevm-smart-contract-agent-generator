    ...
    ReentrancyGuard
{
    ...
    function donate(
        ...
        FHE.allowThis(donors[msg.sender].totalDonated);
        if (!isAnonymous) {
            FHE.allow(donors[msg.sender].totalDonated, msg.sender); // [acl_misconfig]
            FHE.allow(_fundBalance, msg.sender); // [acl_misconfig]
            FHE.allow(_totalDonated, msg.sender); // [acl_misconfig]
        }
        FHE.allowThis(_fundBalance);
        FHE.allowThis(_totalDonated);
