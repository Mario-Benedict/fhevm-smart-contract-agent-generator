...
contract EncryptedCarbonCreditForwardContract is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function executeForward(
        ...
        ebool _safeMul39 = FHE.le(tonnes, FHE.asEuint64(type(uint32).max));
    ...
    function setForwardQuality(
        ...
        FHE.allowThis(f.qualityScoreBps); FHE.allowThis(f.additionalityBps);
        FHE.allowThis(f.permanenceBps); FHE.allowThis(f.collateralPostedUSD);
    }
    ...
    function submitVerification(
    ...
    ) external onlyVerifier returns (uint256 vIdx) {
        CarbonForward storage f = forwards[forwardId];
        ...
        vIdx = verifications[forwardId].length;
        VerificationStatement storage vs = verifications[forwardId].push();
        vs.forwardId = forwardId;
        vs.verifiedTonnesCO2 = verifiedTonnes;
        vs.verificationScoreBps = verifScore;
        vs.accepted = false;
        vs.verifiedAt = block.timestamp;
    ...
    function setVerificationId(uint256 forwardId, uint256 vIdx, string calldata verifierId) external onlyVerifier {
        verifications[forwardId][vIdx].verifierId = verifierId;
    }
