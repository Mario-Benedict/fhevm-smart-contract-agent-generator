...
contract PrivateTelecommunicationsInfrastructureSharing is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerTower(
        ...
        id = towerCount++;
        towers[id].towerOwner = msg.sender;
        towers[id].towerId = towerId;
        towers[id].location = location;
        towers[id].towerType = towerType;
        towers[id].maxTenants = maxT;
        towers[id].currentTenants = FHE.asEuint8(0);
        towers[id].groundRentUSD = groundRent;
        towers[id].totalTenancyRevenueUSD = FHE.asEuint64(0);
        towers[id].operational = true;
    ...
    function allowPortfolioView(address viewer) external onlyOwner {
        FHE.allow(_totalPortfolioRevenueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalPortfolioRevenueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalGroundRentPaidUSD, msg.sender); // [acl_misconfig]
