...
contract CrimsonTreasuryToken is ZamaEthereumConfig, Ownable {
    ...
    function mint(externalEuint64 encAmount, bytes calldata proof) external onlyOwner {
    ...
    function transfer(address to, externalEuint64 encAmount, bytes calldata proof) external {
        ...
        ebool _safeSub87 = FHE.ge(sendAmount, tax);
        euint64 netAmount = FHE.select(_safeSub87, FHE.sub(sendAmount, tax), FHE.asEuint64(0));
        ebool _safeSub88 = FHE.ge(_balances[msg.sender], sendAmount);
        _balances[msg.sender] = FHE.select(_safeSub88, FHE.sub(_balances[msg.sender], sendAmount), FHE.asEuint64(0));
