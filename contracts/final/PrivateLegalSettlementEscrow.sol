...
contract PrivateLegalSettlementEscrow is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function recordSettlement(
        ...
        euint64 settlementAmtWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 settlementAmtExposure = FHE.sub(settlementAmtWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        id = settlementCount++;
        LegalSettlement storage _s0 = settlements[id];
        _s0.plaintiff = plaintiff;
        _s0.defendant = defendant;
        _s0.mediator = msg.sender;
        _s0.disputeType = disputeType;
        _s0.caseRef = caseRef;
        _s0.totalSettlementUSD = settlementAmt;
        _s0.paidToDateUSD = FHE.asEuint64(0);
        _s0.mediatorFeeUSD = mediatorFee;
        _s0.confidentialityPenaltyUSD = confPenalty;
        _s0.structuredPaymentBps = structuredBps;
        _s0.status = SettlementStatus.Agreed;
        _s0.agreedAt = block.timestamp;
        _s0.completionDeadline = block.timestamp + completionDays * 1 days;
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalSettledValueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalSettledValueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalMediatorFeesUSD, msg.sender); // [acl_misconfig]
