...
contract PrivateDiamondGradingRegistry is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerDiamond(
        ...
        id = diamondCount++;
        diamonds[id].certificateId = certId;
        diamonds[id].origin = origin;
        diamonds[id].caratWeight = carat;
        diamonds[id].cutGrade = cut;
        diamonds[id].colorGrade = color;
        diamonds[id].clarityGrade = clarity;
        diamonds[id].appraisalValueUSD = appraisal;
        diamonds[id].askingPriceUSD = FHE.asEuint64(0);
        diamonds[id].currentOwner = initialOwner;
        diamonds[id].status = TradingStatus.Graded;
        diamonds[id].gradedAt = block.timestamp;
