...
contract CybersecurityPrivateBugBounty is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function payBounty(uint256 programId, uint256 reportIdx) external onlyOwner nonReentrant {
        ...
        ebool _safeSub89 = FHE.ge(r.bountyAmount, fee);
        euint64 net = FHE.select(_safeSub89, FHE.sub(r.bountyAmount, fee), FHE.asEuint64(0));
        ...
        ebool _safeSub90 = FHE.ge(prog.totalPool, r.bountyAmount);
        prog.totalPool = FHE.select(_safeSub90, FHE.sub(prog.totalPool, r.bountyAmount), FHE.asEuint64(0));
