...
contract UnionStrikeVote is ZamaEthereumConfig, Ownable {
    ...
    function createBallot(string calldata issue, uint256 duration) external onlyOwner returns (uint256 ballotId) {
        ...
        FHE.allow(b.yesVotes, msg.sender); // [acl_misconfig]
    ...
    function castVote(uint256 ballotId, externalEuint32 encChoice, bytes calldata inputProof) external {
        ...
        b.yesVotes = FHE.add(b.yesVotes, FHE.select(isYes, FHE.asEuint32(1), FHE.asEuint32(0))); // [arithmetic_overflow_underflow]
