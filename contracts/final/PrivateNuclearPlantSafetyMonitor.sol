...
contract PrivateNuclearPlantSafetyMonitor is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerUnit(string calldata unitId) external returns (uint256 id) {
        ...
        id = unitCount++;
        units[id].unitId = unitId;
        units[id].coreTemperatureK = FHE.asEuint64(5730);
        units[id].neutronFluxSv = FHE.asEuint64(0);
        units[id].coolantPressurePa = FHE.asEuint64(155000);
        units[id].steamFlowKgs = FHE.asEuint64(0);
        units[id].radiationDoseMuSvH = FHE.asEuint64(0);
        units[id].safetyMarginBps = FHE.asEuint64(9000);
        units[id].powerOutputMW = FHE.asEuint64(0);
        units[id].inScram = false;
        units[id].operational = true;
        units[id].lastReadingTime = block.timestamp;
    ...
    function reportIncident(
        ...
        euint64 severityWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 severityExposure = FHE.sub(severityWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
    ...
    function grantRegulatorAccess(uint256 unitId, address regulator) external {
        ...
        FHE.allow(units[unitId].coreTemperatureK, regulator); // [acl_misconfig]
        FHE.allow(_totalRadiationReleased, msg.sender); // [acl_misconfig]
