...
contract PrivateHighFrequencyTradingCollateral is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function openAccount(
        ...
        id = accountCount++;
        accounts[id].firm = firm;
        accounts[id].firmId = firmId;
        accounts[id].netAssetValueUSD = nav;
        accounts[id].initialMarginUSD = initMargin;
        accounts[id].maintenanceMarginUSD = maintMargin;
        accounts[id].postedCollateralUSD = FHE.asEuint64(0);
        accounts[id].portfolioVaRBps = var_;
        accounts[id].leverageRatioBps = lev;
        accounts[id].status = MarginStatus.Compliant;
        ...
        FHE.allow(accounts[id].netAssetValueUSD, firm); // [acl_misconfig]
        FHE.allow(_totalCollateralUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalMarginRequiredUSD, msg.sender); // [acl_misconfig]
    ...
    function postCollateral(
        ...
        euint64 haircutAmt = FHE.mul(notional, FHE.asEuint64(0)); // [arithmetic_overflow_underflow]
        euint64 notionalScaled = FHE.mul(notional, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
