...
contract PrivateLuxuryRealEstateTokenization is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function purchaseFractions(
        ...
        FHE.allowThis(h.fractionCount);
        FHE.allow(h.fractionCount, msg.sender); // [acl_misconfig]
        FHE.allow(_totalPortfolioValue, msg.sender); // [acl_misconfig]
        FHE.allow(_totalRentalDistributed, msg.sender); // [acl_misconfig]
    ...
    function distributeRental(uint8 propertyId, address holder, uint64 totalFractionsPlaintext) external onlyOwner nonReentrant {
        ...
        euint64 netShare = FHE.sub(grossShare, mgmtFee); // [arithmetic_overflow_underflow]
        euint64 mgmtFeeScaled = FHE.mul(mgmtFee, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
