...
contract PrivateNuclearFuelCycleTrading is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createFuelContract(
        ...
        euint64 total = FHE.mul(qty, price); // [arithmetic_overflow_underflow]
        euint64 priceScaled = FHE.mul(price, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        id = contractCount++;
        contracts_[id].supplier = msg.sender;
        contracts_[id].utility = utility;
        contracts_[id].component = component;
        contracts_[id].safeguardsRef = safeguardsRef;
        contracts_[id].quantityKgU = qty;
        contracts_[id].pricePerKgUSD = price;
        contracts_[id].totalContractUSD = total;
        contracts_[id].iaaeComplianceScore = FHE.asEuint16(0);
        contracts_[id].proliferationRiskScore = FHE.asEuint8(0);
        contracts_[id].deliveryDate = block.timestamp + deliveryDays * 1 days;
        contracts_[id].settled = false;
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalVolumeKgU, viewer); // [acl_misconfig]
        FHE.allow(_totalVolumeKgU, msg.sender); // [acl_misconfig]
        FHE.allow(_totalContractValueUSD, msg.sender); // [acl_misconfig]
