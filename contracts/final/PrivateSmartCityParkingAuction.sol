...
contract PrivateSmartCityParkingAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createParkingSlot(
        ...
        id = slotCount++;
        slots[id].slotId = slotId;
        slots[id].zone = zone;
        slots[id].baseHourlyRateCents = baseRate;
        slots[id].demandMultiplierBps = FHE.asEuint32(10000);
        slots[id].currentHourlyRateCents = baseRate;
        slots[id].totalRevenueUSD = FHE.asEuint64(0);
        slots[id].operatorShareBps = operatorShare;
        slots[id].status = SlotStatus.Available;
        slots[id].evCharging = evCharging;
    ...
    function allowRevenueView(address viewer) external onlyOwner {
        FHE.allow(_totalSystemRevenueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalSystemRevenueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCityShareUSD, msg.sender); // [acl_misconfig]
