...
contract VotingFutarchyMarket_c2_029 is ZamaEthereumConfig, Ownable {
    ...
    function mintTokens(address to, externalEuint64 encAmount, bytes calldata proof) external onlyOwner {
        ...
        FHE.allow(_tokenBalance[to], to); // [acl_misconfig]
        FHE.allow(_tokenBalance[to], msg.sender); // [acl_misconfig]
    }
    ...
    function claimWinnings(uint256 policyId) external {
        ...
        euint64 totalPool = FHE.add(p.betPool, p.againstPool); // [arithmetic_overflow_underflow]
        euint64 myBetScaled = FHE.mul(myBet, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
