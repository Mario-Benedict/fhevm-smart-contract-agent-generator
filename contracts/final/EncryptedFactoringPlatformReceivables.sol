...
contract EncryptedFactoringPlatformReceivables is ZamaEthereumConfig, AccessControl, ReentrancyGuard {
    ...
    function submitInvoice(
        ...
        ebool _safeSub224 = FHE.ge(faceValue, advanceAmount);
        euint64 reserveAmount = FHE.select(_safeSub224, FHE.sub(faceValue, advanceAmount), FHE.asEuint64(0));
    ...
    function fundInvoice(bytes32 invoiceId) external onlyRole(FACTOR_ROLE) nonReentrant {
        ...
        ebool _safeSub225 = FHE.ge(inv.advanceAmount, inv.discountFee);
        euint64 netAdvance = FHE.select(_safeSub225, FHE.sub(inv.advanceAmount, inv.discountFee), FHE.asEuint64(0));
    ...
    function markCollected(bytes32 invoiceId) external onlyRole(FACTOR_ROLE) {
        ...
        OriginatorProfile storage profile = originators[inv.originator];
        ebool _safeSub226 = FHE.ge(profile.usedFacility, inv.advanceAmount);
        profile.usedFacility = FHE.select(_safeSub226, FHE.sub(profile.usedFacility, inv.advanceAmount), FHE.asEuint64(0));
        ebool _safeSub227 = FHE.ge(profile.reserveBalance, inv.reserveAmount);
        profile.reserveBalance = FHE.select(_safeSub227, FHE.sub(profile.reserveBalance, inv.reserveAmount), FHE.asEuint64(0));
        ebool _safeSub228 = FHE.ge(_totalPortfolioOutstanding, inv.faceValue);
        _totalPortfolioOutstanding = FHE.select(_safeSub228, FHE.sub(_totalPortfolioOutstanding, inv.faceValue), FHE.asEuint64(0));
        ebool _safeSub229 = FHE.ge(_totalReserves, inv.reserveAmount);
        _totalReserves = FHE.select(_safeSub229, FHE.sub(_totalReserves, inv.reserveAmount), FHE.asEuint64(0));
