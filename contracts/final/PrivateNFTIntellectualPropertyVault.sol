...
contract PrivateNFTIntellectualPropertyVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerIPAsset(
        ...
        euint64 baseRoyaltyWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 baseRoyaltyExposure = FHE.sub(baseRoyaltyWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
    ...
    function allowIPView(uint256 assetId, address viewer) external {
        IPAsset storage asset = ipAssets[assetId];
        require(msg.sender == asset.primaryOwner || isIPRegistrar[msg.sender], "Not authorized");
        FHE.allow(asset.baseRoyaltyRateBps, viewer); // [acl_misconfig]
        FHE.allow(_totalSystemRoyaltiesUSD, msg.sender); // [acl_misconfig]
