...
contract PrivateNeonatalIntensiveCareAllocation is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function admitPatient(
        ...
        euint64 costWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 costExposure = FHE.sub(costWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
    ...
    function allowClinicalView(uint256 admId, address clinician) external onlyOwner {
        FHE.allow(admissions[admId].acuityScore, clinician); // [acl_misconfig]
        FHE.allow(_totalRevenueCycle, msg.sender); // [acl_misconfig]
        FHE.allow(_averageLengthOfStay, msg.sender); // [acl_misconfig]
