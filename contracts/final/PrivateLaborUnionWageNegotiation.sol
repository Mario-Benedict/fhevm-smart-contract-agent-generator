...
contract PrivateLaborUnionWageNegotiation is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function startNegotiation(
        ...
        euint64 unionDemandWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 unionDemandExposure = FHE.sub(unionDemandWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        id = negotiationCount++;
        CBANegotiation storage _s0 = negotiations[id];
        _s0.union = msg.sender;
        _s0.employer = employer;
        _s0.mediator = mediator;
        _s0.contractType = contractType;
        _s0.industryCode = industryCode;
        _s0.unionWageDemandBps = unionDemand;
        _s0.employerCounterBps = FHE.asEuint64(0);
        _s0.agreedWageIncreaseBps = FHE.asEuint64(0);
        _s0.workerCount = workerCount;
        _s0.economicImpactUSD = FHE.asEuint64(0);
        _s0.productivityIndexBps = productivity;
        _s0.cpiInflationBps = cpi;
        _s0.stage = NegotiationStage.Tabling;
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalWorkersNegotiating, viewer); // [acl_misconfig]
        FHE.allow(_totalWorkersNegotiating, msg.sender); // [acl_misconfig]
        FHE.allow(_totalAgreedWageImpactUSD, msg.sender); // [acl_misconfig]
