...
contract DeFiConfidentialStablecoinPeg is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function redeemAndBurn(externalEuint64 encAmount, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub104 = FHE.ge(_balances[msg.sender], actual);
        _balances[msg.sender] = FHE.select(_safeSub104, FHE.sub(_balances[msg.sender], actual), FHE.asEuint64(0));
        ebool _safeSub105 = FHE.ge(_totalSupply, actual);
        _totalSupply = FHE.select(_safeSub105, FHE.sub(_totalSupply, actual), FHE.asEuint64(0));
        ebool _safeSub106 = FHE.ge(_totalCollateral, collateralReturn);
        _totalCollateral = FHE.select(_safeSub106, FHE.sub(_totalCollateral, collateralReturn), FHE.asEuint64(0));
        FHE.allowThis(_balances[msg.sender]);
        FHE.allow(_balances[msg.sender], msg.sender);
    ...
    function rebase(bool isExpansion, externalEuint64 encDeviation, bytes calldata proof) external onlyOwner {
        ...
        if (isExpansion) {
            ...
            ebool _safeSub107 = FHE.ge(_totalSupply, burnAmount);
            _totalSupply = FHE.select(_safeSub107, FHE.sub(_totalSupply, burnAmount), FHE.asEuint64(0));
        }
        FHE.allowThis(_totalSupply);
    ...
    function transfer(address to, externalEuint64 encAmount, bytes calldata proof) external {
        ...
        ebool _safeSub108 = FHE.ge(_balances[msg.sender], actual);
        _balances[msg.sender] = FHE.select(_safeSub108, FHE.sub(_balances[msg.sender], actual), FHE.asEuint64(0));
