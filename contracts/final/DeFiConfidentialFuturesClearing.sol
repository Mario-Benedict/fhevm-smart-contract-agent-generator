...
contract DeFiConfidentialFuturesClearing is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function checkMarginCall(address trader) external returns (bool) {
        ...
        ebool _safeMul20 = FHE.le(FHE.add(p.margin, p.unrealizedPnL), FHE.asEuint64(type(uint64).max / 10000));
        euint64 effectiveMarginScaled = FHE.mul(FHE.add(p.margin, p.unrealizedPnL), FHE.asEuint64(10000));
        ebool _safeMul21 = FHE.le(p.notionalValue, FHE.asEuint64(type(uint32).max));
    ...
    function liquidate(address trader) external onlyOwner nonReentrant {
        ...
        p.liquidated = true;
        p.open = false;
        ebool _safeSub95 = FHE.ge(_totalOpenInterest, p.notionalValue);
        _totalOpenInterest = FHE.select(_safeSub95, FHE.sub(_totalOpenInterest, p.notionalValue), FHE.asEuint64(0));
    ...
    function closePosition() external nonReentrant {
        ...
        require(p.open && !p.liquidated, "Not active");
        p.open = false;
        ebool _safeSub96 = FHE.ge(_totalOpenInterest, p.notionalValue);
        _totalOpenInterest = FHE.select(_safeSub96, FHE.sub(_totalOpenInterest, p.notionalValue), FHE.asEuint64(0));
