...
contract PrivatePeerToPeerLending is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function requestLoan(
        ...
        FHE.allow(loanRequests[id].requestedAmount, msg.sender); // [acl_misconfig]
        FHE.allow(_totalActiveLoans, msg.sender); // [acl_misconfig]
    ...
    function makeRepayment(uint256 loanId, externalEuint64 encPayment, bytes calldata proof) external nonReentrant {
        ...
        euint64 principalPortion = FHE.sub(payment, interestPortion); // [arithmetic_overflow_underflow]
        euint64 interestPortionScaled = FHE.mul(interestPortion, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
