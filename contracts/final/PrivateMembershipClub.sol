...
contract PrivateMembershipClub is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function enrollMember(
        ...
        FHE.allow(m.membershipTier, member); // [acl_misconfig]
        FHE.allow(m.membershipTier, msg.sender); // [acl_misconfig]
        ...
        onlyOwner
    {
        ...
        euint64 ptsWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 ptsExposure = FHE.sub(ptsWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
