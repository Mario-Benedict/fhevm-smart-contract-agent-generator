...
contract LegalPrivateEscrowDispute is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createEscrow(
        ...
        euint64 arbFee = FHE.div(FHE.mul(amount, _arbitrationFeeRate), 10000); // [arithmetic_overflow_underflow]
        euint64 amountScaled = FHE.mul(amount, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        escrows[id].depositor = msg.sender;
        escrows[id].beneficiary = beneficiary;
        escrows[id].amount = FHE.sub(amount, arbFee);
        escrows[id].arbitrationFee = arbFee;
        escrows[id].description = desc;
        escrows[id].releaseDate = block.timestamp + releaseDays * 1 days;
        escrows[id].released = false;
        escrows[id].disputed = false;
        escrows[id].disputeStatus = DisputeStatus.None;
    ...
    function openDispute(
        ...
        disputes[id].escrowId = escrowId;
        disputes[id].claimant = msg.sender;
        disputes[id].claimDescription = claim;
        disputes[id].depositorEvidenceWeight = FHE.asEuint8(0);
        disputes[id].beneficiaryEvidenceWeight = FHE.asEuint8(0);
        disputes[id].proposedSettlement = proposedSettlement;
        disputes[id].assignedArbitrator = address(0);
        disputes[id].resolved = false;
        disputes[id].depositorWon = false;
