...
contract ConfidentialCrossMarginLending is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function repay(
        ...
        ebool _safeSub27 = FHE.ge(owed, repayAmt);
        userBorrows[msg.sender][marketId] = FHE.select(_safeSub27, FHE.sub(owed, repayAmt), FHE.asEuint64(0));
        ebool _safeSub28 = FHE.ge(markets[marketId].totalBorrows, repayAmt);
        markets[marketId].totalBorrows = FHE.select(_safeSub28, FHE.sub(markets[marketId].totalBorrows, repayAmt), FHE.asEuint64(0));
        ebool _safeSub29 = FHE.ge(positions[msg.sender].borrowedValue, repayAmt);
        positions[msg.sender].borrowedValue = FHE.select(_safeSub29, FHE.sub(positions[msg.sender].borrowedValue, repayAmt), FHE.asEuint64(0));
        _refreshHealthFactor(msg.sender);
        FHE.allowThis(userBorrows[msg.sender][marketId]);
        FHE.allowThis(markets[marketId].totalBorrows);
        FHE.allowThis(positions[msg.sender].borrowedValue);
    ...
    function liquidate(address borrower) external onlyLiquidator nonReentrant {
        ...
        ebool _safeSub30 = FHE.ge(pos.depositedValue, seizedCollateral);
        pos.depositedValue = FHE.select(_safeSub30, FHE.sub(pos.depositedValue, seizedCollateral), FHE.asEuint64(0));
