...
contract ConfidentialPeerToPeerInsurancePool is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function joinPool(
        ...
        members[id].coverage = coverage;
        members[id].premiumPaidUSD = premium;
        members[id].coverageLimitUSD = limit;
        members[id].riskScore = risk;
        members[id].claimsHistoryScore = FHE.asEuint64(5000);
        members[id].cashbackReceivedUSD = FHE.asEuint64(0);
        members[id].joinedAt = block.timestamp;
        members[id].policyExpiry = block.timestamp + termMonths * 30 days;
    ...
    function processClaim(uint256 claimId, bool approve, externalEuint64 encApprovedAmt, bytes calldata proof, externalEuint16 encScore, bytes calldata sProof) external onlyOwner nonReentrant {
        ...
        if (approve) {
            ...
            ebool _safeSub58 = FHE.ge(poolReserves[c.coverage], approvedAmt);
            poolReserves[c.coverage] = FHE.select(_safeSub58, FHE.sub(poolReserves[c.coverage], approvedAmt), FHE.asEuint64(0));
