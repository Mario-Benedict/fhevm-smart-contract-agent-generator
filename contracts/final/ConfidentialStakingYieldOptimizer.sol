...
contract ConfidentialStakingYieldOptimizer is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 stakeIndex) external nonReentrant {
        ...
        if (block.timestamp < us.lockupUntil) {
            ...
            ebool _safeSub66 = FHE.ge(withdrawAmount, penalty);
            withdrawAmount = FHE.select(_safeSub66, FHE.sub(withdrawAmount, penalty), FHE.asEuint64(0));
        }
        ebool _safeSub67 = FHE.ge(strat.tvl, us.stakedAmount);
        strategies[us.strategyId].tvl = FHE.select(_safeSub67, FHE.sub(strat.tvl, us.stakedAmount), FHE.asEuint64(0));
        ebool _safeSub68 = FHE.ge(_totalProtocolTVL, us.stakedAmount);
        _totalProtocolTVL = FHE.select(_safeSub68, FHE.sub(_totalProtocolTVL, us.stakedAmount), FHE.asEuint64(0));
