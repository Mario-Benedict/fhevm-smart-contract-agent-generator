...
contract PrivateLabourUnionNegotiationVault is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function openNegotiation(
        ...
        NegotiationSession storage _s0 = sessions[sessionId];
        _s0.unionName = unionName;
        _s0.employerName = employerName;
        _s0.unionLead = msg.sender;
        _s0.employerRep = employerRep;
        _s0.status = NegotiationStatus.OPEN;
        _s0.currentBaseWage = curWage;
        _s0.unionWageDemand = demand;
        _s0.employerWageOffer = FHE.asEuint64(0);
        _s0.mediatedSettlement = FHE.asEuint64(0);
        _s0.unionMemberCount = members;
        _s0.votesForRatification = FHE.asEuint32(0);
        _s0.votesAgainst = FHE.asEuint32(0);
        _s0.strikeFundBalance = strikeFund;
        _s0.sessionStart = block.timestamp;
        _s0.deadline = deadline;
        _s0.ratified = false;
        ...
        FHE.allow(sessions[sessionId].unionWageDemand, msg.sender); // [acl_misconfig]
        FHE.allow(_totalStrikeFunds, msg.sender); // [acl_misconfig]
        FHE.allow(_totalWageGainsAchieved, msg.sender); // [acl_misconfig]
    ...
    function ratifyAgreement(uint256 sessionId) external {
        ...
        euint64 gain = FHE.sub(sessions[sessionId].mediatedSettlement, sessions[sessionId].currentBaseWage); // [arithmetic_overflow_underflow]
