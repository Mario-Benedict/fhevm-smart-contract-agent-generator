...
contract PrivateOilGasPipelineCapacity is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function approveShipper(
        ...
        FHE.allow(shippers[shipper].prepaidBalance, shipper); // [acl_misconfig]
        FHE.allow(shippers[shipper].prepaidBalance, msg.sender); // [acl_misconfig]
    ...
    function confirmNomination(uint256 nomId, externalEuint64 encConfirmed, bytes calldata proof) external {
        ...
        euint64 charge = FHE.mul(nom.confirmedMcf, segments[nom.segmentId].tariffRateUSDPerMcf); // [arithmetic_overflow_underflow]
        euint64 confirmedScaled = FHE.mul(confirmed, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
