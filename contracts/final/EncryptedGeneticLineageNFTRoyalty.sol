...
contract EncryptedGeneticLineageNFTRoyalty is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function mintGeneticSequence(
        ...
        seqId = sequenceCount++;
        sequences[seqId].tokenId = seqId;
        sequences[seqId].sequenceHash = seqHash;
        sequences[seqId].seqType = seqType;
        sequences[seqId].discoveryRoyaltyBps = discRoy;
        sequences[seqId].contributionRoyaltyBps = contRoy;
        sequences[seqId].platformFeeBps = platFee;
        sequences[seqId].totalLicenseRevenue = FHE.asEuint64(0);
        sequences[seqId].contributorCount = FHE.asEuint32(0);
        sequences[seqId].discoveryDate = block.timestamp;
        sequences[seqId].commercialized = false;
        sequences[seqId].patentFiled = false;
        ...
        FHE.allow(sequences[seqId].platformFeeBps, msg.sender); // [acl_misconfig]
        FHE.allow(_platformTotalRevenue, msg.sender); // [acl_misconfig]
    ...
    function grantLicense(
        ...
        licId = licenseCount++;
        licenses[licId].sequenceId = seqId;
        licenses[licId].licensee = licensee;
        licenses[licId].licenseType = licType;
        licenses[licId].upfrontFeeUSD = upfront;
        licenses[licId].annualRoyaltyBps = annRoy;
        licenses[licId].revenueCap = revCap;
        licenses[licId].revenueToDate = FHE.asEuint64(0);
        licenses[licId].effectiveDate = block.timestamp;
        licenses[licId].expiryDate = block.timestamp + durationDays * 1 days;
