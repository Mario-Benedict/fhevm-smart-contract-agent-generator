...
contract TimesharePropertyAuction is ZamaEthereumConfig, Ownable {
    ...
    function listWeek(string calldata resort, uint8 weekNum, uint8 unitType,
        ...
        euint64 askWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 askExposure = FHE.sub(askWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        id = weekCount++;
        listedWeeks[id].resortName = resort;
        listedWeeks[id].weekNumber = weekNum;
        listedWeeks[id].unitType = unitType;
        listedWeeks[id].seller = msg.sender;
        listedWeeks[id].askPrice = ask;
        listedWeeks[id].highestBid = FHE.asEuint64(0);
        listedWeeks[id].highestBidder = address(0);
        listedWeeks[id].deadline = block.timestamp + days_ * 1 days;
        listedWeeks[id].sold = false;
        FHE.allowThis(listedWeeks[id].askPrice);
        FHE.allow(listedWeeks[id].askPrice, msg.sender); // [acl_misconfig]
        FHE.allow(listedWeeks[id].askPrice, msg.sender); // [acl_misconfig]
