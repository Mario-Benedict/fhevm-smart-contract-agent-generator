...
contract EncryptedAircraftMaintenanceBid is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function issueTender(
        ...
        id = tenderCount++;
        tenders[id].airline = msg.sender;
        tenders[id].aircraftReg = acReg;
        tenders[id].aircraftType = acType;
        tenders[id].mxType = mxType;
        tenders[id].estimatedBudgetUSD = budget;
        tenders[id].turnaroundDays = tat;
        tenders[id].requiredCertification = cert;
        tenders[id].submissionDeadline = block.timestamp + deadlineDays * 1 days;
