...
contract PrivateInstitutionalCustodianVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function onboardClient(address client) external onlyOwner {
        ...
        FHE.allow(clients[client].totalNAV, client); // [acl_misconfig]
        FHE.allow(_platformTotalAUM, msg.sender); // [acl_misconfig]
    ...
    function depositAsset(
        ...
        euint64 qtyWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 qtyExposure = FHE.sub(qtyWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
