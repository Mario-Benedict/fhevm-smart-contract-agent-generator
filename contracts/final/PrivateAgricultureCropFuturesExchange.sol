...
contract PrivateAgricultureCropFuturesExchange is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function openFuturesPosition(
        ...
        positions[posKey].farmer = msg.sender;
        positions[posKey].contractId = contractId;
        positions[posKey].contractsHeld = actualContracts;
        positions[posKey].avgEntryPriceUSD = fc.currentFuturesPriceUSD;
        positions[posKey].unrealizedPnLUSD = FHE.asEuint64(0);
        positions[posKey].marginPostedUSD = FHE.select(hasSufficientMargin, initialMargin, FHE.asEuint64(0));
        positions[posKey].maintenanceMarginUSD = maintenanceMargin;
        positions[posKey].physicalHarvestEstKg = harvestEst;
        positions[posKey].isHedger = isHedger;
        positions[posKey].isSpeculator = !isHedger;
        positions[posKey].marginCallActive = false;
