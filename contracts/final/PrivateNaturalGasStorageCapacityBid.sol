...
contract PrivateNaturalGasStorageCapacityBid is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        storagePool.totalWorkingGasBCF = twg;
        storagePool.availableWorkingGasBCF = twg;
        storagePool.maxInjectionRateMmcfd = maxInj;
        storagePool.maxWithdrawalRateMmcfd = maxWdl;
        storagePool.baseReservationCharge = baseRes;
        storagePool.commodityInjectionCharge = FHE.asEuint64(0);
        storagePool.commodityWithdrawalCharge = FHE.asEuint64(0);
        storagePool.minimumInventoryRequirementBCF = FHE.asEuint64(0);
        storagePool.openSeasonStart = 0;
        storagePool.openSeasonEnd = 0;
        storagePool.openSeasonActive = false;
    ...
    function submitCapacityBid(
        ...
        CapacityBid storage _s0 = bids[bidId];
        _s0.bidder = msg.sender;
        _s0.serviceType = serviceType;
        _s0.term = term;
        _s0.status = StorageStatus.OPEN_SEASON;
        _s0.workingGasBidBCF = workingGas;
        _s0.injectionCapacityBidMmcfd = injCapacity;
        _s0.withdrawalCapacityBidMmcfd = wdlCapacity;
        _s0.reservationPriceOffered = priceOffered;
        _s0.priorityIndex = FHE.asEuint64(0);
        _s0.creditSupportPosted = creditSupport;
        _s0.awardedWorkingGasBCF = FHE.asEuint64(0);
        _s0.contractStart = contractStart;
        _s0.contractEnd = contractEnd;
        _s0.priceAccepted = false;
    ...
    function recordInjection(
        ...
        euint64 injFee = FHE.mul(volume, storagePool.commodityInjectionCharge); // [arithmetic_overflow_underflow]
        euint64 volumeScaled = FHE.mul(volume, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
    ...
    function allowStorageStats(address viewer) external onlyOwner {
        FHE.allow(storagePool.totalWorkingGasBCF, viewer); // [acl_misconfig]
        FHE.allow(_totalReservationRevenue, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCommodityRevenue, msg.sender); // [acl_misconfig]
