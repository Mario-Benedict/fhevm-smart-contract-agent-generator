...
contract EncryptedAircraftEngineLeaseTracker is ZamaEthereumConfig, AccessControl, ReentrancyGuard, Pausable {
    ...
    function negotiateLease(
        ...
        leases[leaseId].engineId = engineId;
        leases[leaseId].lessee = msg.sender;
        leases[leaseId].monthlyRent = FHE.fromExternal(encMonthlyRent, rentProof);
        leases[leaseId].maintenanceReserve = FHE.fromExternal(encMaintReserve, maintProof);
        leases[leaseId].creditScore = FHE.fromExternal(encCreditScore, creditProof);
        leases[leaseId].securityDeposit = FHE.fromExternal(encDeposit, depositProof);
        leases[leaseId].totalPaid = FHE.asEuint64(0);
        leases[leaseId].startDate = block.timestamp;
        leases[leaseId].endDate = block.timestamp + durationDays * 1 days;
        leases[leaseId].status = LeaseStatus.Negotiating;
