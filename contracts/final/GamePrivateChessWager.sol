...
contract GamePrivateChessWager is ZamaEthereumConfig, Ownable {
    ...
    function settleWager(uint256 wagerId, bool challengerWon) external onlyOwner {
        ...
        euint64 platformFee = FHE.div(FHE.mul(totalPot, _platformFeeBps), 10000); // [arithmetic_overflow_underflow]
        euint64 totalPotScaled = FHE.mul(totalPot, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
