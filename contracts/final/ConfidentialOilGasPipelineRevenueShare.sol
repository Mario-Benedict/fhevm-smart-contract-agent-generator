...
contract ConfidentialOilGasPipelineRevenueShare is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerSegment(
        ...
        segId = segmentCount++;
        segments[segId].segmentName = name;
        segments[segId].originPoint = origin;
        segments[segId].destinationPoint = destination;
        segments[segId].capacityBOED = capacity;
        segments[segId].currentThroughput = FHE.asEuint64(0);
        segments[segId].tariffRateUSDPerBBL = tariff;
        segments[segId].revenueYTD = FHE.asEuint64(0);
        segments[segId].lengthKm = length;
        segments[segId].utilizationPct = FHE.asEuint8(0);
        segments[segId].operational = true;
    ...
    function signShipperAgreement(
        ...
        ShipperAgreement storage _s0 = agreements[agrId];
        _s0.shipper = shipper;
        _s0.segmentId = segId;
        _s0.shipperClass = sClass;
        _s0.commodity = commodity;
        _s0.reservedCapacityBOED = reserved;
        _s0.minimumVolumeBBL = minVol;
        _s0.actualVolumeShipped = FHE.asEuint64(0);
        _s0.totalTariffPaid = FHE.asEuint64(0);
        _s0.deficiencyPaymentDue = FHE.asEuint64(0);
        _s0.contractDurationDays = duration;
    ...
    function recordMonthlyThroughput(
        ...
        ebool _safeMul10 = FHE.le(volume, FHE.asEuint64(type(uint32).max));
    ...
    function distributeRevenue(
        ...
        ebool _safeSub53 = FHE.ge(gross, FHE.add(opex, maintRes));
        euint64 net     = FHE.select(_safeSub53, FHE.sub(gross, FHE.add(opex, maintRes)), FHE.asEuint64(0));
