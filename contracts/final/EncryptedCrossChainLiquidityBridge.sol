...
contract EncryptedCrossChainLiquidityBridge is ZamaEthereumConfig, AccessControl, ReentrancyGuard, Pausable {
    ...
    function removeLiquidity(externalEuint64 encAmount, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub193 = FHE.ge(lpPositions[msg.sender], actual);
        lpPositions[msg.sender] = FHE.select(_safeSub193, FHE.sub(lpPositions[msg.sender], actual), FHE.asEuint64(0));
        ebool _safeSub194 = FHE.ge(_totalLiquidity, actual);
        _totalLiquidity = FHE.select(_safeSub194, FHE.sub(_totalLiquidity, actual), FHE.asEuint64(0));
        FHE.allowThis(lpPositions[msg.sender]);
        FHE.allow(lpPositions[msg.sender], msg.sender);
        FHE.allowThis(_totalLiquidity);
    }
    ...
    function requestBridge(
        ...
        requests[nonce].sender = msg.sender;
        requests[nonce].recipient = recipient;
        requests[nonce].amount = amount;
        requests[nonce].feeEncrypted = fee;
        requests[nonce].destinationChainId = destinationChainId;
        requests[nonce].nonce = nonce;
        requests[nonce].requestTime = block.timestamp;
        requests[nonce].state = BridgeState.Pending;
        requests[nonce].validationsReceived = 0;
