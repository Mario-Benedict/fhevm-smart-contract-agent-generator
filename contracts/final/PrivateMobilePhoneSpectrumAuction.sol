...
contract PrivateMobilePhoneSpectrumAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createLicense(
        ...
        euint64 reserveWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 reserveExposure = FHE.sub(reserveWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        euint64 coverage = FHE.fromExternal(encCoverage, covProof);
        id = licenseCount++;
        licenses[id].regulatoryRef = regulatoryRef;
        licenses[id].band = band;
        licenses[id].geographicRegion = region;
        licenses[id].bandwidthMHz = bandwidthMHz;
        licenses[id].licenseDurationYears = licenseDurationYears;
        licenses[id].reservePriceUSD = reserve;
        licenses[id].awardedPriceUSD = FHE.asEuint64(0);
        licenses[id].coverageObligationBps = coverage;
        licenses[id].awardedBidder = address(0);
        licenses[id].status = LicenseStatus.BidPhase;
    ...
    function allowRevenueView(address viewer) external onlyOwner {
        FHE.allow(_totalAuctionRevenueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalAuctionRevenueUSD, msg.sender); // [acl_misconfig]
    }
}
