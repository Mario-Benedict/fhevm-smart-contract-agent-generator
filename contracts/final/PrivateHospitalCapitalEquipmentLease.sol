...
contract PrivateHospitalCapitalEquipmentLease is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createEquipmentLease(
        ...
        euint64 totalCommitment = FHE.mul(monthlyRate, FHE.asEuint64(uint64(durationMonths))); // [arithmetic_overflow_underflow]
        euint64 maintReserveScaled = FHE.mul(maintReserve, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        id = leaseCount++;
    ...
    function allowPortfolioStats(address viewer) external onlyOwner {
        FHE.allow(_totalPortfolioValueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalPortfolioValueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalMaintenanceReservesUSD, msg.sender); // [acl_misconfig]
