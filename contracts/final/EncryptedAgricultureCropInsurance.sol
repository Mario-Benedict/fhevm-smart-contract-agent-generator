...
contract EncryptedAgricultureCropInsurance is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function issuePolicy(
        ...
        id = policyCount++;
        policies[id].farmer = farmer;
        policies[id].crop = crop;
        policies[id].insuredAreaHectares = area;
        policies[id].insuredYieldTonnesHa = yield_;
        policies[id].premiumUSD = premium;
        policies[id].maxPayoutUSD = maxPayout;
        policies[id].deductiblePct = FHE.asEuint64(500);
        policies[id].historicalYieldAvg = histYield;
        policies[id].coverageYear = year;
        policies[id].active = true;
        policies[id].claimed = false;
    ...
    function fileClaim(
        ...
        ebool _safeMul30 = FHE.le(shortfall, FHE.asEuint64(type(uint32).max));
        ...
        ebool _safeSub145 = FHE.ge(indemnity, deductible);
        euint64 netIndemnity = FHE.select(_safeSub145, FHE.sub(indemnity, deductible), FHE.asEuint64(0));
    ...
    function payIndemnity(uint256 policyId, uint256 claimIdx) external nonReentrant {
        ...
        ebool _safeSub146 = FHE.ge(_totalPremiumPool, payout);
        _totalPremiumPool = FHE.select(_safeSub146, FHE.sub(_totalPremiumPool, payout), FHE.asEuint64(0));
