...
contract PrivateJewelryDiamondAMLTracker is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function onboardCustomer(
        ...
        euint64 riskScoreWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 riskScoreExposure = FHE.sub(riskScoreWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        Customer storage _s0 = customers[customer];
        _s0.totalTransactionValue30Days = FHE.asEuint64(0);
        _s0.totalTransactionValue12Months = FHE.asEuint64(0);
        _s0.riskScore = riskScore;
        _s0.cashTransactionTotal = FHE.asEuint64(0);
        _s0.transactionCount = FHE.asEuint32(0);
        _s0.riskLevel = initialRisk;
        _s0.sarStatus = SARStatus.NONE;
        _s0.pepFlag = isPEP;
        _s0.sanctionedFlag = false;
        _s0.enhancedDueDiligence = isPEP || initialRisk >= RiskLevel.HIGH;
        _s0.lastScreenedAt = block.timestamp;
        _s0.active = true;

        FHE.allowThis(riskScore);
        FHE.allow(riskScore, msg.sender); // [acl_misconfig]
        FHE.allow(_cashReportingThreshold, msg.sender); // [acl_misconfig]
        FHE.allow(_sarFilingThreshold, msg.sender); // [acl_misconfig]
    ...
    function recordTransaction(
        ...
        JewelryTransaction storage _s1 = transactions[txId];
        _s1.customer = customer;
        _s1.txType = txType;
        _s1.paymentMethod = paymentMethod;
        _s1.transactionValue = txValue;
        _s1.gemstoneCaratWeight = caratWeight;
        _s1.metalWeightGrams = FHE.asEuint64(0);
        _s1.appraisedValue = appraisedValue;
        _s1.certificateNumber = FHE.asEuint32(0);
        _s1.riskAtTimeOfTx = cust.riskLevel;
        _s1.sarFiled = SARStatus.NONE;
        _s1.transactedAt = block.timestamp;
        _s1.reportingThresholdTriggered = thresholdTriggered;
        _s1.beneficialOwnerDisclosed = beneficialOwnerDisclosed;
