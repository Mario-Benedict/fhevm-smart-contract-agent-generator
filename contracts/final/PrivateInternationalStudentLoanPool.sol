...
contract PrivateInternationalStudentLoanPool is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function processRepayment(
        ...
        euint8 newScore = FHE.add(l.repaymentScorePoints, FHE.asEuint8(1)); // [arithmetic_overflow_underflow]
        euint64 appliedScaled = FHE.mul(applied, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
    ...
    function allowPoolStats(address viewer) external onlyOwner {
        FHE.allow(_totalPoolSizeUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalPoolSizeUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalRepaidUSD, msg.sender); // [acl_misconfig]
