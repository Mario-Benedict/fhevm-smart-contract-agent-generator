...
contract PrivateGamingGuildTreasuryDAO is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function enrollMember(
        ...
        euint64 tokenContribWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 tokenContribExposure = FHE.sub(tokenContribWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        members[id].memberAddress = memberAddress;
        members[id].rank = rank;
        members[id].tokenContributionUSD = tokenContrib;
        members[id].scholarshipEarningsUSD = FHE.asEuint64(0);
        members[id].tournamentRevenueUSD = FHE.asEuint64(0);
        members[id].contributionScore = contribScore;
        members[id].governanceWeightBps = govWeight;
        members[id].joinedAt = block.timestamp;
    ...
    function allowTreasuryStats(address viewer) external onlyOwner {
        FHE.allow(_totalTreasuryUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalTreasuryUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalScholarshipPaidUSD, msg.sender); // [acl_misconfig]
