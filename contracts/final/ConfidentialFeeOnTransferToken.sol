...
contract ConfidentialFeeOnTransferToken is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function transfer(address to, externalEuint64 encAmt, bytes calldata proof) external nonReentrant {
        ...
        // Safe subtraction chain: verify each step won't underflow
        euint64 totalDeductions = FHE.add(FHE.add(burnAmt, reflectAmt), feeAmt);
        ebool _deductionSafe = FHE.ge(eff, totalDeductions);
        euint64 netAmt = FHE.select(_deductionSafe, FHE.sub(eff, totalDeductions), FHE.asEuint64(0));
        ebool _safeSub38 = FHE.ge(_balances[msg.sender], eff);
        _balances[msg.sender] = FHE.select(_safeSub38, FHE.sub(_balances[msg.sender], eff), FHE.asEuint64(0));
        _balances[to] = FHE.add(_balances[to], netAmt);
        _totalBurned = FHE.add(_totalBurned, burnAmt);
        ebool _safeSub39 = FHE.ge(_totalSupply, burnAmt);
        _totalSupply = FHE.select(_safeSub39, FHE.sub(_totalSupply, burnAmt), FHE.asEuint64(0));
