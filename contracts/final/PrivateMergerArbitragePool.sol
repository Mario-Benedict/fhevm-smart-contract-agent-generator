...
contract PrivateMergerArbitragePool is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function addDeal(
        ...
        euint64 spreadWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 spreadExposure = FHE.sub(spreadWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
    ...
    function lpDeposit(externalEuint64 encCapital, bytes calldata proof) external {
        ...
        FHE.allowThis(lp.capitalUSD);
        FHE.allow(lp.capitalUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalPoolCapital, msg.sender); // [acl_misconfig]
