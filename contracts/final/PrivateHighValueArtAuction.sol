...
contract PrivateHighValueArtAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function consignLot(
        ...
        id = lotCount++;
        ArtLot storage _s0 = lots[id];
        _s0.consignor = msg.sender;
        _s0.artistName = artistName;
        _s0.workTitle = workTitle;
        _s0.medium = medium;
        _s0.period = period;
        _s0.creationYear = creationYear;
        _s0.estimateLowUSD = lowEst;
        _s0.estimateHighUSD = highEst;
        _s0.hammerPriceUSD = FHE.asEuint64(0);
        _s0.totalWithPremiumUSD = FHE.asEuint64(0);
        _s0.consignorSettlementUSD = FHE.asEuint64(0);
        _s0.provenanceScoreBps = provenance;
        _s0.conditionReportBps = condition;
        _s0.offered = false;
        _s0.sold = false;
    ...
    function hammerLot(
        ...
        euint64 totalWithPremium = FHE.add(hammerPrice, buyerPremium); // [arithmetic_overflow_underflow]
        euint64 buyerPremiumScaled = FHE.mul(buyerPremium, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
    ...
    function allowAuctionStats(address viewer) external onlyOwner {
        FHE.allow(_totalHammerPriceUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalHammerPriceUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalConsignorPayoutsUSD, msg.sender); // [acl_misconfig]
