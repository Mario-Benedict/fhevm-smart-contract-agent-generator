...
contract PrivateElderCareAssetProtection is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function enrollElder(
        ...
        id = profileCount++;
        profiles[id].elder = elder;
        profiles[id].primaryCaregiver = primaryCaregiver;
        profiles[id].careLevel = careLevel;
        profiles[id].totalAssetValueUSD = totalAssets;
        profiles[id].medicaidThresholdUSD = medThreshold;
        profiles[id].monthlyCareCostUSD = monthlyCost;
        profiles[id].careReserveUSD = FHE.asEuint64(0);
        profiles[id].estimatedCareMonths = careMonths;
        profiles[id].active = true;
        profiles[id].enrolledAt = block.timestamp;
