...
contract VotingAcademicTenure is ZamaEthereumConfig, Ownable {
    ...
    function submitScore(
        ...
        candidates[candidateId].totalScore = FHE.add(candidates[candidateId].totalScore, contribution); // [arithmetic_overflow_underflow]
    ...
    function allowCandidateScore(uint256 candidateId, address viewer) external onlyOwner {
        FHE.allow(candidates[candidateId].totalScore, viewer); // [acl_misconfig]
        FHE.allow(_passingScore, msg.sender); // [acl_misconfig]
    }
