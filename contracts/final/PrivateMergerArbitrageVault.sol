...
contract PrivateMergerArbitrageVault is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function takeExposure(
        ...
        euint64 sharesWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 sharesExposure = FHE.sub(sharesWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        FHE.allow(investorExposure[msg.sender][dealId], msg.sender); // [acl_misconfig]
        FHE.allow(vaultTotalExposure, msg.sender); // [acl_misconfig]
