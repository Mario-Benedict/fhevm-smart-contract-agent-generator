...
contract PrivateRealEstateInvestmentTrust is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function acquireProperty(
        ...
        euint64 capRate = valuePlaintext > 0 ? FHE.div(FHE.mul(noi, 10000), valuePlaintext) : FHE.asEuint64(0);
        euint64 ltv = valuePlaintext > 0 ? FHE.div(FHE.mul(mortgage, 10000), valuePlaintext) : FHE.asEuint64(0);
    ...
    function updateAppraisal(
        ...
        p.capRateBps = newValuePlaintext > 0 ? FHE.div(FHE.mul(newNOI, 10000), newValuePlaintext) : FHE.asEuint64(0);
        p.ltvRatioBps = newValuePlaintext > 0 ? FHE.div(FHE.mul(p.mortgageBalanceUSD, 10000), newValuePlaintext) : FHE.asEuint64(0);
        FHE.allowThis(p.appraisedValueUSD);
        FHE.allowThis(p.annualRentIncomeUSD);
        FHE.allowThis(p.netOperatingIncomeUSD);
        FHE.allowThis(p.capRateBps);
    ...
    function claimDividend() external nonReentrant {
        ...
        FHE.allow(inv.totalDividendsReceived, msg.sender); // [acl_misconfig]
        FHE.allow(_totalGrossAssetValue, msg.sender); // [acl_misconfig]
    ...
    function updateNAV(externalEuint64 encNAV, bytes calldata navProof, uint64 sharesOutstandingPlaintext) external onlyREITManager {
        ...
        reitShare.ffoPerShareUSD = sharesOutstandingPlaintext > 0
            ? FHE.div(_distributableFFO, sharesOutstandingPlaintext)
            : FHE.asEuint64(0);
        FHE.allowThis(reitShare.navPerShareUSD);
        FHE.allowThis(reitShare.ffoPerShareUSD);
