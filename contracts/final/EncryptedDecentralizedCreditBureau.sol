...
contract EncryptedDecentralizedCreditBureau is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function initializeProfile(address wallet) external {
        ...
        profiles[wallet].paymentHistoryScore = FHE.asEuint64(175);
        profiles[wallet].debtUtilizationScore = FHE.asEuint64(150);
        profiles[wallet].creditHistoryScore = FHE.asEuint64(75);
        profiles[wallet].newCreditScore = FHE.asEuint64(50);
        profiles[wallet].creditMixScore = FHE.asEuint64(50);
        profiles[wallet].compositeScore = FHE.asEuint64(500);
        profiles[wallet].totalOnChainDebt = FHE.asEuint64(0);
        profiles[wallet].totalCreditLines = FHE.asEuint64(0);
        profiles[wallet].lastUpdated = block.timestamp;
        profiles[wallet].initialized = true;
