...
contract PrivateReinsuranceCatastropheSwap is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function offerSwap(
        ...
        id = swapCount++;
        swaps[id].cedant = msg.sender;
        swaps[id].reinsurer = address(0);
        swaps[id].peril = peril;
        swaps[id].territory = territory;
        swaps[id].attachmentPointUSD = attach;
        swaps[id].exhaustionPointUSD = exhaust;
        swaps[id].limitUSD = lim;
        swaps[id].premiumUSD = premium;
        swaps[id].accumulatedLossUSD = FHE.asEuint64(0);
        swaps[id].status = SwapStatus.Offered;
        swaps[id].inceptionDate = block.timestamp;
    ...
    function bindSwap(uint256 swapId) external {
        ...
        FHE.allowThis(_totalPremiumsUSD);
        FHE.allow(s.attachmentPointUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalPremiumsUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalLossesSettledUSD, msg.sender); // [acl_misconfig]
