...
contract PrivateSportsArbitrageEscrow is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function allowPlatformStats(address viewer) external onlyOwner {
        FHE.allow(_totalCapitalDeployedUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalCapitalDeployedUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalProfitLockedUSD, msg.sender); // [acl_misconfig]
