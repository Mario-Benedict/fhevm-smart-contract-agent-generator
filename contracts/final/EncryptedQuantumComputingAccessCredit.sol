...
contract EncryptedQuantumComputingAccessCredit is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerProcessor(
        ...
        id = processorCount++;
        processors[id].provider = msg.sender;
        processors[id].deviceName = deviceName;
        processors[id].architecture = arch;
        processors[id].qubitCount = qubits;
        processors[id].coherenceTimeMicrosec = coherence;
        processors[id].gateErrorRateBps = gateError;
    ...
    function purchaseSubscription(
        ...
        subscriptions[id].client = msg.sender;
        subscriptions[id].processorId = processorId;
        subscriptions[id].tier = tier;
        subscriptions[id].purchasedQPUHours = _hours;
        subscriptions[id].usedQPUHours = FHE.asEuint64(0);
        subscriptions[id].maxCircuitDepth = maxDepth;
        subscriptions[id].maxQubitsAccessible = maxQubits;
        subscriptions[id].totalSpentCents = totalCost;
        subscriptions[id].validUntil = block.timestamp + validDays * 1 days;
        subscriptions[id].active = true;
