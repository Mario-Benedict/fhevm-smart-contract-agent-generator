...
contract PrivateNutritionTracker is ZamaEthereumConfig, Ownable {
    ...
    function registerUser() external {
        ...
        euint16 dailyBurn = FHE.mul(FHE.asEuint16(uint16(block.timestamp % 500)), FHE.asEuint16(10)); // [arithmetic_overflow_underflow]
        euint16 deficit = FHE.sub(dailyBurn, FHE.asEuint16(2000)); // [arithmetic_overflow_underflow]
        FHE.allowThis(g.targetCarbs); FHE.allowThis(g.targetFat);
        FHE.allow(g.targetCalories, msg.sender); // [acl_misconfig]
    ...
    function recordDailyLog(
        ...
        l.logDate      = FHE.asEuint32(logDate);
        FHE.allowThis(l.logDate);
        l.logged       = true;
