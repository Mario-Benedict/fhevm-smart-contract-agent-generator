...
contract PrivateOncologyTreatmentRegistry is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerPatient(
        ...
        euint64 tumorSizeWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 tumorSizeExposure = FHE.sub(tumorSizeWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        patients[patientHash].patientHash = patientHash;
        patients[patientHash].cancerType = ctype;
        patients[patientHash].stageAtDiagnosis = stage;
        patients[patientHash].primaryTumorSizeMm = tumorSize;
        patients[patientHash].biomarkerScore = biomarker;
        patients[patientHash].treatmentResponseBps = FHE.asEuint64(0);
        patients[patientHash].survivalProbBps = survival;
        patients[patientHash].trialEligibilityScore = eligibility;
        patients[patientHash].diagnosisDate = block.timestamp;
        patients[patientHash].active = true;
    ...
    function startTreatment(
        ...
        courses[courseId].patientHash = patientHash;
        courses[courseId].treatment = treatment;
        courses[courseId].doseIntensity = dose;
        courses[courseId].cycleCount = cycles;
        courses[courseId].responseScore = FHE.asEuint64(0);
        courses[courseId].toxicityScore = FHE.asEuint64(0);
        courses[courseId].qualityOfLifeScore = FHE.asEuint64(0);
        courses[courseId].startDate = block.timestamp;
        courses[courseId].endDate = 0;
        courses[courseId].completed = false;
    ...
    function allowResearchView(bytes32 patientHash, address coordinator) external {
        ...
        FHE.allow(patients[patientHash].biomarkerScore, coordinator); // [acl_misconfig]
        FHE.allow(_cohortSurvivalAvg, msg.sender); // [acl_misconfig]
