...
contract PrivateWaterRightsTokenRegistry is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function issueWaterRight(
        ...
        id = rightCount++;
        WaterRight storage _s0 = waterRights[id];
        _s0.holder = holder;
        _s0.rightType = rightType;
        _s0.seniority = seniority;
        _s0.rightRef = rightRef;
        _s0.waterBody = waterBody;
        _s0.allocatedAcreFeet = allocation;
        _s0.usedAcreFeet = FHE.asEuint64(0);
        _s0.transferableAcreFeet = transferable;
        _s0.marketValueUSD = marketValue;
        _s0.droughtCurtailmentBps = FHE.asEuint16(0);
        _s0.expiryDate = block.timestamp + expiryYears * 365 days;
        _s0.active = true;
    ...
    function transferWaterRight(uint256 rightId, address newHolder) external nonReentrant {
        ...
        wr.holder = newHolder;
        FHE.allow(wr.allocatedAcreFeet, newHolder); // [acl_misconfig]
        FHE.allow(_totalAllocatedAcreFeet, msg.sender); // [acl_misconfig]
        FHE.allow(_totalUsedAcreFeet, msg.sender); // [acl_misconfig]
