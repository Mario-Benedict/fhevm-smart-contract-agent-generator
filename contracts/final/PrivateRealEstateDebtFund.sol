...
contract PrivateRealEstateDebtFund is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function originateLoan(
        ...
        euint64 ltv = propValuePlaintext > 0
            ? FHE.div(FHE.mul(loanAmt, 10000), propValuePlaintext)
            : FHE.asEuint64(0);
        id = loanCount++;
        RealEstateLoan storage _s0 = loans[id];
        _s0.loanId = loanId;
        _s0.borrower = borrower;
        _s0.propertyType = ptype;
        _s0.loanAmountUSD = loanAmt;
        _s0.propertyValueUSD = propValue;
        _s0.ltvRatioBps = ltv;
        _s0.interestRateBps = rate;
        _s0.originationFeeBps = fee;
        _s0.dscr = dscr;
        _s0.borrowerCreditScore = credit;
        _s0.accrualBalance = FHE.asEuint64(0);
        _s0.status = LoanStatus.ORIGINATED;
        ...
        FHE.allow(loans[id].ltvRatioBps, borrower); // [acl_misconfig]
