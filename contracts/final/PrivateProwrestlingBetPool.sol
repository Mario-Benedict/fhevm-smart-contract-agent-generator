...
contract PrivateProwrestlingBetPool is ZamaEthereumConfig, Ownable {
    ...
    function createMatch(string calldata w1, string calldata w2, uint256 matchHoursFromNow) external onlyOfficial returns (uint256 id) {
        id = matchCount++;
        matches[id].wrestler1 = w1;
        matches[id].wrestler2 = w2;
        matches[id].poolWrestler1 = FHE.asEuint64(0);
        matches[id].poolWrestler2 = FHE.asEuint64(0);
        matches[id].poolDraw = FHE.asEuint64(0);
        matches[id].randomFactor = FHE.randEuint8();
        matches[id].matchTime = block.timestamp + matchHoursFromNow * 1 hours;
        matches[id].winner = 0;
        matches[id].resolved = false;
    ...
    function placeBet(uint256 matchId, uint8 outcome, externalEuint64 encBet, bytes calldata proof) external {
        ...
        FHE.allow(_bets[matchId][msg.sender][outcome], msg.sender); // [acl_misconfig]
        FHE.allow(matches[matchId].poolWrestler1, msg.sender); // [acl_misconfig]
