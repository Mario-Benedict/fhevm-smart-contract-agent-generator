...
contract ConfidentialYieldToken is ZamaEthereumConfig, Ownable, Pausable, ReentrancyGuard {
    ...
    function setVestingSchedule(
        ...
        ebool _safeSub79 = FHE.ge(_encryptedBalances[msg.sender], amount);
        _encryptedBalances[msg.sender] = FHE.select(_safeSub79, FHE.sub(_encryptedBalances[msg.sender], amount), FHE.asEuint64(0));
        FHE.allowThis(_encryptedBalances[msg.sender]);
        FHE.allow(_encryptedBalances[msg.sender], msg.sender);
    ...
    function transfer(
        ...
        ebool _safeSub80 = FHE.ge(_encryptedBalances[msg.sender], actualAmount);
        _encryptedBalances[msg.sender] = FHE.select(_safeSub80, FHE.sub(_encryptedBalances[msg.sender], actualAmount), FHE.asEuint64(0));
