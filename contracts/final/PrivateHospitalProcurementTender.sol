...
contract PrivateHospitalProcurementTender is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function issueTender(
        ...
        euint64 budgetWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 budgetExposure = FHE.sub(budgetWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        euint32 minQ = FHE.fromExternal(encMinQuality, qProof);
        id = tenderCount++;
        tenders[id].hospitalName = hospitalName;
        tenders[id].category = category;
        tenders[id].itemDescription = itemDesc;
        tenders[id].estimatedBudgetUSD = budget;
        tenders[id].requiredQualityScore = minQ;
        tenders[id].submissionDeadline = block.timestamp + submissionDays * 1 days;
        tenders[id].awardDate = 0;
        tenders[id].status = TenderStatus.Open;
        tenders[id].winningBidId = type(uint256).max;
    ...
    function submitBid(
        ...
        FHE.allow(bids[bidId].bidPriceUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalProcuredUSD, msg.sender); // [acl_misconfig]
