...
contract EncryptedAIModelMarketplace is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerModel(
        ...
        id = modelCount++;
        models[id].modelName = name;
        models[id].modelVersion = version;
        models[id].modelType = modelType;
        models[id].provider = msg.sender;
        models[id].accuracyBps = accuracy;
        models[id].latencyMs = latency;
        models[id].pricePerCallUSD = priceCall;
        models[id].monthlySubscription = monthly;
        models[id].totalRevenue = FHE.asEuint64(0);
        models[id].totalCalls = FHE.asEuint64(0);
        models[id].active = true;
    ...
    function makeAPICall(bytes32 licId) external nonReentrant {
        ...
        ebool _safeSub147 = FHE.ge(lic.callsRemaining, callDeduct);
        lic.callsRemaining = FHE.select(_safeSub147, FHE.sub(lic.callsRemaining, callDeduct), FHE.asEuint64(0));
        ...
        ebool _safeSub148 = FHE.ge(lic.prepaidBalance, charge);
        lic.prepaidBalance = FHE.select(_safeSub148, FHE.sub(lic.prepaidBalance, charge), FHE.asEuint64(0));
        ...
        ebool _safeSub149 = FHE.ge(charge, platformFee);
        euint64 providerShare = FHE.select(_safeSub149, FHE.sub(charge, platformFee), FHE.asEuint64(0));
