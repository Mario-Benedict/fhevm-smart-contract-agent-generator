...
contract EncryptedRareEarthMineralCertificate is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerBatch(
        ...
        id = batchCount++;
        batches[id].miner = msg.sender;
        batches[id].mineId = mineId;
        batches[id].mineralType = mineralType;
        batches[id].countryOfOrigin = country;
        batches[id].batchWeightKg = weight;
        batches[id].purityPercBps = purity;
        batches[id].estimatedValueUSD = val;
        batches[id].radiationLevel = radiation;
        batches[id].mineDate = block.timestamp;
