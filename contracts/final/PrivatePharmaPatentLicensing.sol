...
contract PrivatePharmaPatentLicensing is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function licensePatent(
        ...
        id = patentCount++;
        patents[id].originator = msg.sender;
        patents[id].patentNumber = patentNo;
        patents[id].drugName = drugName;
        patents[id].indication = indication;
        patents[id].stage = stage;
        patents[id].upfrontPaymentUSD = upfront;
        patents[id].royaltyRateBps = royalty;
        patents[id].maxRoyaltyCap = cap;
        patents[id].totalRoyaltiesEarned = FHE.asEuint64(0);
        patents[id].expiryDate = block.timestamp + licenseYears * 365 days;
        patents[id].granted = true;
    ...
    function allowIPStats(address viewer) external onlyOwner {
        FHE.allow(_totalLicenseRevenue, viewer); // [acl_misconfig]
        FHE.allow(_totalLicenseRevenue, msg.sender); // [acl_misconfig]
    }
