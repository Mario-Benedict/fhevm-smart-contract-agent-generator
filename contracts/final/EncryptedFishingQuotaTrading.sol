...
contract EncryptedFishingQuotaTrading is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function assignQuota(
        ...
        id = quotaCount++;
        quotas[id].vesselId = vesselId;
        quotas[id].species = species;
        quotas[id].fishingZone = zone;
        quotas[id].allowedTonnes = allowance;
        quotas[id].caughtTonnes = FHE.asEuint64(0);
        quotas[id].remainingTonnes = allowance;
        quotas[id].listedPriceCentsPerT = FHE.asEuint64(0);
        quotas[id].seasonEnd = block.timestamp + seasonDays * 1 days;
        quotas[id].status = QuotaStatus.Assigned;
    ...
    function reportCatch(
        ...
        ebool _safeSub231 = FHE.ge(q.remainingTonnes, FHE.asEuint64(0));
        q.remainingTonnes = FHE.select(_safeSub231, FHE.sub(q.remainingTonnes, FHE.asEuint64(0)), FHE.asEuint64(0));
