...
contract EncryptedTaxiFleetDynamicPricing is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function requestRide(
        ...
        rideId = rideCount++;
        rides[rideId].passenger = msg.sender;
        rides[rideId].driver = address(0);
        rides[rideId].vehicleClass = vClass;
        rides[rideId].baseFareUSD = baseFare;
        rides[rideId].surgeMultiplierBps = surge;
        rides[rideId].finalFareUSD = finalFare;
        rides[rideId].driverPayout = driverPayout;
        rides[rideId].distanceMeters = FHE.asEuint32(0);
        rides[rideId].requestTime = block.timestamp;
        rides[rideId].completionTime = 0;
        rides[rideId].status = RideStatus.PENDING;
