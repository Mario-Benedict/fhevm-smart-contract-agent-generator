...
contract PrivateNFTFractionalizationVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintShares(
        ...
        FHE.allow(sh.sharesHeld, to); // [acl_misconfig]
        FHE.allow(_totalLockedValueUSD, msg.sender); // [acl_misconfig]
    ...
    function claimRevenue(uint256 vaultId) external nonReentrant whenNotPaused {
        ...
        euint64 unclaimed = FHE.sub(proRataRevenue, sh.unclaimedRevenue); // [arithmetic_overflow_underflow]
        euint64 proRataRevenueScaled = FHE.mul(proRataRevenue, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
