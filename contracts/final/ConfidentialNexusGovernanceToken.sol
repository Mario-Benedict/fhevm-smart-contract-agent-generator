...
contract ConfidentialNexusGovernanceToken is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function transfer(address to, externalEuint64 encAmt, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub50 = FHE.ge(_balances[msg.sender], effectiveAmt);
        _balances[msg.sender] = FHE.select(_safeSub50, FHE.sub(_balances[msg.sender], effectiveAmt), FHE.asEuint64(0));
        _balances[to] = FHE.add(_balances[to], effectiveAmt);
        ...
        if (del != address(0)) {
            ...
            ebool _safeSub51 = FHE.ge(_delegatedPower[del], effectiveAmt);
            _delegatedPower[del] = FHE.select(_safeSub51, FHE.sub(_delegatedPower[del], effectiveAmt), FHE.asEuint64(0));
    ...
    function delegateTo(address delegatee) external {
        ...
        if (prev != address(0) && FHE.isInitialized(_delegatedPower[prev])) {
            ebool _safeSub52 = FHE.ge(_delegatedPower[prev], _balances[msg.sender]);
            _delegatedPower[prev] = FHE.select(_safeSub52, FHE.sub(_delegatedPower[prev], _balances[msg.sender]), FHE.asEuint64(0));
