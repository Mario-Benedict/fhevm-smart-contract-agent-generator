...
contract TradeFinancePrivate is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function depositBankReserves(externalEuint64 encAmount, bytes calldata proof) external {
        ...
        FHE.allow(_bankReserves[msg.sender], msg.sender); // [acl_misconfig]
        FHE.allow(_totalFinanced, msg.sender); // [acl_misconfig]
    }
    ...
    function financeInvoice(
        ...
        euint64 financed = FHE.sub(inv.invoiceAmount, discount); // [arithmetic_overflow_underflow]
        euint64 discountScaled = FHE.mul(discount, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
