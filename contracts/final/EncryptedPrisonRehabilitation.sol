...
contract EncryptedPrisonRehabilitation is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerInmate(
        ...
        FHE.allow(risk, msg.sender); // [acl_misconfig]
        FHE.allow(_totalReintegrationPool, msg.sender); // [acl_misconfig]
        euint8 edu = FHE.fromExternal(encEducation, eProof);
        euint8 recidivism = FHE.fromExternal(encRecidivism, recProof);
        records[inmateHash].inmateHash = inmateHash;
        records[inmateHash].riskScore = risk;
        records[inmateHash].educationLevel = edu;
        records[inmateHash].programHoursCompleted = FHE.asEuint64(0);
        records[inmateHash].reintegrationFund = FHE.asEuint64(0);
        records[inmateHash].paroleRecommendation = FHE.asEuint8(0);
        records[inmateHash].recidivismRisk = recidivism;
        records[inmateHash].intakeDate = block.timestamp;
        records[inmateHash].active = true;
