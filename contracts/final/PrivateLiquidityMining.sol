...
contract PrivateLiquidityMining is ZamaEthereumConfig, Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        FHE.allowThis(p.totalLPStaked);
        FHE.allow(m.lpAmount, msg.sender); // [acl_misconfig]
    ...
    function harvest(uint256 poolId) external nonReentrant {
        ...
        euint64 earned = FHE.mul(m.lpAmount, FHE.mul(p.emissionRatePerBlock, FHE.asEuint64(uint64(blocksElapsed)))); // [arithmetic_overflow_underflow]
