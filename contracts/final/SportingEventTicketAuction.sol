...
contract SportingEventTicketAuction is ZamaEthereumConfig, Ownable {
    ...
    function createLot(string calldata event_, uint256 seats, string calldata section,
        ...
        euint64 floorWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 floorExposure = FHE.sub(floorWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        id = lotCount++;
