...
contract VotingPrivateShareholder is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerShareholder(
        ...
        FHE.allow(shareholders[holder].votingShares, holder); // [acl_misconfig]
        FHE.allow(_totalVotingShares, msg.sender); // [acl_misconfig]
        FHE.allowThis(_totalVotingShares);
    }
    ...
    function executeResolution(uint256 resId) external {
        ...
        euint64 totalVotes = FHE.add(FHE.add(res.votesFor, res.votesAgainst), res.votesAbstain); // [arithmetic_overflow_underflow]
