...
contract PrivatePatientInsuranceClaim is ZamaEthereumConfig, Ownable {
    ...
    function issuePolicy(
        ...
        FHE.allow(policies[patient].annualCoverageLimit, patient); // [acl_misconfig]
        FHE.allow(_totalClaimsApproved, msg.sender); // [acl_misconfig]
    ...
    function processClaim(uint256 claimId) external onlyAdjuster {
        ...
        euint64 insurancePays = FHE.sub(afterDeductible, copayAmt); // [arithmetic_overflow_underflow]
        euint64 copayAmtScaled = FHE.mul(copayAmt, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
