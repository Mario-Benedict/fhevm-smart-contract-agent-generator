...
contract PrivateLivestockGeneticsMarketplace is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function listGeneticLot(
        ...
        euint64 totalLotVal = FHE.mul(FHE.asEuint64(1), price); // [arithmetic_overflow_underflow]
        euint64 royaltyBpsScaled = FHE.mul(royaltyBps, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        id = lotCount++;
        GeneticLot storage _s0 = lots[id];
        _s0.breeder = msg.sender;
        _s0.species = species;
        _s0.materialType = materialType;
        _s0.sireId = sireId;
        _s0.damId = damId;
        _s0.unitCount = units;
        _s0.pricePerUnitUSD = price;
        _s0.totalLotValueUSD = totalLotVal;
        _s0.ebvRankingBps = ebv;
        _s0.genomicTestScore = genomic;
        _s0.bloodlineRoyaltyBps = royaltyBps;
        _s0.verified = false;
    ...
    function verifyLot(uint256 lotId) external onlyGeneticsAuthority {
        ...
        FHE.allow(lots[lotId].ebvRankingBps, msg.sender); // [acl_misconfig]
        FHE.allow(_totalMarketplaceValueUSD, msg.sender); // [acl_misconfig]
