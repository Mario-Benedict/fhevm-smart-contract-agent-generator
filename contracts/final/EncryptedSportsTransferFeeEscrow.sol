...
contract EncryptedSportsTransferFeeEscrow is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function initiateTransfer(
        ...
        id = transferCount++;
        transfers[id].sellingClub = msg.sender;
        transfers[id].buyingClub = buyingClub;
        transfers[id].playerName = playerName;
        transfers[id].playerPosition = position;
        transfers[id].baseTransferFeeUSD = base;
        transfers[id].contingentAddOnsUSD = addons;
        transfers[id].sellOnClausePercBps = sellOn;
        transfers[id].totalPaidUSD = FHE.asEuint64(0);
        transfers[id].contractLengthMonths = contractLen;
