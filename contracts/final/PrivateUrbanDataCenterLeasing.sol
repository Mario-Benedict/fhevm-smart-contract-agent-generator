...
contract PrivateUrbanDataCenterLeasing is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function allowFacilityStats(address viewer) external onlyOwner {
        FHE.allow(_totalLeaseRevenueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalLeaseRevenueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCarbonFootprintKg, msg.sender); // [acl_misconfig]
