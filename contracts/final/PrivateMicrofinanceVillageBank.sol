...
contract PrivateMicrofinanceVillageBank is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function joinGroup(uint256 groupId, externalEuint64 encWeeklyContribution, bytes calldata proof) external {
        ...
        FHE.allow(accounts[key].savingsBalance, msg.sender); // [acl_misconfig]
        FHE.allow(_totalNetworkSavings, msg.sender); // [acl_misconfig]
        FHE.allow(_totalNetworkLoans, msg.sender); // [acl_misconfig]
    ...
    function requestLoan(
        ...
        euint64 maxLoan = FHE.mul(acc.savingsBalance, FHE.asEuint64(3)); // [arithmetic_overflow_underflow]
