...
contract PrivateCinemaBoxOfficeRevenueSharing is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerFilm(
        ...
        id = filmCount++;
        films[id].distributor = msg.sender;
        films[id].filmTitle = filmTitle;
        films[id].rating = rating;
        films[id].settlementType = settlementType;
        films[id].productionBudgetUSD = prodBudget;
        films[id].marketingBudgetUSD = mktBudget;
        films[id].openingWeekendBoxUSD = FHE.asEuint32(0);
        films[id].cumulativeBoxOfficeUSD = FHE.asEuint32(0);
        films[id].distributorSplitBps = distSplit;
        films[id].exhibitorFloorBps = exhFloor;
        films[id].wideRelease = wideRelease;
