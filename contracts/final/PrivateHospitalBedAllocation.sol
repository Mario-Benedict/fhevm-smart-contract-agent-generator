...
contract PrivateHospitalBedAllocation is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function admitPatient(
        ...
        FHE.allow(patients[patient].acuityScore, msg.sender); // [acl_misconfig]
        FHE.allow(_totalRevenueAccrued, msg.sender); // [acl_misconfig]
        FHE.allow(_totalBedsSystem, msg.sender); // [acl_misconfig]
    ...
    function updateBilling(address patient) external onlyClinician {
        ...
        euint64 totalBilling = FHE.mul(p.dailyRateUSD, FHE.asEuint64(uint64(daysElapsed))); // [arithmetic_overflow_underflow]
