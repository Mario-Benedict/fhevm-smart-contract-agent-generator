...
contract EncryptedPredictionMarketResolution is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createMarket(
        ...
        markets[id].marketRef = marketRef;
        markets[id].totalPoolA = FHE.asEuint64(0);
        markets[id].totalPoolB = FHE.asEuint64(0);
        markets[id].totalFees = FHE.asEuint64(0);
        markets[id].oddsA = FHE.asEuint64(5000);
        markets[id].oddsB = FHE.asEuint64(5000);
        markets[id].outcome = MarketOutcome.Unresolved;
        markets[id].resolutionDeadline = block.timestamp + resolutionDays * 1 days;
        markets[id].resolutionOracle = resolutionOracle;
        markets[id].resolved = false;
