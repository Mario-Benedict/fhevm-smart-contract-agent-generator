...
contract ERC20ConfidentialVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function depositAndMint(externalEuint64 encCollateral, bytes calldata proof, uint64 minRatioPlaintext, uint64 newSupplyPlaintext) external nonReentrant whenNotPaused {
        ...
        euint64 tokensToMint = minRatioPlaintext > 0 ? FHE.div(FHE.mul(collateral, 10000), minRatioPlaintext) : FHE.asEuint64(0); // [arithmetic_overflow_underflow]
        euint64 collateralScaled = FHE.mul(collateral, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
