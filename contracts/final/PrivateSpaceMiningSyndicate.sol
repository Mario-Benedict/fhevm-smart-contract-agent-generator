...
contract PrivateSpaceMiningSyndicate is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function addMember(
        ...
        FHE.allow(members[idx].sharesBps, member); // [acl_misconfig]
        FHE.allow(_syndicateTreasuryUSD, msg.sender); // [acl_misconfig]
    ...
    function registerClaim(
        ...
        id = claimCount++;
        claims[id].claimHolder = msg.sender;
        claims[id].asteroidId = asteroidId;
        claims[id].oreType = oreType;
        claims[id].estimatedTonnes = tonnes;
        claims[id].extractionCostUSD = cost;
        claims[id].marketValueUSD = mktValue;
        claims[id].netRevenueUSD = FHE.asEuint64(0);
        claims[id].missionLaunch = block.timestamp;
        claims[id].status = MissionStatus.Planning;
