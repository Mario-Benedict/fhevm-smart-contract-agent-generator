...
contract PrivateHedgeFundShares is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function onboardInvestor(address investor, uint256 lockupDays) external {
        ...
        FHE.allow(investors[investor].sharesOwned, investor); // [acl_misconfig]
        FHE.allow(_navPerShare, msg.sender); // [acl_misconfig]
    ...
    function requestRedemption(externalEuint64 encShares, bytes calldata proof) external nonReentrant {
        ...
        euint64 redemptionValue = FHE.mul(actualShares, _navPerShare); // [arithmetic_overflow_underflow]
        euint64 actualSharesScaled = FHE.mul(actualShares, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
