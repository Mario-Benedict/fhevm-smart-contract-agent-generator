...
contract PrivateGeneticDataVault is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createProfile(
        ...
        profiles[id].owner = msg.sender;
        profiles[id].sampleRef = sampleRef;
        profiles[id].ancestryEuropeBps = ancEurope;
        profiles[id].ancestryAsiaBps = ancAsia;
        profiles[id].ancestryAfricaBps = ancAfrica;
        profiles[id].diseaseRiskScore = diseaseRisk;
        profiles[id].pharmacogenomicScore = pharma;
        profiles[id].consentScope = consent;
        profiles[id].licensingPriceUSD = price;
        profiles[id].createdAt = block.timestamp;
    ...
    function licenseGenomicData(uint256 profileId, uint256 durationDays) external onlyGenomicsResearcher whenNotPaused nonReentrant returns (uint256 licenseId) {
        ...
        euint64 platformShare = FHE.sub(fee, ownerShare); // [arithmetic_overflow_underflow]
        euint64 ownerShareScaled = FHE.mul(ownerShare, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        ...
        FHE.allow(ownerShare, p.owner); // [acl_misconfig]
        FHE.allow(_totalResearchRevenue, msg.sender); // [acl_misconfig]
        FHE.allow(_totalLicensesIssued, msg.sender); // [acl_misconfig]
