...
contract PrivateInsuranceSubrogationPool is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function fileClaim(
        ...
        id = claimCount++;
        claims[id].claimRef = claimRef;
        claims[id].primaryInsurer = msg.sender;
        claims[id].thirdPartyInsurer = thirdPartyInsurer;
        claims[id].originalClaimUSD = original;
        claims[id].recoveryAmountUSD = FHE.asEuint64(0);
        claims[id].legalCostUSD = FHE.asEuint64(0);
        claims[id].netRecoveryUSD = FHE.asEuint64(0);
        claims[id].liabilitySplitBps = liabilitySplit;
        claims[id].filedAt = block.timestamp;
        claims[id].settled = false;
        claims[id].litigated = false;
        ...
        FHE.allow(claims[id].originalClaimUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalRecovered, msg.sender); // [acl_misconfig]
        FHE.allow(_totalLegalCosts, msg.sender); // [acl_misconfig]
    ...
    function settleClaim(uint256 claimId) external nonReentrant {
        ...
        euint64 thirdPartyShare = FHE.sub(cl.netRecoveryUSD, primaryShare); // [arithmetic_overflow_underflow]
        euint64 primaryShareScaled = FHE.mul(primaryShare, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
