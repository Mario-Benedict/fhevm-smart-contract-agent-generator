...
contract PrivateGeneticDataMarketplace is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function contributeDataset(
        ...
        euint64 feeWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 feeExposure = FHE.sub(feeWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        id = datasetCount++;
    ...
    function requestAccess(
        ...
        FHE.allow(licenses[licenseId].paidFeeUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalMarketRevenue, msg.sender); // [acl_misconfig]
        FHE.allow(_totalDonorPayouts, msg.sender); // [acl_misconfig]
