...
contract EncryptedRandomizedClinicalTrial is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function enrollPatient(
        ...
        id = patientCount++;
        patients[id].patientWallet = patientWallet;
        patients[id].patientRef = patientRef;
        patients[id].trialArm = arm;
        patients[id].dosageCode = dosage;
        patients[id].baselineScore = baseline;
        patients[id].weekFourScore = FHE.asEuint16(0);
        patients[id].weekEightScore = FHE.asEuint16(0);
        patients[id].adverseEventGrade = FHE.asEuint8(0);
        patients[id].enrolledAt = block.timestamp;
        patients[id].withdrawn = false;
