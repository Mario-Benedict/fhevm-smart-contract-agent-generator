...
contract DeFiEncryptedRebalancer is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function withdraw(externalEuint64 encShares, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub116 = FHE.ge(userShares[msg.sender], actual);
        userShares[msg.sender] = FHE.select(_safeSub116, FHE.sub(userShares[msg.sender], actual), FHE.asEuint64(0));
        ebool _safeSub117 = FHE.ge(_totalShares, actual);
        _totalShares = FHE.select(_safeSub117, FHE.sub(_totalShares, actual), FHE.asEuint64(0));
        euint64 returned = actual; // simplified
        ebool _safeSub118 = FHE.ge(portfolio.totalValue, returned);
        portfolio.totalValue = FHE.select(_safeSub118, FHE.sub(portfolio.totalValue, returned), FHE.asEuint64(0));
        FHE.allowThis(userShares[msg.sender]);
        FHE.allow(userShares[msg.sender], msg.sender);
        FHE.allowThis(_totalShares);
