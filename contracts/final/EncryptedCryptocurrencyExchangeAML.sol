...
contract EncryptedCryptocurrencyExchangeAML is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function recordTransaction(
        ...
        txId = txCount++;
        transactions[txId].sender = sender;
        transactions[txId].recipient = recipient;
        transactions[txId].amountUSD = amount;
        transactions[txId].mlRiskScore = mlScore;
        transactions[txId].velocityScore = velocity;
        transactions[txId].chainRef = chainRef;
        transactions[txId].timestamp = block.timestamp;
        transactions[txId].flagged = flagged;
        transactions[txId].reported = false;
