...
contract PrivatePrisonRehabilitationFunding is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function issueBond(
        ...
        id = bondCount++;
        SocialImpactBond storage _s0 = bonds[id];
        _s0.serviceProvider = msg.sender;
        _s0.outcomePayer = outcomePayer;
        _s0.programType = programType;
        _s0.prisonFacilityId = facilityId;
        _s0.participantCount = participants;
        _s0.programCostUSD = cost;
        _s0.maximumPayoutUSD = maxPayout;
        _s0.targetRecidivismBps = targetRecid;
        _s0.baselineRecidivismBps = baselineRecid;
        _s0.actualPayoutUSD = FHE.asEuint64(0);
        _s0.measurementDate = block.timestamp + measurementDays * 1 days;
        _s0.status = OutcomeStatus.InProgress;
        _totalCommitted = FHE.add(_totalCommitted, maxPayout);
        FHE.allowThis(bonds[id].participantCount);
        FHE.allow(bonds[id].participantCount, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCommitted, msg.sender); // [acl_misconfig]
        FHE.allow(_totalPaidOut, msg.sender); // [acl_misconfig]
