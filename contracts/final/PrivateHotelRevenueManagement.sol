...
contract PrivateHotelRevenueManagement is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createBooking(
        ...
        euint64 totalCharge = FHE.mul(rate, FHE.asEuint64(uint64(nightsStay))); // [arithmetic_overflow_underflow]
        euint64 rateScaled = FHE.mul(rate, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        ...
        FHE.allow(bookings[bookingId].rateChargedUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalHotelRevenue, msg.sender); // [acl_misconfig]
        FHE.allow(_totalDepositHeld, msg.sender); // [acl_misconfig]
