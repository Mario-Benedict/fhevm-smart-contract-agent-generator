...
contract PrivateHedgeFundSideLetterTerms is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function signSideLetter(
        ...
        euint64 commitmentWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 commitmentExposure = FHE.sub(commitmentWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        ...
        id = sideLetterCount++;
        SideLetter storage _s0 = sideLetters[id];
        _s0.investor = investor;
        _s0.fundManager = msg.sender;
        _s0.investorClass = investorClass;
        _s0.feeArrangement = feeArrangement;
        _s0.committedCapitalUSD = commitment;
        _s0.negotiatedMgmtFeeBps = mgmtFee;
        _s0.negotiatedPerfFeeBps = perfFee;
        _s0.capacityReservationFeeUSD = capacityFee;
        _s0.lockupDays = lockup;
        _s0.redemptionPriorityScore = redemptionPriority;
        _s0.highWaterMarkUSD = FHE.asEuint64(0);
        _s0.signedAt = block.timestamp;
        _s0.termEndDate = block.timestamp + termDays * 1 days;
        _s0.active = true;
    ...
    function allowCapitalStats(address viewer) external onlyOwner {
        FHE.allow(_totalCommittedCapitalUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalCommittedCapitalUSD, msg.sender); // [acl_misconfig]
    }
}
