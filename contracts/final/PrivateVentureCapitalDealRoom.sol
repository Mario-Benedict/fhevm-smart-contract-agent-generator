...
contract PrivateVentureCapitalDealRoom is ZamaEthereumConfig, Ownable {
    ...
    function submitDeal(
        ...
        id = dealCount++;
        deals[id].companyName = company;
        deals[id].sector = sector;
        deals[id].requestedAmount = ask;
        deals[id].preMoneyValuation = valuation;
        deals[id].revenueARR = arr;
        deals[id].burnRateMonthly = burn;
        deals[id].partnerVoteScore = FHE.asEuint64(0);
        deals[id].partnerVoteCount = 0;
        deals[id].stage = DealStage.Screening;
        deals[id].dealLead = msg.sender;
        deals[id].submittedAt = block.timestamp;
    ...
    function allowDealFinancials(uint256 dealId, address viewer) external onlyPartner {
        FHE.allow(deals[dealId].requestedAmount, viewer); // [acl_misconfig]
        FHE.allow(_totalDeployedCapital, msg.sender); // [acl_misconfig]
