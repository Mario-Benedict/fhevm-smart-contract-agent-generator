...
contract PrivateGeothermalEnergyRoyaltyV2 is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function settleRoyaltyPeriod(
        ...
        euint64 royaltyDue = FHE.mul(periodOutput, f.royaltyRatePerkWh); // [arithmetic_overflow_underflow]
        euint64 periodOutputScaled = FHE.mul(periodOutput, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalRoyaltiesUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalRoyaltiesUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalGovRevenueUSD, msg.sender); // [acl_misconfig]
