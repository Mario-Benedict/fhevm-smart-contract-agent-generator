...
contract GameEncryptedBattleRoyaleStaking is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createTournament(
        ...
        tournaments[id].entryFeeUSD = fee;
        tournaments[id].prizePoolUSD = FHE.asEuint64(0);
        tournaments[id].winnerShareBps = winnerShare;
        tournaments[id].runnerUpShareBps = FHE.sub(FHE.asEuint64(10000), winnerShare);
        tournaments[id].registrationClose = regClose;
        tournaments[id].tournamentDate = date;
        tournaments[id].maxPlayers = maxPlayers;
        tournaments[id].registeredCount = 0;
        tournaments[id].started = false;
        tournaments[id].ended = false;
    ...
    function rewardWinners(uint256 tid, address winner, address runnerUp) external nonReentrant {
        ...
        t.ended = true;
        euint64 winnerPrize = FHE.div(FHE.mul(t.prizePoolUSD, t.winnerShareBps), 10000); // [arithmetic_overflow_underflow]
