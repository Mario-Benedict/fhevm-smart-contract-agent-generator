...
contract EncryptedCommunityDevelopmentFinance is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function applyForLoan(
        ...
        id = loanCount++;
        loans[id].applicant = msg.sender;
        loans[id].projectDescription = projectDescription;
        loans[id].requestedAmountUSD = amount;
        loans[id].annualIncomeUSD = income;
        loans[id].dscrScore = dscr;
        loans[id].impactScore = impact;
        loans[id].approvedAmount = FHE.asEuint64(0);
        loans[id].interestRateBps = FHE.asEuint64(0);
        loans[id].riskTier = 0;
        loans[id].approved = false;
        loans[id].disbursed = false;
    ...
    function disburseLoan(uint256 loanId) external {
        ...
        ebool _safeSub187 = FHE.ge(_availableLoanCapital, loan.approvedAmount);
        _availableLoanCapital = FHE.select(_safeSub187, FHE.sub(_availableLoanCapital, loan.approvedAmount), FHE.asEuint64(0));
    ...
    function makeRepayment(
        ...
        ebool _safeSub188 = FHE.ge(_totalLoanBook, principal);
        _totalLoanBook = FHE.select(_safeSub188, FHE.sub(_totalLoanBook, principal), FHE.asEuint64(0));
