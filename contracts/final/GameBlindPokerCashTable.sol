...
contract GameBlindPokerCashTable is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function sitDown(
        ...
        seats[seat].player = msg.sender;
        seats[seat].stackSize = buyIn;
        seats[seat].currentBet = FHE.asEuint64(0);
        seats[seat].totalBetRound = FHE.asEuint64(0);
        seats[seat].handStrength = FHE.asEuint8(0);
        seats[seat].card1 = FHE.asEuint8(0);
        seats[seat].card2 = FHE.asEuint8(0);
        seats[seat].folded = false;
        seats[seat].active = true;
        seats[seat].sittingOut = false;
    ...
    function takeAction(Action action, externalEuint64 encAmount, bytes calldata proof) external nonReentrant {
        ...
        if (action == Action.FOLD) {
            ...
            seat.stackSize = FHE.sub(seat.stackSize, actual); // [arithmetic_overflow_underflow]
