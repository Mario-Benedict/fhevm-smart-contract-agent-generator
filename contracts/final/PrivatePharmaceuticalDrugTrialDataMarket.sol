...
contract PrivatePharmaceuticalDrugTrialDataMarket is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function listClinicalDataset(
        ...
        id = datasetCount++;
        datasets[id].clinicalSite = msg.sender;
        datasets[id].sponsor = sponsor;
        datasets[id].trialPhase = trialPhase;
        datasets[id].protocolRef = protocolRef;
        datasets[id].patientCount = patients;
        datasets[id].dataLicenseFeeUSD = licenseFee;
        datasets[id].sitePerformanceScore = siteScore;
        datasets[id].biomarkerValueUSD = biomarker;
        datasets[id].regulatorySubmissionValue = FHE.asEuint64(0);
        datasets[id].patientRetentionRateBps = retention;
        datasets[id].available = true;
    ...
    function allowMarketStats(address viewer) external onlyOwner {
        FHE.allow(_totalDataMarketRevenueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalDataMarketRevenueUSD, msg.sender); // [acl_misconfig]
    }
