...
contract EncryptedCryptoExchangeMarginEngine is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function openPosition(
        ...
        ebool _safeMul48 = FHE.le(size, FHE.asEuint64(type(uint32).max));
        ...
        PerpPosition storage pos = positions[msg.sender][marketId];
        pos.side = side;
        pos.marginMode = marginMode;
        pos.liquidationStatus = LiquidationStatus.HEALTHY;
        pos.size = size;
        pos.entryPrice = entryPrice;
        pos.markPrice = entryPrice;
        pos.margin = margin;
        pos.unrealizedPnL = FHE.asEuint64(0);
        pos.realizedPnL = FHE.asEuint64(0);
        pos.fundingFeeAccrued = FHE.asEuint64(0);
        pos.maintenanceMargin = maintenanceMargin;
        pos.liquidationPrice = liqPrice;
        pos.adlRankScore = adlScore;
    ...
    function updateMarkPrice(
        ...
        for (uint256 i = 0; i < traderAddresses.length; i++) {
            ...
            ebool _safeSub201 = FHE.ge(newMarkPrice, pos.entryPrice);
            euint64 longProfitDelta = FHE.select(_safeSub201, FHE.sub(newMarkPrice, pos.entryPrice), FHE.asEuint64(0));
            ebool _safeSub202 = FHE.ge(pos.entryPrice, newMarkPrice);
            euint64 shortProfitDelta = FHE.select(_safeSub202, FHE.sub(pos.entryPrice, newMarkPrice), FHE.asEuint64(0));
            ...
            ebool _safeMul49 = FHE.le(priceDelta, FHE.asEuint64(type(uint32).max));
            ...
            ebool _safeMul50 = FHE.le(pos.maintenanceMargin, FHE.asEuint64(type(uint64).max / 2));
    ...
    function applyFunding(
        ...
        for (uint256 i = 0; i < traders.length; i++) {
            ...
            if (!pos.active) continue;
            ebool _safeMul51 = FHE.le(pos.size, FHE.asEuint64(type(uint32).max));
    ...
    function liquidatePosition(address trader, bytes32 marketId) external onlyOwner {
        ...
        pos.active = false;
        ebool _safeSub203 = FHE.ge(pos.margin, pos.maintenanceMargin);
        euint64 surplus = FHE.select(_safeSub203, FHE.sub(pos.margin, pos.maintenanceMargin), FHE.asEuint64(0));
