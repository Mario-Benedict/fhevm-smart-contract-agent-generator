...
contract ConfidentialGridEnergyTrade is ZamaEthereumConfig, Ownable {
    ...
    function buyEnergy(address seller, externalEuint64 unitsRequestedStr, bytes calldata proof) public {
        ...
        ebool _safeMul6 = FHE.le(requested, FHE.asEuint64(type(uint32).max));
        ...
        ebool _safeSub40 = FHE.ge(energyCredits[seller], actualEnergy);
        energyCredits[seller] = FHE.select(_safeSub40, FHE.sub(energyCredits[seller], actualEnergy), FHE.asEuint64(0));
        energyCredits[msg.sender] = FHE.add(energyCredits[msg.sender], actualEnergy);
        
        ebool _safeSub41 = FHE.ge(fiatBalances[msg.sender], actualCost);
        fiatBalances[msg.sender] = FHE.select(_safeSub41, FHE.sub(fiatBalances[msg.sender], actualCost), FHE.asEuint64(0));
