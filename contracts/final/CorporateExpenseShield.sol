...
contract CorporateExpenseShield is ZamaEthereumConfig, AccessControl {
    ...
    function viewMyEncryptedAvailable(address employee) external returns (euint64) {
        require(msg.sender == employee || hasRole(FINANCE_ROLE, msg.sender), "Unauthorized");
        ExpenseAccount storage acc = accounts[employee];
        ebool _safeSub81 = FHE.ge(acc.encryptedAllowance, acc.encryptedSpent);
        return FHE.select(_safeSub81, FHE.sub(acc.encryptedAllowance, acc.encryptedSpent), FHE.asEuint64(0));
    }
}