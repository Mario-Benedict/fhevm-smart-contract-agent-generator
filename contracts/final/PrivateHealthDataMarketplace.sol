...
contract PrivateHealthDataMarketplace is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function licenseData(
        ...
        euint64 platformShare = FHE.sub(fee, patientShare); // [arithmetic_overflow_underflow]
        euint64 patientShareScaled = FHE.mul(patientShare, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
    ...
    function allowMarketStats(address viewer) external onlyOwner {
        FHE.allow(_totalPatientEarningsUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalPatientEarningsUSD, msg.sender); // [acl_misconfig]
