...
contract PrivateMicrofinanceLoanOrigination is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function disburseLoan(
        ...
        loans[id].product = product;
        loans[id].frequency = frequency;
        loans[id].principalUSD = principal;
        loans[id].interestRateBps = rate;
        loans[id].outstandingBalanceUSD = principal;
        loans[id].guaranteeAmountUSD = guarantee;
        loans[id].parRatioBps = FHE.asEuint32(0);
        loans[id].disbursementDate = block.timestamp;
    ...
    function allowPortfolioStats(address viewer) external onlyOwner {
        FHE.allow(_totalDisbursedUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalDisbursedUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalOutstandingUSD, msg.sender); // [acl_misconfig]
