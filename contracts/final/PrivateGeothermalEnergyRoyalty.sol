...
contract PrivateGeothermalEnergyRoyalty is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function activateWell(uint256 wellId, address landowner) external onlyRegulator {
        ...
        FHE.allow(landownerRecords[lrId].totalRoyaltyEarnedUSD, landowner); // [acl_misconfig]
        FHE.allow(_totalSystemOutputMWh, msg.sender); // [acl_misconfig]
        FHE.allow(_totalGovernmentLeviesUSD, msg.sender); // [acl_misconfig]
    }
    ...
    function reportProduction(
        ...
        euint64 grossRev = FHE.mul(outputMWh, w.revenuePerMWhUSD); // [arithmetic_overflow_underflow]
        euint64 outputMWhScaled = FHE.mul(outputMWh, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
