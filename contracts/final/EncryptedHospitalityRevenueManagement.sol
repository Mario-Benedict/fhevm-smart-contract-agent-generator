...
contract EncryptedHospitalityRevenueManagement is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerProperty(
        ...
        id = propertyCount++;
        HotelProperty storage _s0 = properties[id];
        _s0.management = msg.sender;
        _s0.propertyName = name;
        _s0.location = location;
        _s0.starRating = stars;
        _s0.totalRooms = rooms;
        _s0.occupiedRooms = FHE.asEuint32(0);
        _s0.occupancyRateBps = FHE.asEuint32(0);
        _s0.adrCentsUSD = FHE.asEuint64(0);
        _s0.revparCentsUSD = FHE.asEuint64(0);
        _s0.totalRevenueCents = FHE.asEuint64(0);
        _s0.currentSeason = SeasonType.LowSeason;
        _s0.active = true;
    ...
    function makeReservation(
        ...
        id = reservationCount++;
        reservations[id].propertyId = propertyId;
        reservations[id].guest = msg.sender;
        reservations[id].roomCategory = category;
        reservations[id].confirmedRateCents = rate;
        reservations[id].nightsStay = nights;
        reservations[id].totalAmountCents = total;
        reservations[id].checkIn = checkIn;
        reservations[id].checkOut = checkIn + uint256(0) * 1 days;
        reservations[id].cancelled = false;
        ...
        FHE.allowThis(_totalGroupRevenueCents);
        FHE.allowThis(_totalBookings);
        // Allow guest to verify their rate relative to market metrics
        FHE.allow(properties[propertyId].adrCentsUSD, msg.sender); // [acl_misconfig]
        FHE.allow(properties[propertyId].revparCentsUSD, msg.sender); // [acl_misconfig]
        FHE.allow(properties[propertyId].totalRevenueCents, msg.sender); // [acl_misconfig]
