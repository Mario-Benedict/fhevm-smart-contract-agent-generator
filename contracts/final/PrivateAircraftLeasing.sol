...
contract PrivateAircraftLeasing is ZamaEthereumConfig, Ownable {
    ...
    function addAircraft(
        ...
        id = aircraftCount++;
        fleet[id].tailNumber = tail;
        fleet[id].model = model;
        fleet[id].lessor = msg.sender;
        fleet[id].lessee = address(0);
        fleet[id].monthlyLeaseUSD = rate;
        fleet[id].maintenanceReserve = reserve;
        fleet[id].flightHoursUsed = FHE.asEuint32(0);
        fleet[id].contractedHours = FHE.asEuint32(0);
        fleet[id].leaseStart = 0;
        fleet[id].leaseEnd = 0;
        fleet[id].onLease = false;
