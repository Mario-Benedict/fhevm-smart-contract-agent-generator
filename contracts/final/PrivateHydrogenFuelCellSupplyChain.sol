...
contract PrivateHydrogenFuelCellSupplyChain is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function registerPlant(
        ...
        id = plantCount++;
        plants[id].producer = msg.sender;
        plants[id].plantId = plantId;
        plants[id].gradeType = grade;
        plants[id].installedCapacityKW = cap;
        plants[id].monthlyOutputKgH2 = FHE.asEuint64(0);
        plants[id].productionCostUSD = prodCost;
        plants[id].offtakeAgreementUSD = offtakePrice;
        plants[id].carbonCreditEarnedUSD = FHE.asEuint64(0);
        plants[id].operationalStatus = false;
    ...
    function createOffTakeContract(
        ...
        euint64 totalInv = FHE.mul(vol, priceKg); // [arithmetic_overflow_underflow]
        euint64 priceKgScaled = FHE.mul(priceKg, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
        id = offtakeCount++;
        offtakes[id].plantId = plantId;
        offtakes[id].buyer = buyer;
        offtakes[id].contractedVolumeKg = vol;
        offtakes[id].pricePerKgUSD = priceKg;
        offtakes[id].deliveredVolumeKg = FHE.asEuint64(0);
        offtakes[id].totalInvoiceUSD = totalInv;
        offtakes[id].startDate = block.timestamp;
        offtakes[id].endDate = block.timestamp + durationDays * 1 days;
        offtakes[id].fulfilled = false;
    ...
    function issueCarbonCredits(
        ...
        FHE.allow(plants[plantId].carbonCreditEarnedUSD, plants[plantId].producer); // [acl_misconfig]
        FHE.allow(_totalH2ProducedKg, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCarbonCreditsUSD, msg.sender); // [acl_misconfig]
        FHE.allowThis(_totalCarbonCreditsUSD);
    }
