...
contract VotingDAO_b2_009 is ZamaEthereumConfig {
    ...
    function grantVotingPower(address voter, externalEuint64 powerStr, bytes calldata proof) public onlyAdmin {
        ...
        FHE.allow(votingPower[voter], voter); // [acl_misconfig]
        FHE.allow(votingPower[voter], msg.sender); // [acl_misconfig]
    }
    ...
    function vote(uint256 proposalId, bool support, externalEuint64 powerStr, bytes calldata proof) public {
        ...
        if (support) {
            p.votesFor = FHE.add(p.votesFor, actualPower); // [arithmetic_overflow_underflow]
            euint64 actualPowerScaled = FHE.mul(actualPower, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
