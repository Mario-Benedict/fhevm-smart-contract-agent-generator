...
contract PrivateHedgeFundStrategyVault is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function addStrategy(
        ...
        euint64 allocationWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 allocationExposure = FHE.sub(allocationWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
    ...
    function allowVaultStats(address viewer) external onlyOwner {
        FHE.allow(_totalFundNAVUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalSupply, msg.sender); // [acl_misconfig]
        FHE.allow(_totalFundNAVUSD, msg.sender); // [acl_misconfig]
    }
}
