...
contract PrivateLivestockAuction is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function approveBuyer(
        ...
        FHE.allow(buyers[buyer].creditScore, buyer); // [acl_misconfig]
        FHE.allow(buyers[buyer].creditScore, msg.sender); // [acl_misconfig]
    ...
    function createLot(
        ...
        id = lotCount++;
        lots[id].lotId = lotId;
        lots[id].animalType = animalType;
        lots[id].headCount = headCount;
        lots[id].reservePricePerHead = reserve;
        lots[id].avgWeightKg = weight;
        lots[id].healthScore = health;
        lots[id].vaccinationStatus = vax;
        lots[id].auctionClose = auctionClose;
        lots[id].sold = false;
        lots[id].seller = msg.sender;
    ...
    function placeBid(
        ...
        euint64 total = FHE.mul(price, FHE.asEuint64(uint64(lot.headCount))); // [arithmetic_overflow_underflow]
