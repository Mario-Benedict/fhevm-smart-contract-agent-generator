...
contract EncryptedAircraftLeasePortfolio is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function addAircraft(
        ...
        id = fleetSize++;
        Aircraft storage _s0 = fleet[id];
        _s0.tailNumber = tail;
        _s0.model = model;
        _s0.aircraftType = aType;
        _s0.currentLessee = address(0);
        _s0.currentLeaseRateUSD = FHE.asEuint64(0);
        _s0.residualValueUSD = residual;
        _s0.maintenanceReserve = FHE.asEuint64(0);
        _s0.lessorBookValue = book;
        _s0.creditScoreLessee = FHE.asEuint64(0);
        _s0.deliveryDate = 0;
        _s0.leaseEnd = 0;
        _s0.onLease = false;
        _s0.inMaintenance = false;
    ...
    function executeLease(
        ...
        contracts[contractId].aircraftId = aircraftId;
        contracts[contractId].lessee = lessee;
        contracts[contractId].monthlyRateUSD = rate;
        contracts[contractId].securityDepositUSD = deposit;
        contracts[contractId].maintenanceReserveBps = maintBps;
        contracts[contractId].totalPaymentsReceived = FHE.asEuint64(0);
        contracts[contractId].arrearsUSD = FHE.asEuint64(0);
        contracts[contractId].leaseStart = leaseStart;
        contracts[contractId].leaseEnd = leaseEnd;
        contracts[contractId].defaulted = false;
        contracts[contractId].active = true;
    ...
    function receivePayment(uint256 contractId, externalEuint64 encAmount, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub155 = FHE.ge(amount, maintReserve);
        euint64 netPayment = FHE.select(_safeSub155, FHE.sub(amount, maintReserve), FHE.asEuint64(0));
    ...
    function returnAircraft(uint256 aircraftId) external {
        ...
        ebool _safeSub156 = FHE.ge(_monthlyLeaseIncome, ac.currentLeaseRateUSD);
        _monthlyLeaseIncome = FHE.select(_safeSub156, FHE.sub(_monthlyLeaseIncome, ac.currentLeaseRateUSD), FHE.asEuint64(0));
