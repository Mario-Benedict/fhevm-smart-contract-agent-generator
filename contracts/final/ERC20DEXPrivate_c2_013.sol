...
contract ERC20DEXPrivate_c2_013 is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function swapAforB(externalEuint64 encAmountIn, bytes calldata proof) external nonReentrant {
        ...
        euint64 feeAmount = FHE.mul(actualIn, FHE.asEuint64(uint64(feeNumerator))); // [arithmetic_overflow_underflow]
        euint64 actualInScaled = FHE.mul(actualIn, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
