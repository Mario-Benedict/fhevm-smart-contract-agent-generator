...
contract ConfidentialERC20StableCoin is ZamaEthereumConfig, Ownable {
    ...
    function burn(externalEuint64 encAmount, bytes calldata proof) external {
        ...
        ebool _safeSub32 = FHE.ge(_balances[msg.sender], actualBurn);
        _balances[msg.sender] = FHE.select(_safeSub32, FHE.sub(_balances[msg.sender], actualBurn), FHE.asEuint64(0));
        ebool _safeSub33 = FHE.ge(_totalSupply, actualBurn);
        _totalSupply = FHE.select(_safeSub33, FHE.sub(_totalSupply, actualBurn), FHE.asEuint64(0));
        ...
        ebool _safeSub34 = FHE.ge(_collateralDeposited[msg.sender], released);
        _collateralDeposited[msg.sender] = FHE.select(_safeSub34, FHE.sub(_collateralDeposited[msg.sender], released), FHE.asEuint64(0));
        ebool _safeSub35 = FHE.ge(_totalCollateral, released);
        _totalCollateral = FHE.select(_safeSub35, FHE.sub(_totalCollateral, released), FHE.asEuint64(0));
        FHE.allowThis(_balances[msg.sender]);
        FHE.allow(_balances[msg.sender], msg.sender);
        FHE.allowThis(_totalSupply);
        FHE.allowThis(_collateralDeposited[msg.sender]);
    ...
    function transfer(address to, externalEuint64 encAmount, bytes calldata proof) external {
        ...
        ebool _safeSub36 = FHE.ge(_balances[msg.sender], actual);
        _balances[msg.sender] = FHE.select(_safeSub36, FHE.sub(_balances[msg.sender], actual), FHE.asEuint64(0));
