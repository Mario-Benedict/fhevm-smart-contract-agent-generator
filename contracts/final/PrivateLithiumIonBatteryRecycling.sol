...
contract PrivateLithiumIonBatteryRecycling is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function processBatch(
        ...
        euint64 recoveryValWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 recoveryValExposure = FHE.sub(recoveryValWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
        euint64 cost = FHE.fromExternal(encCost, costProof);
        id = batchCount++;
        RecyclingBatch storage _s0 = batches[id];
        _s0.recycler = msg.sender;
        _s0.batteryProvider = batteryProvider;
        _s0.chemistry = chemistry;
        _s0.process = process;
        _s0.batteryWeightKg = weight;
        _s0.lithiumRecoveredKg = lithium;
        _s0.cobaltRecoveredKg = cobalt;
        _s0.nickelRecoveredKg = nickel;
        _s0.recoveryValueUSD = recoveryVal;
        _s0.processingCostUSD = cost;
        _s0.complianceScoreBps = FHE.asEuint16(0);
        _s0.carbonCreditsEarned = FHE.asEuint64(0);
        _s0.processedAt = block.timestamp;
    ...
    function allowSystemStats(address viewer) external onlyOwner {
        FHE.allow(_totalRecoveryValueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalRecoveryValueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalCarbonCreditsIssued, msg.sender); // [acl_misconfig]
