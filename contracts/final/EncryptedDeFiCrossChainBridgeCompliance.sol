...
contract EncryptedDeFiCrossChainBridgeCompliance is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function createLane(
        ...
        laneId = laneCount++;
        BridgeLane storage _s0 = lanes[laneId];
        _s0.sourceChain = src;
        _s0.destChain = dst;
        _s0.tokenSymbol = token;
        _s0.liquidityReserveUSD = liquidity;
        _s0.totalVolumeBridged = FHE.asEuint64(0);
        _s0.dailyVolumeCap = dailyCap;
        _s0.todayVolume = FHE.asEuint64(0);
        _s0.feeBps = fee;
        _s0.totalFeesEarned = FHE.asEuint64(0);
        _s0.txCount = FHE.asEuint32(0);
        _s0.complianceScore = FHE.asEuint8(95);
        _s0.status = BridgeStatus.ACTIVE;
    ...
    function initiateBridge(
        ...
        ebool _safeSub211 = FHE.ge(amount, fee);
        euint64 received = FHE.select(_safeSub211, FHE.sub(amount, fee), FHE.asEuint64(0));
        txId = txCount++;
        txs[txId].laneId = laneId;
        txs[txId].sender = msg.sender;
        txs[txId].recipient = recipient;
        txs[txId].amountUSD = amount;
        txs[txId].feeUSD = fee;
        txs[txId].receivedAmountUSD = received;
        txs[txId].initiatedAt = block.timestamp;
        txs[txId].completed = false;
        txs[txId].flagged = false;
