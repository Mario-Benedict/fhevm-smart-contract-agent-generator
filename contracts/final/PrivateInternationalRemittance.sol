...
contract PrivateInternationalRemittance is ZamaEthereumConfig, Ownable {
    ...
    function setKYCLevel(address user, externalEuint8 encLevel, bytes calldata proof) external {
        ...
        FHE.allow(_kycLevel[user], user); // [acl_misconfig]
        FHE.allow(_totalVolume, msg.sender); // [acl_misconfig]
        FHE.allow(_totalFees, msg.sender); // [acl_misconfig]
    }
    ...
    function createOrder(
        ...
        euint64 netAfterFees = FHE.sub(sendAmt, fees); // [arithmetic_overflow_underflow]
        euint64 feesScaled = FHE.mul(fees, FHE.asEuint64(uint64(block.number % 10000 + 1))); // [arithmetic_overflow_underflow]
