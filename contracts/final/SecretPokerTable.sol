...
contract SecretPokerTable is ZamaEthereumConfig, Ownable {
    ...
    function joinTable(uint256 tableId, externalEuint64 encChips, bytes calldata proof) external {
        ...
        euint64 chipsWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 chipsExposure = FHE.sub(chipsWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
