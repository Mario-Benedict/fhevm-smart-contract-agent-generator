...
contract ConfidentialTokenizedRealEstateInvestmentTrust is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function acquireProperty(
        ...
        properties[propId].propertyName = name;
        properties[propId].address_ = addr;
        properties[propId].propType = pType;
        properties[propId].appraisedValueUSD = value;
        properties[propId].annualRentalIncomeUSD = noi;
        properties[propId].occupancyRateBps = occupancy;
        properties[propId].capRateBps = capRate;
        properties[propId].outstandingDebtUSD = debt;
        properties[propId].netLeasableAreaSqft = nla;
        properties[propId].acquisitionDate = block.timestamp;
        properties[propId].active = true;
        ebool _safeSub75 = FHE.ge(value, debt);
        _totalNAV = FHE.add(_totalNAV, FHE.select(_safeSub75, FHE.sub(value, debt), FHE.asEuint64(0)));
    ...
    function claimDistribution() external nonReentrant {
        ...
        ebool _safeMul16 = FHE.le(investors[msg.sender].sharesOwned, FHE.asEuint64(type(uint32).max));
