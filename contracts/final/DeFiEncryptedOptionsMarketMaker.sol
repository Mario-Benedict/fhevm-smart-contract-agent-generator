...
contract DeFiEncryptedOptionsMarketMaker is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function writeOption(
        ...
        ebool _safeMul22 = FHE.le(contractsNum, FHE.asEuint64(type(uint32).max));
    ...
    function buyOption(
        ...
        ebool _safeMul23 = FHE.le(contractsNum, FHE.asEuint64(type(uint32).max));
        euint64 totalPremium = FHE.mul(contractsNum, o.premiumCents);
        euint64 protocolFee = FHE.div(totalPremium, 100); // 1% fee
        ebool _safeSub110 = FHE.ge(totalPremium, protocolFee);
        euint64 netPremium = FHE.select(_safeSub110, FHE.sub(totalPremium, protocolFee), FHE.asEuint64(0));
    ...
    function exerciseOption(uint256 optionId) external nonReentrant {
        ...
        ebool _safeMul24 = FHE.le(bp.contractsHeld, FHE.asEuint64(type(uint32).max));
