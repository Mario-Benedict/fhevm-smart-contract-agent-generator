...
contract EncryptedCrowdfundingLaunchpad is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function contribute(uint256 campaignId, externalEuint64 encAmount, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub196 = FHE.ge(c.hardCap, c.totalRaised);
        euint64 remaining = FHE.select(_safeSub196, FHE.sub(c.hardCap, c.totalRaised), FHE.asEuint64(0));
    ...
    function creatorWithdraw(uint256 campaignId) external nonReentrant {
        ...
        ebool _safeSub197 = FHE.ge(c.totalRaised, fee);
        euint64 creatorNet = FHE.select(_safeSub197, FHE.sub(c.totalRaised, fee), FHE.asEuint64(0));
