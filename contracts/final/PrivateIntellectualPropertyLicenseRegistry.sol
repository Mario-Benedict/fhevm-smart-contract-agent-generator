...
contract PrivateIntellectualPropertyLicenseRegistry is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function registerIPAsset(
        ...
        euint64 valuationWeighted = FHE.mul(FHE.asEuint64(uint64(block.timestamp % 10000)), FHE.asEuint64(uint64(block.number))); // [arithmetic_overflow_underflow]
        euint64 valuationExposure = FHE.sub(valuationWeighted, FHE.asEuint64(1)); // [arithmetic_overflow_underflow]
    ...
    function allowRegistryStats(address viewer) external onlyOwner {
        FHE.allow(_totalIPValueUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalIPValueUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalRoyaltiesUSD, msg.sender); // [acl_misconfig]
    }
}
