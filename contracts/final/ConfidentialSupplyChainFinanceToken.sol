...
contract ConfidentialSupplyChainFinanceToken is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function submitInvoice(
        ...
        ebool _safeSub69 = FHE.ge(faceValue, discount);
        euint64 earlyPay     = FHE.select(_safeSub69, FHE.sub(faceValue, discount), FHE.asEuint64(0));
        id = invoiceCount++;
        invoices[id].supplier = msg.sender;
        invoices[id].buyer = buyer;
        invoices[id].invoiceRef = invoiceRef;
        invoices[id].faceValueUSD = faceValue;
        invoices[id].discountRateBps = discountRate;
        invoices[id].earlyPayAmountUSD = earlyPay;
        invoices[id].supplierCreditScore = creditScore;
        invoices[id].supplyChainRiskScore = riskScore;
        invoices[id].dueDate = block.timestamp + dueDays * 1 days;
        invoices[id].financed = false;
        invoices[id].settled = false;
    ...
    function financeInvoice(uint256 invoiceId) external nonReentrant {
        ...
        ebool _safeSub70 = FHE.ge(inv.faceValueUSD, inv.earlyPayAmountUSD);
        _totalDiscountEarnedUSD = FHE.add(_totalDiscountEarnedUSD, FHE.select(_safeSub70, FHE.sub(inv.faceValueUSD, inv.earlyPayAmountUSD), FHE.asEuint64(0)));
        inv.financed = true;
    ...
    function transfer(address to, externalEuint64 encAmt, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub71 = FHE.ge(_balances[msg.sender], eff);
        _balances[msg.sender] = FHE.select(_safeSub71, FHE.sub(_balances[msg.sender], eff), FHE.asEuint64(0));
