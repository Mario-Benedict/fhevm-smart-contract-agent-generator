...
contract PrivateTelecommunicationsNetworkSharingAgreement is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function allowIndustryStats(address viewer) external onlyOwner {
        FHE.allow(_totalInfrastructureSavingsUSD, viewer); // [acl_misconfig]
        FHE.allow(_totalInfrastructureSavingsUSD, msg.sender); // [acl_misconfig]
        FHE.allow(_totalEnergySavingsUSD, msg.sender); // [acl_misconfig]
