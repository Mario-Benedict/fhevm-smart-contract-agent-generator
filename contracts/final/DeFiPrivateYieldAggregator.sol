...
contract DeFiPrivateYieldAggregator is ZamaEthereumConfig, Ownable, ReentrancyGuard {
    ...
    function harvest(externalEuint64 encShares, bytes calldata proof) external nonReentrant {
        ...
        ebool _safeSub140 = FHE.ge(ud.shares, actual);
        ud.shares = FHE.select(_safeSub140, FHE.sub(ud.shares, actual), FHE.asEuint64(0));
        ebool _safeSub141 = FHE.ge(ud.amount, actual);
        ud.amount = FHE.select(_safeSub141, FHE.sub(ud.amount, actual), FHE.asEuint64(0));
        ebool _safeSub142 = FHE.ge(_totalShares, actual);
        _totalShares = FHE.select(_safeSub142, FHE.sub(_totalShares, actual), FHE.asEuint64(0));
        ebool _safeSub143 = FHE.ge(_totalDeposited, actual);
        _totalDeposited = FHE.select(_safeSub143, FHE.sub(_totalDeposited, actual), FHE.asEuint64(0));
