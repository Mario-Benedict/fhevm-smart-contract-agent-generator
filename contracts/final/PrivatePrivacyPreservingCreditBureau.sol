...
contract PrivatePrivacyPreservingCreditBureau is ZamaEthereumConfig, Ownable, ReentrancyGuard, Pausable {
    ...
    function createProfile(
        ...
        profiles[msg.sender].consumer = msg.sender;
        profiles[msg.sender].ficoCreditScore = score;
        profiles[msg.sender].totalDebtUSD = debt;
        profiles[msg.sender].totalCreditLimitUSD = limit;
        profiles[msg.sender].utilizationRateBps = utilization;
        profiles[msg.sender].inquiriesLast12M = inqs;
        profiles[msg.sender].accountAgeMonths = age;
        profiles[msg.sender].profileCreated = block.timestamp;
    ...
    function addTradeline(
        ...
        id = tradelineCount++;
        tradelines[id].consumer = consumer;
        tradelines[id].creditor = msg.sender;
        tradelines[id].tlType = tlType;
        tradelines[id].originalBalanceUSD = original;
        tradelines[id].currentBalanceUSD = current;
        tradelines[id].creditLimitUSD = limit;
        tradelines[id].paymentHistoryScore = payHist;
        tradelines[id].latestStatus = status;
        tradelines[id].openedDate = block.timestamp;
        tradelines[id].closedDate = 0;
    ...
    function allowBureauStats(address viewer) external onlyOwner {
        FHE.allow(_totalProfilesCreated, viewer); // [acl_misconfig]
        FHE.allow(_totalProfilesCreated, msg.sender); // [acl_misconfig]
    }
